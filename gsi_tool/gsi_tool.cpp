...
static int Install([[maybe_unused]] sp<IVold> vold, int argc, char** argv) {
    ...
    while ((rv = getopt_long_only(argc, argv, "", options, &index)) != -1) {
        switch (rv) {
            case 's':
                if (!android::base::ParseInt(optarg, &image_size) || image_size <= 0) {
    ...
    if (image_size <= 0) {
        std::cout << "Must specify --image-size." << std::endl;
        return EX_USAGE;
    }
    ...
    auto status = vold->startLiveImageInstall(image_size);
    if (!status.isOk()) {
    ...
    status = vold->commitLiveImageChunk(input, image_size);
    if (!status.isOk()) {
    ...
    status = vold->setLiveImageBootable();
    if (!status.isOk()) {
...
static int Wipe(sp<IVold> vold, int argc, char** /* argv */) {
    ...
    auto status = vold->removeLiveImageInstall();
    if (!status.isOk()) {
