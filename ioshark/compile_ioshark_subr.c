...
void files_db_write_objects(FILE* fp) {
    ...
    for (i = 0; i < FILE_DB_HASHSIZE; i++) {
        ...
        while (db_node != NULL) {
            ...
            st.global_filename_ix = db_node->global_filename_ix;
...
void* files_db_lookup(char* pathname) {
    ...
    while (db_node != NULL) {
        if (strcmp(db_node->filename, pathname) == 0) break;
        db_node = db_node->next;
    }
    return db_node;
}
...
void* files_db_add(char* filename) {
    ...
    if ((db_node = files_db_lookup(filename))) return db_node;
    ...
    db_node->global_filename_ix = filename_cache_lookup(filename);
...
void init_filename_cache(void) {
    ...
    filename_cache = calloc(filename_cache_size, sizeof(struct ioshark_filename_struct));
    if (filename_cache == NULL) {
        fprintf(stderr, "%s Can't allocate memory - this is fatal\n", __func__);
        exit(EXIT_FAILURE);
    }
    if (fread(filename_cache, sizeof(struct ioshark_filename_struct), filename_cache_num_entries,
              filename_cache_fp) != (size_t)filename_cache_num_entries) {
        fprintf(stderr, "%s Can't read ioshark_filenames file\n", progname);
        exit(EXIT_FAILURE);
    }
    if (file_exists) fclose(filename_cache_fp);
}
...
static int filename_cache_lookup(char* filename) {
    ...
    for (i = 0; i < filename_cache_num_entries; i++) {
        if (strcmp(filename_cache[i].path, filename) == 0) return i;
    }
    if (filename_cache_num_entries >= filename_cache_size) {
        ...
        newsize = filename_cache_size * sizeof(struct ioshark_filename_struct);
    ...
    strcpy(filename_cache[filename_cache_num_entries].path, filename);
...
void store_filename_cache(void) {
    ...
    if (fwrite(filename_cache, sizeof(struct ioshark_filename_struct), filename_cache_num_entries,
               filename_cache_fp) != (size_t)filename_cache_num_entries) {
        fprintf(stderr, "%s Can't read ioshark_filenames file\n", progname);
        exit(EXIT_FAILURE);
    }
