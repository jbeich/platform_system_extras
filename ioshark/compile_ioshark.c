...
struct flags_map_s open_flags_map[] = {{"O_RDONLY", O_RDONLY},
                                       {"O_WRONLY", O_WRONLY},
                                       {"O_RDWR", O_RDWR},
                                       {"O_CREAT", O_CREAT},
                                       {"O_SYNC", O_SYNC},
...
struct flags_map_s fileop_map[] = {{"lseek", IOSHARK_LSEEK},         {"_llseek", IOSHARK_LLSEEK},
                                   {"pread64", IOSHARK_PREAD64},     {"pwrite64", IOSHARK_PWRITE64},
                                   {"read", IOSHARK_READ},           {"write", IOSHARK_WRITE},
                                   {"mmap", IOSHARK_MMAP},           {"mmap2", IOSHARK_MMAP2},
                                   {"openat", IOSHARK_OPEN},         {"fsync", IOSHARK_FSYNC},
                                   {"fdatasync", IOSHARK_FDATASYNC}, {"close", IOSHARK_CLOSE},
...
void init_prev_time(struct timeval* tv) {
 ...
 */
static u_int64_t get_delta_ts(char* buf, struct timeval* prev) {
...
void get_tracetype(char* buf, char* trace_type) {
    char *s, *s2;

    *trace_type = '\0';
    s = strchr(buf, ' ');
    if (s == NULL) {
        fprintf(stderr, "%s Malformed Trace Type ? %s\n", progname, __func__);
        exit(EXIT_FAILURE);
    }
    while (*s == ' ') s++;
    ...
    if (strcmp(trace_type, "strace") != 0 && strcmp(trace_type, "ftrace") != 0) {
        fprintf(stderr, "%s Unknown/Missing Trace Type (has to be strace|ftrace) %s\n", progname,
                __func__);
        exit(EXIT_FAILURE);
    }
    /*
    ...
    if (*s2 == '\0') {
         ...
         */
        fprintf(stderr, "%s Mal-formed strace/ftrace record %s:%s\n", progname, __func__, buf);
        exit(EXIT_FAILURE);
    }
...
void get_pathname(char* buf, char* pathname, enum file_op file_op) {
    char *s, *s2, save;
...
void get_syscall(char* buf, char* syscall) {
    char *s, *s2;
...
void get_mmap_offset_len_prot(char* buf, int* prot, off_t* offset, u_int64_t* len) {
    char *s, *s1;
    int i;
    char protstr[128];
    ...
    *s = '\0';
    strcpy(protstr, s1);
    *prot = 0;
    if (strstr(protstr, "PROT_READ")) *prot |= IOSHARK_PROT_READ;
    if (strstr(protstr, "PROT_WRITE")) *prot |= IOSHARK_PROT_WRITE;
    *s = ',';
    s += 2;
    for (i = 0; i < 2; i++) {
        s = strchr(s, ',');
...
void get_lseek_offset_action(char* buf, enum file_op op, off_t* offset, char* action) {
    char *s, *s2;
...
void get_rw_len(char* buf, u_int64_t* len) {
    char* s_len;
...
void get_prw64_offset_len(char* buf, off_t* offset, u_int64_t* len) {
    char *s_offset, *s_len;
...
void get_ftrace_offset_len(char* buf, off_t* offset, u_int64_t* len) {
    char* s_offset;
...
void get_openat_flags_mode(char* buf, char* flags, mode_t* mode) {
    char *s, *s2, lookfor;
...
int lookup_map(char* s, struct flags_map_s* flags_map, int maplen) {
    int found = 0, flag = 0;
    int i;
...
int map_open_flags(char* s) {
    int flags = 0;
    char* s1;

    while ((s1 = strchr(s, '|'))) {
        *s1 = '\0';
        flags |= lookup_map(s, open_flags_map, ARRAY_SIZE(open_flags_map));
        *s1 = '|';
        s = s1 + 1;
    }
    /* Last option */
...
int map_lseek_action(char* s) {
    int flags = 0;
    char* s1;

    while ((s1 = strchr(s, '|'))) {
        *s1 = '\0';
        flags |= lookup_map(s, lseek_action_map, ARRAY_SIZE(lseek_action_map));
        *s1 = '|';
        s = s1 + 1;
    }
    /* Last option */
...
enum file_op map_syscall(char* syscall) {
    return lookup_map(syscall, fileop_map, ARRAY_SIZE(fileop_map));
}
...
int main(int argc, char** argv) {
    ...
    while (fgets(in_buf, 2048, fp)) {
        ...
        switch (disk_file_op->ioshark_io_op) {
            case IOSHARK_LLSEEK:
            case IOSHARK_LSEEK:
                get_lseek_offset_action(s, disk_file_op->ioshark_io_op,
                                        (off_t*)&disk_file_op->lseek_offset, lseek_action_str);
                disk_file_op->lseek_action = map_lseek_action(lseek_action_str);
                if (disk_file_op->lseek_action == SEEK_SET)
                    files_db_update_size(db_node, disk_file_op->lseek_offset);
                break;
            case IOSHARK_PREAD64:
            case IOSHARK_PWRITE64:
                get_prw64_offset_len(s, (off_t*)&disk_file_op->prw_offset,
                                     (u_int64_t*)&disk_file_op->prw_len);
                files_db_update_size(db_node, disk_file_op->prw_offset + disk_file_op->prw_len);
                break;
            case IOSHARK_READ:
            case IOSHARK_WRITE:
            ...
            case IOSHARK_MMAP:
            case IOSHARK_MMAP2:
                get_mmap_offset_len_prot(s, (int*)&disk_file_op->mmap_prot,
                                         (off_t*)&disk_file_op->mmap_offset,
                                         (u_int64_t*)&disk_file_op->mmap_len);
                files_db_update_size(db_node, disk_file_op->mmap_offset + disk_file_op->mmap_len);
                break;
            case IOSHARK_OPEN:
                disk_file_op->open_mode = 0;
                get_openat_flags_mode(s, open_flags_str, &disk_file_op->open_mode);
                disk_file_op->open_flags = map_open_flags(open_flags_str);
                break;
            case IOSHARK_FSYNC:
            ...
            case IOSHARK_MAPPED_PREAD:
                ...
                get_ftrace_offset_len(s, (off_t*)&disk_file_op->prw_offset,
                                      (u_int64_t*)&disk_file_op->prw_len);
                files_db_update_size(db_node, disk_file_op->prw_offset + disk_file_op->prw_len);
                break;
            default:
