...
void usage() {
    fprintf(stderr,
            "%s [-b blockdev_name] [-d preserve_delays] [-n num_iterations] [-t num_threads] -q -v "
            "| -s <list of parsed input files>\n",
            progname);
 ...
 */
static void update_time(struct timeval* aggr_time, struct timeval* delta_time) {
    struct timeval tmp;
...
static void update_op_counts(u_int64_t* op_counts) {
    int i;

    pthread_mutex_lock(&stats_mutex);
    for (i = IOSHARK_LSEEK; i < IOSHARK_MAX_FILE_OP; i++) aggr_op_counts[i] += op_counts[i];
    pthread_mutex_unlock(&stats_mutex);
}

static void update_byte_counts(struct rw_bytes_s* dest, struct rw_bytes_s* delta) {
    pthread_mutex_lock(&stats_mutex);
...
void init_work(int next_file, int num_files) {
...
static struct thread_state_s* get_work() {
    struct thread_state_s* work = NULL;

    pthread_mutex_lock(&work_mutex);
    if (work_next_file < work_num_files) work = &thread_state[work_next_file++];
    pthread_mutex_unlock(&work_mutex);
    return work;
}

static void create_files(struct thread_state_s* state) {
    int i;
    ...
    for (i = 0; i < state->num_files; i++) {
         ...
         */
        filename = get_ro_filename(file_state.global_filename_ix);
        if (quick_mode) assert(filename != NULL);
        if (quick_mode == 0 || is_readonly_mount(filename, file_state.size) == 0) {
            sprintf(path, "file.%d.%" PRIu64 "", (int)(state - thread_state), file_state.fileno);
            create_file(path, file_state.size, &rw_bytes);
            filename = path;
            readonly = 0;
        } else {
            readonly = 1;
        }
        db_node = files_db_add_byfileno(state->db_handle, file_state.fileno, readonly);
...
static void do_one_io(void* db_node, struct ioshark_file_operation* file_op, u_int64_t* op_counts,
                      struct rw_bytes_s* rw_bytes, char** bufp, int* buflen) {
    ...
    switch (file_op->ioshark_io_op) {
        ...
        case IOSHARK_LSEEK:
        case IOSHARK_LLSEEK:
            ret = lseek(files_db_get_fd(db_node), file_op->lseek_offset, file_op->lseek_action);
            if (ret < 0) {
                fprintf(stderr, "%s: lseek(%s %" PRIu64 " %d) returned error %d\n", progname,
                        files_db_get_filename(db_node), file_op->lseek_offset,
        ...
        case IOSHARK_PREAD64:
            ...
            ret = pread(files_db_get_fd(db_node), p, file_op->prw_len, file_op->prw_offset);
            rw_bytes->bytes_read += file_op->prw_len;
            if (ret < 0) {
                fprintf(stderr, "%s: pread(%s %" PRIu64 " %" PRIu64 ") error %d\n", progname,
                        files_db_get_filename(db_node), file_op->prw_len, file_op->prw_offset,
                        errno);
                exit(EXIT_FAILURE);
            }
            break;
        case IOSHARK_PWRITE64:
            ...
            ret = pwrite(files_db_get_fd(db_node), p, file_op->prw_len, file_op->prw_offset);
            rw_bytes->bytes_written += file_op->prw_len;
            if (ret < 0) {
                fprintf(stderr, "%s: pwrite(%s %" PRIu64 " %" PRIu64 ") error %d\n", progname,
                        files_db_get_filename(db_node), file_op->prw_len, file_op->prw_offset,
                        errno);
                exit(EXIT_FAILURE);
            }
        ...
        case IOSHARK_READ:
            ...
            ret = read(files_db_get_fd(db_node), p, file_op->rw_len);
            rw_bytes->bytes_read += file_op->rw_len;
            if (ret < 0) {
                fprintf(stderr, "%s: read(%s %" PRIu64 ") error %d\n", progname,
                        files_db_get_filename(db_node), file_op->rw_len, errno);
                exit(EXIT_FAILURE);
            }
            break;
        case IOSHARK_WRITE:
            p = get_buf(bufp, buflen, file_op->rw_len, 1);
            ret = write(files_db_get_fd(db_node), p, file_op->rw_len);
            rw_bytes->bytes_written += file_op->rw_len;
            if (ret < 0) {
                fprintf(stderr, "%s: write(%s %" PRIu64 ") error %d\n", progname,
                        files_db_get_filename(db_node), file_op->rw_len, errno);
                exit(EXIT_FAILURE);
            }
            break;
        case IOSHARK_MMAP:
        case IOSHARK_MMAP2:
            ioshark_handle_mmap(db_node, file_op, bufp, buflen, op_counts, rw_bytes);
            break;
        case IOSHARK_OPEN:
            if (file_op->open_flags & O_CREAT) {
                fd = open(files_db_get_filename(db_node), file_op->open_flags, file_op->open_mode);
                if (fd < 0) {
                    /*
                    ...
                    if (errno == EEXIST) {
                        ...
                        fprintf(stderr, "%s: O_CREAT open(%s %x %o) error %d\n", progname,
                                files_db_get_filename(db_node), file_op->open_flags,
                                file_op->open_mode, errno);
                        exit(EXIT_FAILURE);
                    }
                }
            } else {
                fd = open(files_db_get_filename(db_node), file_op->open_flags);
                if (fd < 0) {
                    if (file_op->open_flags & O_DIRECTORY) {
                        ...
                        fprintf(stderr, "%s: open(%s %x) error %d\n", progname,
                                files_db_get_filename(db_node), file_op->open_flags, errno);
                        exit(EXIT_FAILURE);
                    }
                }
            }
        ...
        case IOSHARK_FDATASYNC:
            if (file_op->ioshark_io_op == IOSHARK_FSYNC) {
                ...
                if (ret < 0) {
                    fprintf(stderr, "%s: fsync(%s) error %d\n", progname,
                ...
                if (ret < 0) {
                    fprintf(stderr, "%s: fdatasync(%s) error %d\n", progname,
                            files_db_get_filename(db_node), errno);
                    exit(EXIT_FAILURE);
                }
            }
        ...
        case IOSHARK_CLOSE:
            ...
            if (ret < 0) {
                fprintf(stderr, "%s: close(%s) error %d\n", progname,
                        files_db_get_filename(db_node), errno);
                exit(EXIT_FAILURE);
            }
...
static void do_io(struct thread_state_s* state) {
    void* db_node;
    ...
    fseek(state->fp,
          sizeof(struct ioshark_header) + header.num_files * sizeof(struct ioshark_file_state),
          SEEK_SET);
    /*
    ...
    for (i = 0; i < (int)header.num_io_operations; i++) {
        ...
        db_node = files_db_lookup_byfileno(state->db_handle, file_op.fileno);
        if (db_node == NULL) {
            fprintf(stderr, "%s Can't lookup fileno %" PRIu64 ", fatal error\n", progname,
                    file_op.fileno);
            fprintf(stderr, "%s state filename %s, i %d\n", progname, state->filename, i);
            goto fail;
        }
        if (file_op.ioshark_io_op != IOSHARK_OPEN && files_db_get_fd(db_node) == -1) {
            int openflags;
            ...
            if (files_db_readonly(db_node))
                ...
                openflags = O_RDWR;
            fd = open(files_db_get_filename(db_node), openflags);
            if (fd < 0) {
                fprintf(stderr, "%s: open(%s %x) error %d\n", progname,
                        files_db_get_filename(db_node), openflags, errno);
                goto fail;
            }
            files_db_update_fd(db_node, fd);
        }
        do_one_io(db_node, &file_op, op_counts, &rw_bytes, &buf, &buflen);
    }
...
void* io_thread(void* unused __attribute__((unused))) {
    struct thread_state_s* state;

    srand(gettid());
    while ((state = get_work())) do_io(state);
    pthread_exit(NULL);
    return (NULL);
}

static void do_create(struct thread_state_s* state) {
    struct ioshark_header header;
...
void* create_files_thread(void* unused __attribute__((unused))) {
    struct thread_state_s* state;

    while ((state = get_work())) do_create(state);
    pthread_exit(NULL);
    return (NULL);
}

int get_start_end(int* start_ix) {
    ...
    if (fssize_clamp_next_index == num_input_files) return 0;
    ...
    for (i = fssize_clamp_next_index; i < num_input_files; i++) {
        ...
        for (j = 0; j < (int)header.num_files; j++) {
            ...
            if (quick_mode == 0 ||
                !is_readonly_mount(get_ro_filename(file_state.global_filename_ix),
                                   file_state.size)) {
    ...
    if (verbose) {
        if (chunk > 0 || i < num_input_files) {
            printf("Breaking up input files, Chunk %d: %d to %d\n", chunk++,
                   fssize_clamp_next_index, i - 1);
        } else {
            printf("Entire Dataset fits start = %d to %d, free_bytes = %ju\n",
                   fssize_clamp_next_index, i - fssize_clamp_next_index, free_fs_bytes);
        }
    }
...
int ioshark_pthread_create(pthread_t* tidp, void* (*start_routine)(void*)) {
    pthread_attr_t attr;
...
static void sizeup_fd_limits(void) {
    struct rlimit r;

    getrlimit(RLIMIT_NOFILE, &r);
    if (r.rlim_cur >= IOSHARK_FD_LIM) /* cur limit already at what we want */
        return;
    /*
     ...
     */
    if (r.rlim_max < IOSHARK_FD_LIM) r.rlim_max = IOSHARK_FD_LIM;
...
int main(int argc, char** argv) {
    ...
    if ((verbose + summary_mode) == 2) usage();

    if (num_threads > MAX_THREADS) usage();
    ...
    if (quick_mode) init_filename_cache();
    ...
    while ((num_files = get_start_end(&start_file))) {
        ...
        /* Create files once */
        if (!summary_mode) printf("Doing Pre-creation of Files\n");
        if (quick_mode && !summary_mode) printf("Skipping Pre-creation of read-only Files\n");
        if (num_threads == 0 || num_threads > num_files) num_threads = num_files;
        (void)system("echo 3 > /proc/sys/vm/drop_caches");
        init_work(start_file, num_files);
        (void)gettimeofday(&time_for_pass, (struct timezone*)NULL);
        for (i = 0; i < num_threads; i++) {
            if (ioshark_pthread_create(&(tid[i]), create_files_thread)) {
                fprintf(stderr, "%s: Can't create creator thread %d\n", progname, i);
                exit(EXIT_FAILURE);
            }
        }
        ...
        for (i = 0; i < num_iterations; i++) {
            ...
            for (c = 0; c < num_threads; c++) {
                if (ioshark_pthread_create(&(tid[c]), io_thread)) {
                    fprintf(stderr, "%s: Can't create thread %d\n", progname, c);
                    exit(EXIT_FAILURE);
                }
            }
            wait_for_threads(num_threads);
            update_delta_time(&time_for_pass, &aggregate_IO_time);
        }
    ...
    if (!summary_mode) {
        printf("Total Creation time = %ju.%ju (msecs.usecs)\n",
               get_msecs(&aggregate_file_create_time), get_usecs(&aggregate_file_create_time));
        printf("Total Remove time = %ju.%ju (msecs.usecs)\n",
               get_msecs(&aggregate_file_remove_time), get_usecs(&aggregate_file_remove_time));
        if (do_delay)
        ...
        if (verbose) print_bytes("Upfront File Creation bytes", &aggr_create_rw_bytes);
        print_bytes("Total Test (IO) bytes", &aggr_io_rw_bytes);
        if (verbose) print_op_stats(aggr_op_counts);
        report_cpu_disk_util();
    } else {
        printf("%ju.%ju ", get_msecs(&aggregate_file_create_time),
               get_usecs(&aggregate_file_create_time));
        printf("%ju.%ju ", get_msecs(&aggregate_file_remove_time),
               get_usecs(&aggregate_file_remove_time));
        if (do_delay)
            printf("%ju.%ju ", get_msecs(&aggregate_delay_time), get_usecs(&aggregate_delay_time));
        printf("%ju.%ju ", get_msecs(&aggregate_IO_time), get_usecs(&aggregate_IO_time));
        print_bytes(NULL, &aggr_io_rw_bytes);
        report_cpu_disk_util();
        printf("\n");
    }
    if (quick_mode) free_filename_cache();
}
