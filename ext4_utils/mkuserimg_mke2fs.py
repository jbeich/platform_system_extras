...
def ParseArguments(argv):
  ...
  parser.add_argument("ext_variant", help="Supported variant: ext2, ext4.")
...
def ConstructE2fsCommands(args):
  ...
  make_ext4fs_cmd = ["mke2fs"] + mke2fs_opts + \
                    ["-t", args.ext_variant, "-b", str(BLOCKSIZE),
                     args.output_file, str(blocks)]

  e2fsdroid_cmd = ["e2fsdroid"] + e2fsdroid_opts + \
                  ["-f", args.src_dir, "-a", args.mount_point, args.output_file]

  return make_ext4fs_cmd, e2fsdroid_cmd
...
def main(argv):
  ...
  if not args.ext_variant in ["ext2", "ext4"]:
    logging.error("Only ext2/4 are supported, got %s", args.ext_variant)
    sys.exit(3)
  ...
  make_ext4fs_cmd, e2fsdroid_cmd = ConstructE2fsCommands(args)
