...
class MkuserimgMke2fsTest(unittest.TestCase):
  ...
  def test_parse_arguments_with_filecontext(self):
    args_list = ["-s", "source_directory", "output_file", "ext4", "data", "8192"]
    for key, value in self.optional_arguments.items():
      args_list += [key, value]
  ...
  def test_construct_e2fs_opts_smoke(self):
    ...
    make_ext4fs_cmd, e2fsdroid_cmd = mkuserimg_mke2fs.ConstructE2fsCommands(args)

    expected_make_ext4fs_cmd = \
      ["mke2fs", "-J", "size=10", "-L", args.label, "-N", args.inodes, "-m",
       args.reserved_percent, "-U", args.mke2fs_uuid, "-E",
       "android_sparse,stripe_width=2,stride=4,hash_seed={}".format(args.mke2fs_hash_seed),
       "-t", args.ext_variant, "-b", "4096", args.output_file, str(int(args.fs_size) / 4096)]

    expected_e2fsdroid_cmd = ["e2fsdroid", "-T", args.timestamp, "-C",
        args.fs_config, "-p", args.product_out, "-B", args.block_list_file, "-d",
        args.base_alloc_file_in, "-D", args.base_alloc_file_out, "-s", "-S",
        args.file_contexts, "-f", args.src_dir, "-a", args.mount_point,
        args.output_file ]

    self.assertEqual(' '.join(make_ext4fs_cmd),
                     ' '.join(expected_make_ext4fs_cmd))
