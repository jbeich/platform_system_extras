...
void ext4_create_fs_aux_info()
{
	...
	aux_info.sb = (struct ext4_super_block *)((char *)aux_info.sb_block
				+ (info.block_size > 1024 ? 1024 : 0));
...
void ext4_fill_in_sb(bool block_device)
{
	...
	struct ext4_super_block *sb_zero = calloc(1, info.block_size);
	...
	for (i = 0; i < aux_info.groups; i++) {
		...
		if (ext4_bg_has_super_block(i)) {
			if (i != 0) {
				...
				ext4_queue_sb(group_start_block, block_device ?
						sb_zero : aux_info.backup_sb[i]);
			}
	...
	ext4_queue_sb(info.block_size > 1024 ? 0 : 1,
		block_device ? sb_zero : aux_info.sb_block);
}
