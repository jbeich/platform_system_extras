...
void ext4_fill_in_sb(int real_uuid)
{
	...
	sb->s_r_blocks_count_lo = info.reserve_size / info.block_size;
...
int ext4_claim_free_blocks(u32 block_len, u32 flags)
{
	...
	u32 resv_blocks = aux_info.sb->s_r_blocks_count_lo;
	...
	warn("Free Blocks: %u", free_blocks);
	warn("Reserved Blocks: %u", resv_blocks);
