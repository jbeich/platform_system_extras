...
// TESTDIR is where we expect the test to run, and DESTDIR is a
// subdirectory within it where the actual collection will be
// performed (intended as a stand-in for the GMScore files directory).
// NB: would be nice if this path was not hard-coded...
#define TESTDIR "/data/nativetest/perfprofd_test"
#define DESTDIR "/data/nativetest/perfprofd_test/tmp"
...
//
// Squeeze out repeated whitespace from expected/acutal logs.
//
...
class PerfProfdRunner {
 public:
  PerfProfdRunner()
      : config_path_(TESTDIR "/" CONFIGFILE)
      , aux_config_path_(DESTDIR "/" CONFIGFILE)
  {
  }
  ...
  int invoke()
  {
    static const char *argv[3] = { "apwd", "-c", "" };
...
{
  struct stat statb;
  const char *encoded_file_path = DESTDIR "/perf.data.encoded";
  int perf_data_stat_result = stat(encoded_file_path, &statb);
  ASSERT_NE(-1, perf_data_stat_result);

  // read
  std::string encoded;
  encoded.resize(statb.st_size);
  FILE *ifp = fopen(encoded_file_path, "r");
  ...
  if (debugDump) {
    ...
    std::string dfp(DESTDIR "/"); dfp += testpoint; dfp += ".dump_encoded.txt";
...
TEST_F(PerfProfdTest, MissingGMS)
{
   ...
   runner.addToConfig("destination_directory=/dev/null/null");
   ...
   const std::string expected = RAW_RESULT(
       ...
       I: option destination_directory set to /dev/null/null
       ...
       I: sleep 90 seconds
       W: unable to open destination directory /dev/null/null: (Not a directory)
...
TEST_F(PerfProfdTest, MissingOptInSemaphoreFile)
{
   ...
   // check is performed in the CMS core component; if the check
   // passes, then it creates a semaphore file for the daemon to pick
   // up on.
   //
   PerfProfdRunner runner;
   runner.addToConfig("destination_directory=" DESTDIR);
   runner.addToConfig("main_loop_iterations=1");
   runner.addToConfig("use_fixed_seed=1");
   runner.addToConfig("collection_interval=100");

  system("rm -f " DESTDIR "/" SEMAPHORE_FILENAME);
...
TEST_F(PerfProfdTest, MissingPerfExecutable)
{
   ...
   system("touch " DESTDIR "/" SEMAPHORE_FILENAME);
...
TEST_F(PerfProfdTest, ConfigFileParsing)
{
   ...
   // Verify log contents
   const std::string expected = RAW_RESULT(
       W: line 5: specified value 0 for 'collection_interval' outside permitted range [100 4294967295] (ignored)
       W: line 6: malformed unsigned value (ignored)
       W: line 7: specified value 2 for 'collection_interval' outside permitted range [100 4294967295] (ignored)
...
{
  //
  // This test exercises the daemon using a canned perf.data file.
  // The daemon still needs to think that it is executing 'perf',
  // however, so we tell it to run /system/bin/true and then just move
  // the canned file into place (note the 'noclean' setting also).
  //
  PerfProfdRunner runner;
  runner.addToConfig("destination_directory=" DESTDIR);
  runner.addToConfig("main_loop_iterations=1");
  runner.addToConfig("use_fixed_seed=1");
  runner.addToConfig("collection_interval=100");
  runner.addToConfig("perf_path=/system/bin/true");
  runner.addToConfig("noclean=1");

  // Create semaphore file
  system("touch " DESTDIR "/" SEMAPHORE_FILENAME);

  // Move canned perf file into place
  int src = system("mv ../canned.perf.data " DESTDIR "/perf.data");
  ASSERT_EQ(0, src);
  ...
  // Expect 26 load modules
  EXPECT_EQ(26, encodedProfile.load_module_samples_size());

  // Examine a couple of them
  const auto &lm1 = encodedProfile.load_module_samples(0);
  std::string act_lm1;
  ::google::protobuf::TextFormat::PrintToString(lm1, &act_lm1);
  std::string sqact1 = squeezeWhite(act_lm1, "actual for lm2");
  const std::string expected_lm1 = RAW_RESULT(
      name: "/data/app/com.google.android.apps.plus-1/lib/arm/libcronet.so"
      address_samples { address: 310106 count: 1 }
      address_samples { address: 1949952 count: 1 }
                                              );
  std::string sqexp1 = squeezeWhite(expected_lm1, "expected_lm1");
  EXPECT_STREQ(sqexp1.c_str(), sqact1.c_str());

  // another load module
  const auto &lm2 = encodedProfile.load_module_samples(5);
  std::string act_lm2;
  ...
  const std::string expected_lm2 = RAW_RESULT(
      ...
      build_id: "7d343378a036f6425e368cfa90c3231c"
      address_samples { address: 7472 count: 2 }
      address_samples { address: 7474 count: 1 }
      address_samples { address: 7482 count: 1 }
                                              );
...
TEST_F(PerfProfdTest, BasicRunWithLivePerf)
{
  ...
  EXPECT_LT(0, encodedProfile.load_module_samples_size());
  ...
  const std::string expected = RAW_RESULT(
   ...
   // check to make sure log excerpt matches
   compareLogMessages(mock_perfprofdutils_getlogged(),
                      expected, "AuxiliaryConfigFile", true);
}
