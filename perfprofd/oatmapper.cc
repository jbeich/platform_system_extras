...
static bool read_oatinfo_from_file(const char *cksumcachepath,
                                   off_t cachefile_size,
                                   OatFileInfo &oatinfo)
{
  assert(cksumcachepath);
  assert(cachefile_size);

  W_ALOGD("read_oatinfo_from_file(%s,%ld)", cksumcachepath, cachefile_size);

  // read
  std::string encoded;
  encoded.resize(cachefile_size);
                                        ...
                                        OatFileInfo &oatinfo)
{
  ...
  W_ALOGD("OMF size is %ld", omf_size);
...
static bool generate_oatmap(const char *oatpath,
...
bool collect_oatfile_checksums(const char *oatpath,
                               ...
                               OatFileInfo &oatinfo)
{
  W_ALOGD("collect_oatfile_checksums(%s,%s,%s)",
          oatpath, oatmapcachepath, cksumcachepath);

  off_t ckf_size = get_file_size(cksumcachepath);

  W_ALOGD("cache %s on oatpath %s cksumcachepath %s",
          ckf_size ? "hit" : "miss", oatpath, cksumcachepath);
  if (ckf_size) {
    if (read_oatinfo_from_file(cksumcachepath, ckf_size, oatinfo)) {
      return true;
    }
  }
  ...
  W_ALOGD("cache %s on oatpath %s oatmapcachepath %s",
          ckf_size ? "hit" : "miss", oatpath, oatmapcachepath);
  if (omf_size) {
