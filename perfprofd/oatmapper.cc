...
void OatMapperImpl::postprocess_encoded_profile(AndroidPerfProfile &prof)
{
  ...
  //
  // Examine each of the load modules to determine if it is
  // an OAT file (as a side efect, this will update the LoadModule
  // with sha1 checksum info).
  //
  std::set<int> oat_ids;
  ...
  for (int i = 0; i < prof.programs_size(); ++i) {
    ...
    for (int j = 0; j < prog.modules_size(); ++i) {
