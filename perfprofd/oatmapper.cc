...
bool OatMapperImpl::examine_potential_oatfile(LoadModule &loadmodule)
{
  ...
  if (has_oat_suffix(loadmodulepath)) {
    ...
    OatFileInfo oatinfo;
    // FIXME: this needs to be computed by looking at the OAT
    // symbol information.
...
uint64_t OatMapperImpl::encode_addr(const char *loadmodulepath,
                                    ...
                                    uint64_t addr)
{
  assert(encoding == OAT_ADDR_METHOD);

  std::string lm(loadmodulepath);
  MapTableIterator it = mappers_.find(lm);
  uint64_t encoded = 0;
  if (it == mappers_.end()) {
    // W_ALOGD("encode_addr: no mapper for lm %s", loadmodulepath);
    return 0;
  }
  if (!it->second->lookup_ip(addr, encoded)) {
    // W_ALOGD("encode_addr: lookup IP failed for 0x%" PRIx64 "", addr);
    return 0;
  }
  return encoded;
}
