...
class Callsite(object):
    ...
    def trim_percentage(self, percentage, global_threshold):
        local_threshold = percentage * 0.01 * self.count
        ...
        for _, v in self.child_map.iteritems():
            v.trim_percentage(percentage, global_threshold)
    ...
    def print_callsite_ascii(self, depth, indent, dsos, syms):
        ...
        print '  ' * indent + "%s (%s) [%d]" % (get_str(self.sym_id, syms),
                                                get_str(self.dso_id, dsos),
                                                self.count)
        if depth == 0:
            return
        for _, v in sorted(self.child_map.iteritems(), cmp=lambda x, y: (x[1].count - y[1].count),
                           reverse=True):
...
class SqliteReader(object):
    ...
    def read(self, percent, global_percent, limit):
        ...
        global_threshold = global_percent * 0.01 * self.root.count
        self.root.trim_percentage(percent, global_threshold)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--html-out', help='output file for JSON', type=str)
