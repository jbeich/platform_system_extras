...
def collect_tid_names(perf_data):
    ...
    for event in perf_data.events:
        if event.HasField('comm_event'):
            tid_name_map[event.comm_event.tid] = intern_uni(
                ...
                if len(parts) == 2:
                    ...
                    if plus_index > 0:
                        ...
                        try:
                            ...
                            logging.warn(
                                'error parsing offset from %s', parts[0])
...
def decode_addr(addr, mmap_state, device_symbols):
    ...
    if (name, offset) in skip_dso:
        ...
        if ret is not None and len(ret) != 0:
            return ret
        logging.warn("Failed to find symbol for %s +%d (%d)",
                     name, offset, addr)
        # Remember the fail.
...
def run(args):
    ...
    if args.print_pid_histogram is not None:
        print_histogram(samples, lambda x: x[0], get_name, 25)
    if args.print_sym_histogram is not None:
...
if __name__ == "__main__":
    ...
    parser.add_argument('file', help='proto file to parse',
                        metavar='file', nargs='+')
    ...
    parser.add_argument('--print-samples', help='print samples',
