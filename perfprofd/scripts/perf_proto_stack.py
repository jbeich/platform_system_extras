...
def update_mmap_states(event, state_map):
    if event.HasField('mmap_event'):
        ...
        if fork_event.pid == fork_event.ppid:
            return
        if not fork_event.ppid in state_map:
...
def find_vaddr(vaddr_map, filename):
    ...
    try:
        res = subprocess.check_output(['readelf', '-l', path])
        lines = res.split("\n")
...
def decode_addr(addr, mmap_state, device_symbols):
    ...
    if name in device_symbols:
        ...
        if symbol is None:
            return [("%s +%d (missing on-device symbol)" % (name, offset), 0, name)]
        else:
...
def symbolize_events(perf_data, device_symbols, tid_name_map, printSamples = False, removeKernelTop = False):
    samples = []
    mmap_states = {}
    for event in perf_data.events:
        ...
        if event.HasField('sample_event'):
            ...
            if sample_ev.pid in mmap_states:
                ...
                if sample_ev.pid == 0:
                    ...
                    samples.append((sample_ev.pid, sample_ev.tid, [(tid_name_map[sample_ev.pid], 0, None)]))
                else:
...
def print_histogram(samples, reduce_key_fn, label_key_fn, size):
    ...
    sorted_count_list = sorted(
        reduce(lambda x, y: count_key_reduce_function(x, y, reduce_key_fn), samples, {}).iteritems(),
    ...
    for i in xrange(0, min(len(sorted_count_topX), size)):
        print '  %d: %s (%s)' % (i+1, label_key_fn(sorted_count_topX[i][0]), sorted_count_topX[i][1])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--skip-kernel-syms', help='skip kernel symbols at the top of stack', action='store_const', const=True)
    parser.add_argument('--print-pid-histogram', help='print a top-25 histogram of processes', action='store_const', const=True)
    parser.add_argument('--print-sym-histogram', help='print a top-100 histogram of symbols', action='store_const', const=True)
    parser.add_argument('--print-dso-histogram', help='print a top-25 histogram of maps', action='store_const', const=True)
