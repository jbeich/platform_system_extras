                             ...
                             std::string* error_msg)
{
  ...
  // -e_evtname(,evtname)*@period={value-ignored}
  // -g_evtname(,evtname)*@period={value-ignored}
  {
    ...
    if (event_key || group_key) {
      ...
      size_t at_index = key.find('@');
      if (at_index != std::string::npos) {
        uint64_t conv;
        if (!android::base::ParseUint(key.substr(at_index + 1), &conv)) {
        ...
        events.names = android::base::Split(key.substr(3, at_index - 3), ",");
