...
HardwireCpuHelper::HardwireCpuHelper(unsigned hardwire, unsigned duration)
    : ncores_(-1)
    , mpdecision_running_(GetMpdecisionRunning())
    , mpdecision_stopped_(false)
{
  if (hardwire && duration <= 5 && mpdecision_running_) {
...
int HardwireCpuHelper::GetNumCores()
{
  ...
  if (fp) {
    char *lineptr = NULL;
    size_t llen = 0;
    if (getline(&lineptr, &llen, fp) != -1) {
      unsigned lo = 0, hi = 0;
      if (sscanf(lineptr, "%u-%u", &lo, &hi) == 2) {
        ncores = hi - lo + 1;
      }
      free(lineptr);
    }
