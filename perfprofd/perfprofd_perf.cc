...
std::vector<std::string>& GetSupportedPerfCountersInternal() {
  static std::vector<std::string>& vec = *new std::vector<std::string>();
  return vec;
}
...
bool FindSupportedPerfCounters(const std::string& perf_path) {
  ...
  std::vector<std::string>& supported = GetSupportedPerfCountersInternal();
  supported.clear();
  ...
  for (const std::string& line : lines) {
    ...
    if (end > 2) {
      supported.push_back(line.substr(2, end - 2));
    }
  }
...
const std::vector<std::string>& GetSupportedPerfCounters() {
