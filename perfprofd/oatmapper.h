...
typedef enum {
  OATMAPGEN_NORMAL=0,  // generate OAT mapfiles internally
  OATMAPGEN_OATDUMP=1, // generating with oatdump (testing purposes)
} OatMapGenFlavor;
...
class OatMapper {
 public:
  OatMapper(const char *cachedir, OatMapGenFlavor mapgenflav);
  ~OatMapper();
  ...
  // an OAT file. If the address cannot be encoded for some reason,
  ...
  // Exposed for unit testing.
  //
  // Return path of the file created to cache data on a specific
  // oat file. This file may or may not exist.
  //
