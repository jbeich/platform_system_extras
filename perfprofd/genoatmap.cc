...
class GenMapOatVisitor : public OatDexVisitor {
 public:
  ...
  virtual void visitOAT(bool is64bit,
                        ...
                        uint64_t base_text) {
    DEBUGLOG(("visitOAT is64bit=%s", is64bit ? "true" : "false"));
#if 0
  ...
  void visitMethod(const std::string &methodName,
    ...
    if (nativeCodeOffset && nativeCodeSize && *nativeCodeSize > 0) {
      if (! current_dexclass_) {
        // add classes lazily so as to skip classes with no compiled methods
      ...
      DEBUGLOG(("visitMethod dmidx=%d ndi=%d nco=%llu ncs=%u",
                dexMethodIdx, numDexInstrs,
