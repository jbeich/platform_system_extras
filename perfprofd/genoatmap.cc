...
class GenMapOatVisitor : public OatVisitor {
 ...
 private:
  ...
  bool validateDexFile(unsigned char *dex_file_pointer);
...
bool GenMapOatVisitor::validateDexFile(unsigned char *dex_file_pointer)
{
  // Not yet implemented
  return true;
}
...
bool GenMapOatVisitor::collectDexFileInfo(unsigned char * &dat)
{
  ...
  if (! validateDexFile(dex_file_pointer))
    return false;
  // advance past other things we're not interested in
