...
class OatDataReader : public OatReaderHooks {
 public:
  ...
  bool getMethodNativeCodeOffset(uint64_t &nativeCodeOffset);
 ...
 private:
  ...
  // code offset for current method
...
bool OatDataReader::setupClass(uint32_t classIdx)
{
  ...
  switch(class_disp_) {
    ...
    case OatDispSomeCompiled:
      ...
      bitmap_ptr_ = reinterpret_cast<uint32_t *>(after_bptr + sizeof(uint32_t));
      methods_ptr_ = bitmap_ptr_ + bitmap_size_;
      DEBUGLOG(("setupClass: bitmap_size_ is %d", bitmap_size_));
      DEBUGLOG(("setupClass: bitmap[0] is 0x%x", bitmap_ptr_[0]));
      break;
...
bool OatDataReader::setupMethod(uint32_t methodIdx)
{
  ...
  DEBUGLOG(("setupMethod: methodIdx %d => midx %d (off %d)",
            methodIdx, midx, code_offset_));

  return true;
}

bool OatDataReader::getMethodNativeCodeOffset(uint64_t &nativeCodeOffset)
{
  return false;
}
