...
template <class ELFT>
bool ExamineDynsymInElfFile(const llvm::object::ELFObjectFile<ELFT>* elf,
                            uint64_t &base_text)
{
...
bool isOatFile(const std::string &path, uint64_t &base_text)
{
  ...
  if (auto elf = llvm::dyn_cast<llvm::object::ELF32LEObjectFile>(obj)) {
    return ExamineDynsymInElfFile(elf, base_text);
  }
