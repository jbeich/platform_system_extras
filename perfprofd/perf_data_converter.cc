...
static constexpr bool kUseSymbolizer ATTRIBUTE_UNUSED = true;
...
static constexpr bool kUseSymbolizerForModulesWithBuildId ATTRIBUTE_UNUSED = false;
...
struct RangeTarget {
  RangeTarget(uint64_t start, uint64_t end, uint64_t to)
...
struct BinaryProfile {
  map<uint64_t, uint64_t> address_count_map;
  map<RangeTarget, uint64_t> range_count_map;
  map<const callchain *, uint64_t, callchain_lt> callchain_count_map;
};
