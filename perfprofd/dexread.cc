...
// Uncomment while debugging for additional log output
// #define DEBUGGING 1
...
//
// Hidden implementation class for reading/examining a DEX file.
//
class DexReader {
 public:
 ...
 private:
  ...
  bool examineMethod(uint32_t methodIdx, uint64_t codeOffset) {
    ...
    // Collect number of dex instructions
    DexCodeItem *codeItem;
    if (!codeOffset)
      codeItem = 0; // native method or abstract method
    else if (! FormWordAlignedPointer(codeOffset, codeItem))
      return false;
    uint32_t numInstrs = (codeItem ? codeItem->insns_size : 0);

    // Collect native code info
    uint64_t nativeCodeOffset = 0;
    uint32_t nativeCodeSize = 0;
    bool haveNative = false;
    if (hooks_)
      haveNative = hooks_->getMethodNativeCodeInfo(nativeCodeOffset,
                                                   nativeCodeSize);

    visitor_.visitMethod(std::string(str), methodIdx, numInstrs,
                         haveNative ? &nativeCodeOffset : nullptr,
                         haveNative ? &nativeCodeSize : nullptr);
  ...
  bool examineClass(unsigned clIdx) {
    ...
    for (uint32_t mc = 0; mc < numMethods; ++mc) {
      ...
      // Note that we are passing method index within the class, not the
      // global DEX file method idx
      if (hooks_)
        hooks_->setupMethod(mc);
