...
static PROFILE_RESULT collect_profile(const Config& config, int seq)
{
  ...
  //
  uint32_t duration = config.sample_duration;
  bool hardwire = config.hardwire_cpus;
  uint32_t max_duration = config.hardwire_cpus_max_duration;
...
static void init(ConfigReader &config)
{
  ...
  set_seed(static_cast<uint32_t>(config.getUnsignedValue("use_fixed_seed")));
