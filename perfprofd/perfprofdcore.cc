...
//
// <Introductory material here on daemon>
//
...
//
// This enumeration holds the results of what happened when we kicked off
// a 'perf' profiling run.
//
typedef enum {
  ...
  // Could not change directory to dest dir
  ERR_CHDIR_FAILED,
...
void ConfigReader::addDefaultEntries()
{
  ...
  // Encoding threshold. If percentage of samples MMAP'd is less than
  // this value (1-99) then we skip encoding the perf.data file in
  // question
  addUnsignedEntry("encoding_threshold", 50, 0, 100);
...
void ConfigReader::readFile(const char *configFilePath)
{
  ...
  char linebuf[CFILE_LINEMAX+1];
  for (unsigned linecount = 1; fgets(&linebuf[0], CFILE_LINEMAX, fp);
       ++linecount) {
    char *eq = 0;
    char *key, *value;

    if (strlen(linebuf) >= CFILE_LINEMAX) {
      W_ALOGW("line %d: line ignored (exceeds max length of %d)", 
              linecount, CFILE_LINEMAX);
      continue;
    }
...
const char *profile_result_to_string(PROFILE_RESULT result)
{
  switch(result) {
    ...
    case ERR_CHDIR_FAILED:
      return "chdir() to destination directory failed";
...
// than the daemon).
//
static void readAuxConfig(ConfigReader &config)
{
...
static CKPROFILE_RESULT check_profiling_enabled(ConfigReader &config)
{
  ...
  if (!dir) {
    char errb[1024]; errb[0] = '\0';
    strerror_r(errno, errb, 1024);
...
static PROFILE_RESULT encode_to_proto(ConfigReader &config,
                                      ...
                                      const std::string &data_file_path)
{
  ...
  //
  double ethresh = 
      static_cast<double>(config.getUnsignedValue("encoding_threshold"));
  wireless_android_play_playlog::AndroidPerfProfile encodedProfile;
  if (!wireless_android_logging_awp::RawPerfDataToAndroidPerfProfile(data_file_path,
                                                                     encodedProfile,
                                                                     ethresh)) {
  ...
  //
  // Emit encoded profile to perf.data.encoded
  //
...
{
  std::string destdir = config.getStringValue("destination_directory");
  if (chdir(destdir.c_str())) {
    return ERR_CHDIR_FAILED;
  }

  //
  // Form perf.data file name
  //
  ...
  char cmd[8192];
  snprintf(cmd, 8192, "%s record %s -c %u -o %s -a -- sleep %d 1> perferr.txt 2>&1 ",
           perf_path.c_str(), gopt, period, PERF_OUTPUT, duration);
  int rc = system(cmd);
  if (rc == -1) {
  ...
  //
  return encode_to_proto(config, destdir, data_file_path);
}
...
static void set_seed(ConfigReader &config)
{
  ...
  if (use_fixed_seed) {
    ...
    //
    // Read some random bytes, combine with current time
    //
