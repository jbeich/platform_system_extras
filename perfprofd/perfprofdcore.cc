...
//
// Post-process after profile is collected and converted to protobuf.
...
//
static void post_process(const ConfigReader &config, int current_seq)
{
  ...
  if (fp == NULL) {
    W_ALOGW("Cannot read %s", processed_file_path.c_str());
    return;
  }
  int seq;
  ...
  if (fp == NULL) {
    W_ALOGW("Cannot read %s", produced_file_path.c_str());
    return;
  }
