...
static PROFILE_RESULT collect_profile(ConfigReader &config)
{
  ...
  //
  struct stat statb;
  if (!stat(data_file_path.c_str(), &statb)) {
...
static void set_seed(ConfigReader &config)
{
  ...
  // distributed the 32-bit seed into the three 16-bit array elements. The code here doesn't
  // especially matter as long as we are setting them to some value based on the seed.
