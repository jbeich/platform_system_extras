...
bool get_camera_active()
{
  ...
  while (getline(&linebuf, &line_length, fp) != -1) {
    if (strstr(linebuf, "Camera module API version:")) {
      have_cam = true;
    }
    if (strstr(linebuf, "No camera module available") ||
        strstr(linebuf, "No active camera clients yet")) {
      have_clients = false;
    }
  }
  fclose(fp);
