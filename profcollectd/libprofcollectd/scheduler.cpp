...
static constexpr const config_t CONFIG_BUILD_FINGERPRINT = {"build_fingerprint", "unknown"};
static constexpr const config_t CONFIG_COLLECTION_INTERVAL_SEC = {"collection_interval", "600"};
static constexpr const config_t CONFIG_SAMPLING_PERIOD_SEC = {"sampling_period", "0.5"};
static constexpr const config_t CONFIG_BINARY_FILTER = {"binary_filter", ""};
...
OptError ProfcollectdScheduler::ReadConfig() {
  ...
  config.collectionInterval = std::chrono::seconds(getConfigFlagInt(CONFIG_COLLECTION_INTERVAL_SEC));
  config.samplingPeriod = std::chrono::milliseconds(getConfigFlagFloat(CONFIG_SAMPLING_PERIOD_SEC) / 1000);
...
std::ostream& operator<<(std::ostream& os, const ProfcollectdScheduler::Config& config) {
  ...
  root[CONFIG_SAMPLING_PERIOD_MS.name] = config.samplingPeriod.count();
...
std::istream& operator>>(std::istream& is, ProfcollectdScheduler::Config& config) {
  ...
  config.samplingPeriod =
      std::chrono::duration<float>(root[CONFIG_SAMPLING_PERIOD_MS.name].asFloat());
