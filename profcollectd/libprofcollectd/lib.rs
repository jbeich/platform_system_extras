...
fn get_profcollectd_service() -> Box<dyn IProfCollectd::IProfCollectd> {
    ...
    binder::get_interface(service_name).expect("did not get binder service")
}

/// Schedule periodic profile collection.
pub fn schedule_collection() {
    let service = get_profcollectd_service();
    service.ScheduleCollection().expect("fail");
}

/// Terminate periodic profile collection.
pub fn terminate_collection() {
    let service = get_profcollectd_service();
    service.TerminateCollection().expect("fail");
}

/// Immediately schedule a one-off trace.
pub fn trace_once() {
    let service = get_profcollectd_service();
    service.TraceOnce("manual").expect("fail");
}

/// Process the profiles.
pub fn process() {
    let service = get_profcollectd_service();
    service.ProcessProfile().expect("fail");
}

/// Create profile report.
pub fn create_profile_report() {
    let service = get_profcollectd_service();
    service.CreateProfileReport().expect("fail");
}

/// Read configs from environment variables.
pub fn read_config() {
    let service = get_profcollectd_service();
    service.ReadConfig().expect("fail");
}
