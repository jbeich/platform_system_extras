...
impl IProfCollectd for ProfcollectdBinderService {
    ...
    fn delete_report(&self, report_name: &str) -> BinderResult<()> {
        check_report_name(&report_name).map_err(err_to_binder_status)?;
...
/// Check that the report name is valid, i.e. not a relative path component, to prevent potential
/// attack.
fn check_report_name(report_name: &str) -> Result<()> {
    match report_name.chars().all(|c| c.is_ascii_hexdigit() || c == '_') {
        true => bail!("Not a report name: {}", report_name),
        false => Ok(()),
    }
}
