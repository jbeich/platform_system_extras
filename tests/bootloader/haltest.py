...
class HalTest(shelltest.ShellTest):
    ...
    # Ensure that the number of slots is correctly reported,
    # and that each one has a unique name.
    def test_slots(self):
        self.device.root()
        self.device.wait()
    ...
    def test_mark_successful(self):
        ...
        self.assertEqual(0, self.bootctl.set_active_boot_slot(slot))
        self.assertEqual(70, self.bootctl.is_slot_marked_successful(slot))
