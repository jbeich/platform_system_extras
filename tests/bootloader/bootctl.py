...
class Bootctl(object):
    ...
    def mark_boot_successful(self):
        return self._exec("mark-boot-successful")[0]

    def set_active_boot_slot(self, slot):
        return self._exec("set-active-boot-slot " + str(slot))[0]

    def set_slot_as_unbootable_slot(self, slot):
        return self._exec("set-slot-as-unbootable " + str(slot))[0]

    def is_slot_bootable(self, slot):
        return self._exec("is-slot-bootable " + str(slot))[0]

    def is_slot_marked_successful(self, slot):
        return self._exec("is-slot-marked-successful " + str(slot))[0]
