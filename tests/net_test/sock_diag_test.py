...
class SockDiagTest(SockDiagBaseTest):
  ...
  def testBytecodeCompilation(self):
    ...
    expected = (
        "\x02\x08\x50\x00\x00\x00\x00\x00"
        "\x05\x08\x48\x00\x00\x00\xff\xff"
        "\x07\x1c\x20\x00\x0a\x80\x00\x00\xff\xff\xff\xff"
            "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x01"
        "\x01\x04\x1c\x00"
        "\x07\x18\x20\x00\x02\x20\x00\x00\xff\xff\xff\xff"
            "\x7f\x00\x00\x01"
        "\x05\x08\x10\x00\x00\x00\x65\x66"
        "\x00\x04\x04\x00"
    )
    self.assertMultiLineEqual(expected.encode("hex"), bytecode.encode("hex"))
    ...
    s = random.choice(random.choice(self.socketpairs.values()))
