...
class TcpNukeAddrHashTest(net_test.NetworkTest):
  ...
  def testClosesAllSockets(self):
    ...
    for i in xrange(HASH_TEST_RUNS):
      sockets.append(CreateSocketPair(AF_INET, IPV4_LOOPBACK_ADDR))
      sockets.append(CreateSocketPair(AF_INET6, IPV6_LOOPBACK_ADDR))

    KillAddrIoctl(AF_INET)
    KillAddrIoctl(AF_INET6)

    for acceptedsock, clientsock in sockets:
      self.assertRaisesErrno(errno.ENOTCONN, acceptedsock.getpeername)
      self.assertRaisesErrno(errno.ENOTCONN, clientsock.getpeername)
