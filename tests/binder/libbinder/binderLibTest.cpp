...
TEST_F(BinderLibTest, IndirectGetId2)
{
    ...
    Parcel data, reply;
    sp<IBinder> server[3];
    int32_t serverId[ARRAY_SIZE(server)];

    data.writeInt32(ARRAY_SIZE(server));
    for (size_t i = 0; i < ARRAY_SIZE(server); i++) {
        server[i] = addServer(&serverId[i]);
        BinderLibTestBundle datai;
        data.writeStrongBinder(server[i]);
    ...
    ASSERT_EQ(NO_ERROR, ret);
    EXPECT_EQ(ARRAY_SIZE(server), count);
...
TEST_F(BinderLibTest, IndirectGetId3)
{
    ...
    for (size_t i = 0; i < ARRAY_SIZE(serverId); i++) {
        ...
        data.writeStrongBinder(addServer(&serverId[i]));
