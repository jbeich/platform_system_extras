...
static String16 binderLibTestServiceName[] = {
    String16("test.binderLib1"),
//    String16("test.binderLib2"),
//    String16("test.binderLib3"),
};
...
enum BinderLibTestTranscationCode {
    ...
    //BINDER_LIB_TEST_INDIRECT_GET_ID_TRANSACTION,
...
class BinderLibTestEnv : public ::testing::Environment {
    public:
        BinderLibTestEnv(const pid_t *pids) : m_pids(pids) {}
        sp<IBinder> *getServers(void) {
            return m_server;
        }
    ...
    private:
        virtual void SetUp() {
            //printf("%s: pid %d\n", __func__, m_pid);
            //m_pid = fork();
            for (size_t i = 0; i < ARRAY_SIZE(binderLibTestServiceName); i++) {
                ASSERT_NE(-1, m_pids[i]);
            }

            sp<IServiceManager> sm = defaultServiceManager();
            //printf("%s: pid %d, get service\n", __func__, m_pid);
            for (size_t i = 0; i < ARRAY_SIZE(binderLibTestServiceName); i++) {
                m_server[i] = sm->getService(binderLibTestServiceName[i]);
            }
            ASSERT_TRUE(m_server != 0);
        ...
        virtual void TearDown() {
            for (size_t i = 0; i < ARRAY_SIZE(binderLibTestServiceName); i++) {
                ...
                //printf("%s: pid %d\n", __func__, m_pid);
                ret = m_server[i]->transact(BINDER_LIB_TEST_GET_STATUS_TRANSACTION, data, &reply);
                EXPECT_EQ(0, ret);
                ret = m_server[i]->transact(BINDER_LIB_TEST_EXIT_TRANSACTION, data, &reply, TF_ONE_WAY);
                EXPECT_EQ(0, ret);
                pid = wait(&exitStatus);
                EXPECT_EQ(m_pids[i], pid);
        ...
        const pid_t *m_pids;
        sp<IBinder> m_server[ARRAY_SIZE(binderLibTestServiceName)];
};

class BinderLibTest : public ::testing::Test {
    public:
        virtual void SetUp() {
            m_server = static_cast<BinderLibTestEnv *>(binder_env)->getServers();
        }
        virtual void TearDown() {
        }
    protected:
        sp<IBinder> addServer(void)
        {
            int ret;
            Parcel data, reply;
            sp<IBinder> binder;

            ret = m_server[0]->transact(BINDER_LIB_TEST_ADD_SERVER, data, &reply);
...
TEST_F(BinderLibTest, Nop) {
    status_t ret;
    Parcel data, reply;
    ret = m_server[0]->transact(BINDER_LIB_TEST_NOP_TRANSACTION, data, &reply);
    EXPECT_EQ(NO_ERROR, ret);
}
...
TEST_F(BinderLibTest, GetId) {
    for (size_t i = 0; i < ARRAY_SIZE(binderLibTestServiceName); i++) {
        ...
        int32_t id;
        Parcel data, reply;
        ret = m_server[i]->transact(BINDER_LIB_TEST_GET_ID_TRANSACTION, data, &reply);
        EXPECT_EQ(NO_ERROR, ret);
...
#if 0
TEST_F(BinderLibTest, IndirectGetId) {
    for (size_t i = 0; i < ARRAY_SIZE(binderLibTestServiceName); i++) {
        status_t ret;
        int32_t id;
        Parcel data, reply;
        data.writeStrongBinder(m_server[1]);
        data.writeStrongBinder(m_server[i]);
        ret = m_server[0]->transact(BINDER_LIB_TEST_INDIRECT_GET_ID_TRANSACTION, data, &reply);
        EXPECT_EQ(NO_ERROR, ret);
        ret = reply.readInt32(&id);
        EXPECT_EQ(NO_ERROR, ret);
        EXPECT_EQ(0, id);
...
TEST_F(BinderLibTest, IndirectGetId2)
{
    ...
    int32_t count;
    Parcel data, reply;
    data.writeInt32(ARRAY_SIZE(binderLibTestServiceName));
    for (size_t i = 0; i < ARRAY_SIZE(binderLibTestServiceName); i++) {
    ...
    ret = m_server[0]->transact(BINDER_LIB_TEST_INDIRECT_TRANSACTION, data, &reply);
    ...
    ASSERT_EQ(NO_ERROR, ret);
    EXPECT_EQ(ARRAY_SIZE(binderLibTestServiceName), count);
...
TEST_F(BinderLibTest, IndirectGetId3)
{
    ...
    for (size_t i = 0; i < ARRAY_SIZE(binderLibTestServiceName); i++) {
        ...
        datai.writeInt32(1);
        datai.writeStrongBinder(m_server[0]);
...
TEST_F(BinderLibTest, CallBack)
{
    ...
    ret = m_server[0]->transact(BINDER_LIB_TEST_NOP_CALL_BACK, data, &reply, TF_ONE_WAY);
    EXPECT_EQ(NO_ERROR, ret);
...
TEST_F(BinderLibTest, PassFile) {
...

    {
        ...
        ret = m_server[0]->transact(BINDER_LIB_TEST_WRITE_FILE_TRANSACTION, data, &reply);
        EXPECT_EQ(NO_ERROR, ret);
    }
...
{
    public:
        BinderLibTestService(int32_t id, int pipefd)
            : m_id(id)
            , m_pipefd(pipefd)
        ...
        virtual status_t onTransact(uint32_t code,
            ...
            case BINDER_LIB_TEST_ADD_SERVER: {
                ...
                uint8_t buf[1] = { 0 };
                pthread_mutex_lock(&m_serverWaitMutex);
                ret = write(m_pipefd, &buf, sizeof(buf));
                if (ret == 1) {
            ...
            case BINDER_LIB_TEST_GET_ID_TRANSACTION:
                ...
                return NO_ERROR;
#if 0
            case BINDER_LIB_TEST_INDIRECT_GET_ID_TRANSACTION: {
                Parcel data2, reply2, reply3;
                reply->writeInt32(m_id);
                data.readStrongBinder()->transact(BINDER_LIB_TEST_GET_ID_TRANSACTION, data2, &reply2);
                reply->writeInt32(reply2.readInt32());
                data.readStrongBinder()->transact(BINDER_LIB_TEST_GET_ID_TRANSACTION, data2, &reply3);
                reply->writeInt32(reply3.readInt32());
                return NO_ERROR;
            }
#endif
            ...
            case BINDER_LIB_TEST_PROMOTE_WEAK_REF_TRANSACTION: {
                ...
                Parcel data2, reply2;
                sp<IServiceManager> sm = defaultServiceManager();
                sp<IBinder> server = sm->getService(binderLibTestServiceName[0]);
    ...
    do {
        ...
        if (ret == 1) {
            ...
            if (new_server == 0) {
                {
                    sp<IServiceManager> sm = defaultServiceManager();
                    sp<BinderLibTestService> testService = new BinderLibTestService(index, -1);
                    sp<IBinder> server = sm->getService(binderLibTestServiceName[0]);
                    Parcel data, reply;
...
int run_server(int index, int readypipefd)
{
    ...
    if (service_pid) {
        ...
        close(readypipefd);
        close(pipefd[1]);
        ret = fork_server(pipefd[0]);
#if 0
        pid = wait(&exitStatus);
        if (pid != service_pid) {
            fprintf(stderr, "wait did not return expected pid: %d != %d, %s\n",
                    pid, service_pid, strerror(errno));
            ret = 1;
        }
#endif
        return ret;
        ...
        close(pipefd[0]);
        sp<IServiceManager> sm = defaultServiceManager();
        sp<BinderLibTestService> testService = new BinderLibTestService(index, pipefd[1]);
        ret = sm->addService(binderLibTestServiceName[index], testService);
        close(readypipefd);
        //printf("%s: ret %d\n", __func__, ret);
        if (ret)
            return 1;
...
int main(int argc, char **argv) {
    ...
    int pipefd[2];
    uint8_t dummy[1];
    pid_t pids[ARRAY_SIZE(binderLibTestServiceName)];
    ...
    for (size_t i = 0; i < ARRAY_SIZE(binderLibTestServiceName); i++) {
        ...
        if (pids[i] == 0) {
            close(pipefd[0]);
            return run_server(i, pipefd[1]);
        }
    }
    close(pipefd[1]);
    read(pipefd[0], &dummy, sizeof(dummy)); /* block until service had registered to avoid 1 second startuip delay */
    close(pipefd[0]);
    ::testing::InitGoogleTest(&argc, argv);
    binder_env = AddGlobalTestEnvironment(new BinderLibTestEnv(pids));
    ProcessState::self()->startThreadPool();
