...
impl AtraceSubscriber {
    ...
    /// Enables event and span filtering. Use this to speed up the subscriber if it's the only layer.
    ...
    fn tag_enabled_inner(&self) -> bool {
...
impl<S: Subscriber + for<'lookup> LookupSpan<'lookup>> Layer<S> for AtraceSubscriber {
    fn register_callsite(
        ...
        if self.should_filter {
            // We can't use callsite caching because there's no callback for when tracing gets
            // enabled - we need to check it every time.
    ...
    fn on_new_span(&self, attrs: &Attributes, id: &Id, ctx: Context<S>) {
        if !self.should_record_fields || attrs.fields().is_empty() || !self.tag_enabled_inner() {
            return;
        }
    ...
    fn on_record(&self, span: &Id, values: &Record, ctx: Context<S>) {
        if !self.should_record_fields || !self.tag_enabled_inner() {
            return;
        }
