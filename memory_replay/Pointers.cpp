...
Pointers::Pointers(size_t max_allocs) {
  ...
  if (memory == MAP_FAILED) {
    ERROR("Unable to allocate data for pointer hash: %zu total_allocs\n", max_allocs);
  }
...
void Pointers::Add(uintptr_t key_pointer, void* pointer) {
  ...
  if (data == nullptr) {
    ERROR("No empty entry found for 0x%" PRIxPTR "\n", key_pointer);
  }
...
void* Pointers::Remove(uintptr_t key_pointer) {
  if (key_pointer == 0) {
    ERROR("Illegal zero value passed to Remove\n");
  }

  pointer_data* data = Find(key_pointer);
  if (data == nullptr) {
    ERROR("No pointer value found for 0x%" PRIxPTR "\n", key_pointer);
  }
