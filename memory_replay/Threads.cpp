...
Threads::Threads(Pointers* pointers, size_t max_threads)
  ...
  if (memory == MAP_FAILED) {
    ERROR("Failed to map in memory for Threads: map size %zu, max threads %zu\n",
          data_size_, max_threads_);
  }

  if (Thread::ACTION_SIZE < Action::MaxActionSize()) {
    ERROR("Thread action size is too small: ACTION_SIZE %zu, max size %zu\n",
...
Thread* Threads::CreateThread(pid_t tid) {
  if (num_threads_ == max_threads_) {
    ERROR("Too many threads created, current max %zu.\n", num_threads_);
  }
  Thread* thread = FindEmptyEntry(tid);
  if (thread == nullptr) {
    ERROR("No empty entries found, current max %zu, num threads %zu\n",
