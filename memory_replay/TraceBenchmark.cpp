...
static std::string GetZipContents(const char* filename) {
  ...
  std::string contents;
  void* cookie;
  if (StartIteration(archive, &cookie, nullptr, nullptr) == 0) {
    ZipEntry entry;
    ZipString name;
...
static size_t GetIndex(std::stack<size_t>& indicies, size_t* max_index) {
  if (indicies.empty()) {
    return (*max_index)++;
  }
  size_t index = indicies.top();
  indicies.pop();
  return index;
}
...
static std::vector<MallocEntry>* GetTraceData(const char* filename, size_t* max_ptrs) {
  ...
  *max_ptrs = 0;
  std::stack<size_t> free_indicies;
  ...
  for (const std::string& line : lines) {
    ...
    if (sscanf(line.c_str(), "%d: %s %" SCNxPTR " %n", &tid, name, &key_pointer, &line_pos) != 3) {
    ...
    if (type == "malloc") {
      ...
      size_t idx = GetIndex(free_indicies, max_ptrs);
      indices[key_pointer] = idx;
      entries->emplace_back(MALLOC, idx, size, 0);
    } else if (type == "free") {
      if (key_pointer != 0) {
        auto entry = indices.find(key_pointer);
        if (entry == indices.end()) {
          errx(1, "Internal Error: Unable to find free pointer %" PRIuPTR, key_pointer);
        }
        free_indicies.push(entry->second);
      ...
      if (old_pointer != 0) {
        ...
        free_indicies.push(old_pointer_idx);
      }
      size_t idx = GetIndex(free_indicies, max_ptrs);
      indices[key_pointer] = idx;
