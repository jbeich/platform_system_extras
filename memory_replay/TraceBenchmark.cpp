...
static void GetTraceData(const std::string& filename, TraceDataType* trace_data) {
  ...
  // Convert the ptr field into a pointer index which is where the pointer is
  // located in the ptrs array. The free ptr will be the index + 1 so that
  // a zero indicates a free(nullptr) call.
