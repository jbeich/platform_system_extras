...
struct TraceAllocEntry {
  TraceAllocEntry(AllocEnum type, size_t idx, size_t size, size_t anon)
...
static size_t GetIndex(std::stack<uint64_t>& indices, size_t* max_index) {
  if (indices.empty()) {
    return (*max_index)++;
  }
  uint64_t index = indices.top();
  indices.pop();
  return index;
}
...
static std::vector<TraceAllocEntry>* GetTraceData(const char* filename, size_t* max_ptrs) {
  ...
  *max_ptrs = 0;
  std::stack<uint64_t> free_indices;
    ...
    switch (entry.type) {
      ...
      case REALLOC: {
        ...
        if (entry.u.old_ptr != 0) {
          auto idx_entry = indices.find(entry.u.old_ptr);
          if (idx_entry == indices.end()) {
      ...
      case FREE:
        if (entry.ptr != 0) {
          auto idx_entry = indices.find(entry.ptr);
          if (idx_entry == indices.end()) {
