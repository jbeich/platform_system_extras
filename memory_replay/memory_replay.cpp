...
int main(int argc, char** argv) {
  ...
  while (fscanf(dump_file, "%d: %s %p", &tid, type, &ptr) > 0) {
    ...
    if ((line_number % 100000) == 0) {
      ...
      pss_bytes = GetPssBytes();
      va_bytes = GetNativeVASpaceBytes();
      printf("  PSS bytes: %zu %0.2fMB\n", pss_bytes, pss_bytes/(1024*1024.0));
      printf("  Native VA Space bytes: %zu %0.2fMB\n", va_bytes, va_bytes/(1024*1024.0));
      fflush(stdout);
    }
  ...
  pss_bytes = GetPssBytes();
  printf("Final PSS bytes: %zu %0.2fMB\n", pss_bytes, pss_bytes/(1024*1024.0));
  va_bytes = GetNativeVASpaceBytes();
  printf("Final Native VA Space bytes: %zu %0.2fMB\n", va_bytes, va_bytes/(1024*1024.0));
  fflush(stdout);
}
