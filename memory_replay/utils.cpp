...
size_t GetNativeVASpaceBytes() {
  ...
  while (fgets(line, sizeof(line), maps)) {
    uintptr_t start, end;
    char name[4096];
    if (sscanf(line, "%" SCNxPTR "-%" SCNxPTR " %*4s %*x %*x:%*x %*d %s", &start, &end, name) == 3) {
      if (strcmp(name, "[anon:libc_malloc]") == 0 || strcmp(name, "[heap]") == 0) {
