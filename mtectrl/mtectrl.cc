...
bool CheckAndUnset(uint32_t& mode, uint32_t bit) {
  bool set = mode & bit;
  mode &= ~bit;
  return set;
}
...
bool UpdateProp(const char* prop_name, const misc_memtag_message& m) {
  ...
  if (mode) LOG(ERROR) << "unknown bits set in memtag_mode: " << mode;
  return mode == 0;
}
...
int main(int argc, char** argv) {
  ...
  if (!value && set_prop) {
    ...
    // This is an unexpected case, as a valid header should not have invalid
    // memtag_mode.
