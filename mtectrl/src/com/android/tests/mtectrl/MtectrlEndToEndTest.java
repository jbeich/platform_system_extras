...
public class MtectrlEndToEndTest extends BaseHostJUnit4Test {
    ...
    public static void setUp(TestInformation testInfo) throws Exception {
        assumeThat(testInfo.getDevice().getProperty("ro.arm64.memtag.bootctl_supported"), equalTo("1"));
    ...
    public String getBTimeLine() throws Exception {
        String[] lines = getDevice().pullFileContents("/proc/stat").split("\n");
    ...
    public void verifyReboot() throws Exception {
        String beforeBTime = getBTimeLine();
        String afterBTime = null;
        // TODO(b/257341333): Tradefed sometimes says "Too early to call reboot()" and moves on.
        do {
            getDevice().reboot();
            afterBTime = getBTimeLine();
        } while (afterBTime.equals(beforeBTime));
        assertThat(beforeBTime).isNotNull();
        assertThat(afterBTime).isNotNull();
        assertThat(afterBTime).isNotEqualTo(beforeBTime);
    }
