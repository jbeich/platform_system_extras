...
TEST(counter, add_event) {
  std::unique_ptr<PerfEventSet> perf(PerfEventSet::CreateInstance(true));
  ASSERT_TRUE(perf);
  ...
  ASSERT_TRUE(perf->StartCounting());
  DoSomeWork();
  ASSERT_TRUE(perf->StopCounting());
...
TEST(counter, different_targets) {
  auto test_function = [](std::function<void(PerfEventSet*)> set_target_func) {
    ...
    ASSERT_TRUE(perf->FinishCounting());
  };
...
TEST(counter, start_stop_multiple_times) {
  ...
  PerfEventSet* perf = PerfEventSet::CreateInstance(true);
