...
static void ThreadA(ThreadArg* thread_arg) {
  ...
  for (size_t i = 0; i < TRY_MMAP_COUNT; ++i) {
    ...
    array[i].mmap_start_addr = mmap(NULL, 4096, PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
  ...
  for (size_t i = 0; i < TRY_MMAP_COUNT; ++i) {
    uint64_t d = array[i].end_system_time_in_ns - array[i].start_system_time_in_ns;
  ...
  thread_arg->mmap_start_addr = reinterpret_cast<uint64_t>(array[best_index].mmap_start_addr);
