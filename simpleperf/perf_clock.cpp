...
static void ThreadA(ThreadArg* thread_arg) {
  ...
  if (mmap_start_addr == MAP_FAILED) {
    PLOG(ERROR) << "mmap() failed";
    thread_arg->has_error = true;
    return;
  }

  if (!GetSystemClock(&thread_arg->system_time_in_ns)) {
    thread_arg->has_error = true;
    return;
  }
  thread_arg->mmap_start_addr = reinterpret_cast<uint64_t>(mmap_start_addr);
  munmap(mmap_start_addr, 4096);
}
