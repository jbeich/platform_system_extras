...
struct MockEventFd : public EventFd {
  MockEventFd(const perf_event_attr& attr, int cpu, char* buffer, size_t buffer_size, bool mock_aux_buffer)
  ...
  MOCK_METHOD5(GetAvailableAuxData, void(char**, size_t*, char**, size_t*, uint64_t*));
...
TEST_F(RecordReadThreadTest, read_aux_data) {
  ...
  if (type == nullptr) {
    GTEST_LOG_(INFO) << "Omit this test as etm type isn't available";
    return;
  }
  ...
  EXPECT_CALL(
      fd, GetAvailableAuxData(Truly(SetBuf1), Truly(SetSize1), Truly(SetBuf2), Truly(SetSize2), _))
      .Times(aux_data.size());
  ...
  for (; test_index < aux_data.size(); ++test_index) {
    ...
    ASSERT_EQ(auxtrace->data->aux_size, expected.buf1.size() + expected.buf2.size() + expected.pad.size());
