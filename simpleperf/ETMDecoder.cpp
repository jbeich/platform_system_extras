  ...
  void ProcessPacket(uint8_t trace_id, const EtmV4ITrcPacket* packet) {
    if (packet->getContext().updated_c) {
      ...
      if (trace_id == trace_id_) {
        // Invalid the cached buffer when the last trace stream changes thread.
        buffer_end_ = 0;
      }
    }
  }
...
class DataDumper : public ElementCallback {
 public:
  ...
  void DumpPackets(const std::unordered_map<uint8_t, EtmV4Config>& configs) {
    for (auto& p : configs) {
      uint8_t trace_id = p.first;
      packet_printers_.emplace(trace_id, trace_id);
      auto& packet_printer = packet_printers_.at(trace_id);
...
//
// ETMDecoderImpl uses OpenCSD library to decode etm data. It has below properties:
// 1. Supports flexible decoding strategy. It allows installing packet callbacks and element
// callbacks, and decodes to either packets or elements based on requirement.
 ...
 public:
  ...
  void CreateDecodeTree(const AuxTraceInfoRecord& auxtrace_info) {
    for (int i = 0; i < auxtrace_info.data->nr_cpu; i++) {
      ...
      packet_sinks_.emplace(trace_id, trace_id);
      decode_tree_.AttachPacketSink(trace_id, packet_sinks_.at(trace_id));
    }
  }
