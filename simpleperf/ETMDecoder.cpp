 ...
 public:
  ...
  ocsd_datapath_resp_t ProcessElement(const ocsd_trc_index_t, uint8_t trace_id,
    ...
    if (elem.getType() == OCSD_GEN_TRC_ELEM_INSTR_RANGE) {
      ...
      // According to ETM Specification, the Trace On element indicates a discontinuity in the
      // instruction trace stream. So it cuts the connection between prev InstrRange and next
      // InstrRange.
