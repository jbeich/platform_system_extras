...
bool RecordFileWriter::MoveToFileEnd(uint64_t* file_end) {
...
bool RecordFileWriter::WriteBuildIdFeature(const std::vector<BuildIdRecord>& build_id_records) {
  CHECK(current_feature_index_ < feature_count_);
  // Always write features at the end of the file.
  uint64_t start_offset;
  if (!MoveToFileEnd(&start_offset)) {
    return false;
  }
