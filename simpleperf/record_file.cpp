...
std::unique_ptr<RecordFileWriter> RecordFileWriter::CreateInstance(
  ...
  int fd = open(filename.c_str(), O_CREAT | O_EXCL | O_RDWR | O_CLOEXEC, S_IRUSR | S_IWUSR);
  if (fd == -1) {
    PLOG(ERROR) << "failed to open record file '" << filename << "'";
    return nullptr;
  }
  FILE* fp = fdopen(fd, "web+");
  if (fp == nullptr) {
    PLOG(ERROR) << "failed to call fdopen() on record file '" << filename << "'";
    return nullptr;
  }
...
bool RecordFileWriter::WriteAttrSection(const EventAttr& event_attr,
                                        const std::vector<std::unique_ptr<EventFd>>& event_fds) {
...
bool RecordFileWriter::GetHitModules(std::vector<std::string>& hit_kernel_modules,
  ...
  std::vector<std::unique_ptr<const Record>> kernel_mmap;
  std::vector<std::unique_ptr<const Record>> user_mmap;
  std::set<std::string> hit_kernel_set;
  std::set<std::string> hit_user_set;

  const char* p = reinterpret_cast<const char*>(mmap_addr) + data_section_offset_;
  const char* end = p + data_section_size_;
  while (p < end) {
    ...
    if (record->header.type == PERF_RECORD_MMAP) {
      ...
      const SampleRecord* sample = static_cast<const SampleRecord*>(record.get());

      if (record->header.misc & PERF_RECORD_MISC_KERNEL) {
        // Loop from back to front, because new MmapRecords are inserted at the end of kernel_mmap,
        // and we want to match the newest one.
        for (auto it = kernel_mmap.rbegin(); it != kernel_mmap.rend(); ++it) {
          const MmapRecord* mmap = static_cast<const MmapRecord*>(it->get());
...
bool RecordFileWriter::WriteFeatureHeader(size_t max_feature_count) {
  max_feature_count_ = max_feature_count;
  current_feature_index_ = 0;
  uint64_t feature_header_size = max_feature_count * sizeof(SectionDesc);
  ...
  if (!Write(zero_data.data(), feature_header_size)) {
    return false;
  }
  return true;
}
...
bool RecordFileWriter::WriteBuildIdFeature(const std::vector<BuildIdRecord>& build_id_records) {
  if (current_feature_index_ >= max_feature_count_) {
    return false;
  }
  // Always write features at the end of the file.
  if (fseek(record_fp_, 0, SEEK_END) != 0) {
    return false;
  }
  ...
  // Write feature SectionDesc for build_id feature.
  SectionDesc desc;
...
std::vector<std::unique_ptr<const Record>> RecordFileReader::DataSection() {
  ...
  EventAttr attr(file_attrs[0]->attr);
