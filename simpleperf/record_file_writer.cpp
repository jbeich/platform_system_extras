...
bool RecordFileWriter::WriteFileFeatures(const std::vector<Dso*>& files) {
  for (Dso* dso : files) {
    ...
    file_feature.path = dso->Path();
    file_feature.type = dso->type();
    dso->GetMinExecutableVaddr(&file_feature.min_vaddr, &file_feature.file_offset_of_min_vaddr);
    ...
    for (const auto& sym : symbols) {
      if (sym.HasDumpId()) {
        file_feature.symbol_ptrs.emplace_back(&sym);
      }
    }
    std::sort(file_feature.symbol_ptrs.begin(), file_feature.symbol_ptrs.end(), Symbol::CompareByAddr);

    if (const auto dex_file_offsets = dso->DexFileOffsets(); dex_file_offsets != nullptr) {
      file_feature.dex_file_offsets = *dex_file_offsets;
    }
    if (!WriteFileFeature(file_feature)) {
      return false;
    }
  }
  return true;
...
bool RecordFileWriter::WriteFileFeature(const FileFeature& file) {
  // Symbols written to the file feature section are only accepted in symbol_ptrs field.
