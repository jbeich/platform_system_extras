...
class DeviceRunner(Runner):
  ...
  def __init__(self, perf_path):
    ...
    # TODO: choose lib64 or lib based on requirement.
    self._download(os.environ['OUT'] + '/system/lib64/libsimpleperf_inplace_sampler.so',
                   self.tmpdir)
  ...
  def _call(self, args, output_file=None):
    ...
    args_with_adb.append('export LD_LIBRARY_PATH=/data/local/tmp && ' + ' '.join(args))
