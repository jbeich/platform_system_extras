...
"""Simpleperf runtest runner: run simpleperf runtests on host or on device.
  ...
  1. Run simpleperf record command to record simpleperf_runtest_one_function's running
     samples, which is generated in perf.data.
  2. Run simpleperf report command to parse perf.data, and generate perf.report.
...
class Symbol(object):
    ...
    def __str__(self):
        return 'Symbol name=%s comm=%s overhead=%f' % (self.name, self.comm, self.overhead)


class SymbolOverheadRequirement(object):

    def __init__(self, symbol_name, comm=None, min_overhead=None, max_overhead=None):
            ...
            if test_item.tag == 'symbol_overhead':
                for symbol_item in test_item:
                    ...
                    symbol_overhead_requirements.append(
                        SymbolOverheadRequirement(symbol_name, comm, overhead_min, overhead_max))
...
class ReportAnalyzer(object):
    ...
    def __read_report_file(self, report_file):
        ...
        for line in report_fh:
            ...
            if not overhead_start:
                if re.search('^Overhead\s+Command.+Symbol$', line):
                    overhead_start = True
            else:
    ...
    def check_symbol_overhead_requirements(self, test, report_file):
        ...
        result = True
        matched = [
            False for x in range(len(test.symbol_overhead_requirements))]
        for symbol in symbols:
            for i in range(len(test.symbol_overhead_requirements)):
                symbol_overhead_requirement = test.symbol_overhead_requirements[
                    i]
                if symbol_overhead_requirement.is_match(symbol):
                    matched[i] = True
                    fulfilled = symbol_overhead_requirement.check_matched_symbol(
                        symbol)
                    if not fulfilled:
                        print "Symbol (%s) doesn't match requirement (%s) in test %s" % (symbol, symbol_overhead_requirement, test)
                        result = False
        for i in range(len(matched)):
            if not matched[i]:
                print "requirement (%s) has no matched symbol in test %s" % (test.symbol_overhead_requirements[i], test)
                result = False
        return result
...
def main():
    ...
    for test in tests:
        ...
        result = report_analyzer.check_symbol_overhead_requirements(
            test, 'perf.report')
        print 'test %s on host %s' % (test.test_name, 'Succeeded' if result else 'Failed')
        if not result:
            exit(1)
        ...
        result = report_analyzer.check_symbol_overhead_requirements(
            test, 'perf.report')
        print 'test %s on device %s' % (test.test_name, 'Succeeded' if result else 'Failed')
        if not result:
            exit(1)
