...
void DumpRecordCommandImpl::DumpFeatureSections() {
  std::vector<SectionDesc> feature_sections = record_file_reader_->FeatureSectionDescriptors();
  CHECK_EQ(feature_sections.size(), features_.size());
  for (size_t i = 0; i < features_.size(); ++i) {
    int feature = features_[i];
    SectionDesc& section = feature_sections[i];
...
class DumpRecordCommand : public Command {
 public:
  DumpRecordCommand()
      : Command("dumprecord", "dump perf record file",
