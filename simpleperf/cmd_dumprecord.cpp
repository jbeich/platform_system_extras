...
using FieldDumpFn = std::function<std::string(const TracingField&, const char*)>;

struct EventInfo {
  size_t tp_data_size = 0;
  std::vector<TracingField> tp_fields;
  std::vector<FieldDumpFn> tp_dump_functions;
};

std::string DumpStringField(const TracingField& field, const char* data) {
  std::string s;
...
std::string DumpIntField(const TracingField& field, const char* data) {
...
template <typename T>
std::string DumpIntArrayField(const TracingField& field, const char* data) {
  std::string s;
  for (size_t i = 0; i < field.elem_count; i++) {
    if (i != 0) {
      s.push_back(' ');
    }
    s += DumpIntField<T>(field, data);
    data += field.elem_size;
  }
  return s;
}

std::string DumpUnknownField(const TracingField& field, const char* data) {
...
FieldDumpFn GetDumpFieldFunction(const TracingField& field) {
  if (field.elem_count > 1 && field.elem_size == 1) {
    ...
    return DumpStringField;
  }
  if (field.elem_count == 1) {
    switch (field.elem_size) {
      case 1: return DumpIntField<int8_t>;
      case 2: return DumpIntField<int16_t>;
      case 4: return DumpIntField<int32_t>;
      case 8: return DumpIntField<int64_t>;
    }
  } else {
    switch (field.elem_size) {
      case 1: return DumpIntArrayField<int8_t>;
      case 2: return DumpIntArrayField<int16_t>;
      case 4: return DumpIntArrayField<int32_t>;
      case 8: return DumpIntArrayField<int64_t>;
    }
  }
  return DumpUnknownField;
}
  ...
  if (!events_.empty()) {
    ...
    if (event.tp_data_size > 0 && sr.raw_data.size >= event.tp_data_size) {
      ...
      for (size_t i = 0; i < event.tp_fields.size(); i++) {
        ...
        std::string s = event.tp_dump_functions[i](field, p);
...
void DumpRecordCommand::ProcessTracingData(const TracingDataRecord& r) {
  ...
  for (size_t i = 0; i < attrs.size(); i++) {
    ...
    for (size_t j = 0; j < event.tp_fields.size(); j++) {
      ...
      event.tp_dump_functions.push_back(GetDumpFieldFunction(field));
