...
std::unique_ptr<EventFd> EventFd::OpenEventFile(const perf_event_attr& attr, pid_t pid, int cpu,
  ...
  if (perf_event_fd == -1) {
    if (errno == EOPNOTSUPP && (perf_attr.sample_type & PERF_SAMPLE_BRANCH_STACK)) {
      (report_error ? PLOG(ERROR) : PLOG(DEBUG))
          << "open perf_event_file (event " << event_name << ", pid " << pid << ", cpu " << cpu
          << ") failed (branch stack sampling is not supported by arm)";
    } else {
