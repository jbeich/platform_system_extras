...
std::unique_ptr<EventFd> EventFd::OpenEventFile(const EventAttr& attr, pid_t pid, int cpu) {
  ...
  const EventType* event_type = EventTypeFactory::FindEventTypeByConfig(perf_attr.type, perf_attr.config);
  if (event_type != nullptr) {
    event_name = event_type->name;
  }
  int perf_event_fd = perf_event_open(&perf_attr, pid, cpu, -1, 0);
  if (perf_event_fd == -1) {
    PLOG(DEBUG) << "open perf_event_file (event " << event_name << ", pid " << pid << ", cpu " << cpu
                << ") failed";
    return nullptr;
  }
  if (fcntl(perf_event_fd, F_SETFD, FD_CLOEXEC) == -1) {
    PLOG(DEBUG) << "fcntl(FD_CLOEXEC) for perf_event_file (event " << event_name << ", pid " << pid
                << ", cpu " << cpu << ") failed";
    return nullptr;
  }
...
bool EventFd::StartMonitoring() {
  int result = ioctl(perf_event_fd_, PERF_EVENT_IOC_ENABLE, 0);
  if (result < 0) {
    PLOG(DEBUG) << "ioctl(enable) " << Name() << " failed";
    return false;
  }
  return true;
}

bool EventFd::StopMonitoring() {
  int result = ioctl(perf_event_fd_, PERF_EVENT_IOC_DISABLE, 0);
  if (result < 0) {
    PLOG(DEBUG) << "ioctl(disable) " << Name() << " failed";
    return false;
  }
  return true;
}
