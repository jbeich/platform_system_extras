...
class TempSymFile {
 public:
  static std::unique_ptr<TempSymFile> Create(std::string&& path, bool remove_in_destructor) {
    FILE* fp = fopen(path.c_str(), "web");
    if (fp == nullptr) {
      PLOG(ERROR) << "failed to create " << path;
      return nullptr;
    }
    return std::unique_ptr<TempSymFile>(new TempSymFile(std::move(path), fp, remove_in_destructor));
  }

  ~TempSymFile() {
    fclose(fp_);
    if (remove_in_destructor_) {
      unlink(path_.c_str());
    }
  }

  bool WriteEntry(const char* data, size_t size) {
    if (fwrite(data, size, 1, fp_) != 1) {
 ...
 private:
  TempSymFile(std::string&& path, FILE* fp, bool remove_in_destructor)
      : path_(std::move(path)), fp_(fp), remove_in_destructor_(remove_in_destructor) {}

  const std::string path_;
  FILE* const fp_;
  const bool remove_in_destructor_;
  uint64_t file_offset_ = 0;
};

JITDebugReader::JITDebugReader(const std::string& symfile_prefix, bool keep_symfiles,
                               bool sync_with_records)
    : symfile_prefix_(symfile_prefix),
      keep_symfiles_(keep_symfiles),
      sync_with_records_(sync_with_records) {}
...
bool JITDebugReader::ReadProcess(Process& process, std::vector<JITDebugInfo>* debug_info) {
  ...
  // 3. Read new symfiles.
  return ReadDebugInfo(process, jit_descriptor, true, debug_info) &&
         ReadDebugInfo(process, dex_descriptor, false, debug_info);
}

bool JITDebugReader::ReadDebugInfo(Process& process, Descriptor& new_descriptor, bool is_jit,
                                   std::vector<JITDebugInfo>* debug_info) {
  Descriptor* old_descriptor = is_jit ? &process.last_jit_descriptor : &process.last_dex_descriptor;

  bool has_update = new_descriptor.action_seqlock != old_descriptor->action_seqlock &&
                    (new_descriptor.action_seqlock & 1) == 0;
  LOG(DEBUG) << (is_jit ? "JIT" : "Dex") << " symfiles of pid " << process.pid << ": old seqlock "
             << old_descriptor->action_seqlock << ", new seqlock " << new_descriptor.action_seqlock;
  if (!has_update) {
    return true;
  }
  ...
  // If the descriptor was changed while we were reading new entries, skip reading debug info this time.
  if (IsDescriptorChanged(process, new_descriptor, is_jit)) {
    return true;
  }
  LOG(DEBUG) << (is_jit ? "JIT" : "Dex") << " symfiles of pid " << process.pid << ": read "
             << new_entries.size() << " new entries";
...
bool JITDebugReader::IsDescriptorChanged(Process& process, Descriptor& prev_descriptor, bool is_jit) {
...
bool JITDebugReader::ReadJITCodeDebugInfo(Process& process,
  ...
  for (auto& jit_entry : jit_entries) {
    ...
    if (!app_symfile_) {
      ...
      app_symfile_ = TempSymFile::Create(std::move(path), !keep_symfiles_);
      if (!app_symfile_) {
        return false;
      }
    }
