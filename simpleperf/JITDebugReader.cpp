...
bool JITDescriptor<ADDRT>::Valid() const {
  ...
  if (sizeof(ADDRT) == 4) {
    return sizeof_entry == (Version() == 1) ? sizeof(JITCodeEntry32) : sizeof(JITCodeEntry32V2);
  }
  return sizeof_entry == (Version() == 1) ? sizeof(JITCodeEntry64) : sizeof(JITCodeEntry64V2);
}
...
bool JITDebugReader::LoadDescriptorImpl(const char* data, Descriptor* descriptor) {
  ...
  descriptor->version = raw_descriptor.Version();
  return true;
}
