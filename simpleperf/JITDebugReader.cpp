...
using jit_debug_reader_impl::Descriptor;
using jit_debug_reader_impl::CodeEntry;
...
bool JITDebugReader::TryInit() {
  ...
  if (descriptors_size_ >= 4096u) {
    PLOG(DEBUG) << "The descriptors_size is unexpected large: " << descriptors_size_;
  }
...
bool JITDebugReader::ReadNewCodeEntriesImpl(const Descriptor& descriptor,
  ...
  for (size_t i = 0u; i < EXPECTED_MAX_JIT_LINKLED_LIST_LENGTH && current_entry_addr != 0u; ++i) {
    ...
    if (entry.register_timestamp <= last_action_timestamp) {
      // The linked list has entries with timestamp in non-increasing order. So stop searching
...
void JITDebugReader::ReadJITSymFiles(const std::vector<CodeEntry>& jit_entries,
  ...
  for (auto& jit_entry : jit_entries) {
    if (jit_entry.symfile_size > EXPECTED_MAX_SYMFILE_SIZE) {
      continue;
    }
    ...
    auto callback = [&](const ElfFileSymbol& symbol) {
      ...
      LOG(VERBOSE) << "JITSymbol " << symbol.name << "  at [" << std::hex << symbol.vaddr
                   << " - " << (symbol.vaddr + symbol.len) << " size " << symbol.len;
    };
    if (ParseSymbolsFromElfFileInMemory(data.data(), jit_entry.symfile_size, callback) !=
        ElfStatus::NO_ERROR) {
      continue;
    }
    TemporaryFile* tmp_file = ScopedTempFiles::CreateTempFile(!keep_symfiles_);
...
void JITDebugReader::ReadDexSymFiles(const std::vector<CodeEntry>& dex_entries,
  ...
  for (auto& dex_entry : dex_entries) {
    ...
    // dex_pcs will hit data sections of dex files. However, for compacted dex files in vdex files,
    // the data section isn't included in the range of the dex file. So we need to find the
    // range of data section in memory.
    uint64_t data_section_start;
    uint64_t data_section_end;
    if (!GetDataSectionRangeOfDexFile(it->name, dex_file_offset, &data_section_start,
                                      &data_section_end)) {
      continue;
    }
    DexSymFile symfile;
    symfile.addr = data_section_start + dex_file_offset - it->pgoff + it->start_addr;
    symfile.len = data_section_end - data_section_start;
    symfile.pgoff = dex_entry.symfile_addr;
    symfile.file_path = it->name + GetDexFileOffsetSep() + std::to_string(dex_file_offset);
