...
// It takes about 30us-130us on Pixel (depending on the cpu frequency) to check update if the
// descriptors have been updated (most time spent in process_vm_preadv). We want to know if the JIT
// debug info changed as soon as possible, while not wasting too much time checking for updates. So
// use a period of 100 ms.
...
bool JITDebugReader::UpdateRecord(const Record* record) {
  if (record->type() == PERF_RECORD_MMAP) {
    ...
    if (android::base::EndsWith(r->filename, "libart.so")) {
