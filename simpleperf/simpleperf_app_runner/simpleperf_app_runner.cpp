...
static void CheckSimpleperfArguments(const char* cmdname, char** args) {
  ...
  std::set<std::string> one_arg_options = {
      "--app", "-c", "--call-graph", "--callchain-joiner-min-matching-nodes", "--clockid", "--cpu",
      "--cpu-percent", "--duration", "-e", "-f", "--group", "--interval", "-j", "--log", "-m",
      "-p", "--size-limit", "--symfs", "-t", "--tracepoint-events",
  };
  ...
  for (int i = 0; args[i] != nullptr; ++i) {
    if (zero_arg_options.count(args[i])) {
      ...
      ++i;
    } else if (fd_options.count(args[i])) {
      if (args[i + 1] == nullptr) {
        error(1, 0, "invalid arg: %s", args[i]);
      }
      ...
      int fd;
      std::string file;
      if (!android::base::ParseInt(args[i + 1], &fd) || fd < 3 ||
          !android::base::Readlink("/proc/self/fd/" + std::to_string(fd), &file)) {
