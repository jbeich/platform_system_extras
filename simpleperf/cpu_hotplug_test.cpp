...
static bool RecordInChildProcess(int record_cpu, int record_duration_in_second) {
  ...
  if (pid == 0) {
    ...
    extern bool fail_on_system_wide_perf_event_open;
    // It is not an error if perf_event_open fails because of cpu-hotplug.
    if (!ret && !fail_on_system_wide_perf_event_open) {
      exit(1);
    }
    exit(0);
  }
