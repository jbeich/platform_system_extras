...
const SampleEntry* SampleTree::AddSample(int pid, int tid, uint64_t ip, uint64_t time,
...
void SampleTree::AddCallChainSample(int pid, int tid, uint64_t ip, uint64_t time, uint64_t period,
  ...
  if (it != sample_tree_.end()) {
    ...
    if (callchains->find(find_sample) != callchains->end()) {
      return;
    }
  }
  const SampleEntry* inserted_sample = InsertSample(sample);
  callchains->insert(inserted_sample);
}

const SampleEntry* SampleTree::InsertSample(const SampleEntry& sample) {
  const SampleEntry* result;
  auto it = sample_tree_.find(sample);
  if (it == sample_tree_.end()) {
    ...
    find_sample->children_period += sample.children_period;
    find_sample->sample_count += sample.sample_count;
    result = find_sample;
  }
