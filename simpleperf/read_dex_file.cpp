...
static bool ReadSymbols(
  ...
  for (uint64_t offset : dex_file_offsets) {
    ...
    std::vector<art_api::dex::MethodInfo> file_syms = dex_file->GetAllMethodInfos();
...
bool ReadSymbolsFromDexFileInMemory(void* addr, uint64_t size,
  ...
  return ReadSymbols(
      dex_file_offsets, symbols, [&](uint64_t offset) -> std::unique_ptr<art_api::dex::DexFile> {
        ...
        return art_api::dex::DexFile::OpenFromMemory(file_addr, &max_file_size, "", &error_msg);
      });
}
...
bool ReadSymbolsFromDexFile(const std::string& file_path,
  ...
  return ReadSymbols(dex_file_offsets, symbols, [&](uint64_t offset) {
    std::string error_msg;
    return art_api::dex::DexFile::OpenFromFd(fd, offset, file_path, &error_msg);
  });
}
