...
static bool OpenDexFile(const std::string& filepath, uint64_t offset,
                        const std::function<bool (const art::DexFile&)>& callback) {
  android::base::unique_fd fd(TEMP_FAILURE_RETRY(open(filepath.c_str(), O_RDONLY | O_CLOEXEC)));
  if (fd == -1) {
    return false;
  }
  struct stat buf;
  ...
  uint64_t length;
  if (__builtin_add_overflow(offset, sizeof(art::DexFile::Header), &length) ||
      file_size < length) {
    return false;
  }
  ...
  art::DexFile::Header* header = reinterpret_cast<art::DexFile::Header*>(
      static_cast<char*>(mapped_addr) + offset);
  if (__builtin_add_overflow(offset, header->file_size_, &length) || file_size < length) {
    return false;
  }
  art::DexFileLoader loader;
  std::string error;
  std::unique_ptr<const art::DexFile> dex_file = loader.Open(
      reinterpret_cast<uint8_t*>(header), header->file_size_, "", 0, nullptr, false,
      false, &error);
  bool result;
  if (dex_file) {
    result = callback(*dex_file);
  } else {
    result = false;
  }
...
bool ParseSymbolsFromDexFile(const std::string& filepath, uint64_t offset,
  ...
  auto dexfile_callback = [&](const art::DexFile& dex_file) {
  ...
  return OpenDexFile(filepath, offset, dexfile_callback);
}
...
bool GetDataSectionRangeOfDexFile(const std::string& filepath, uint64_t offset,
  ...
  auto callback = [&](const art::DexFile& dex_file) {
    *data_section_start = dex_file.GetHeader().data_off_;
    *data_section_end = dex_file.GetHeader().data_off_ + dex_file.GetHeader().data_size_;
    return true;
  };
  return OpenDexFile(filepath, offset, callback);
}
