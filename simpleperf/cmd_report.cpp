...
void ProcessReportFormatter::SampleCallback(const SampleInMap& sample_in_map) {
  ...
  uint64_t increase_period = 0;
  for (auto& sample : sample_in_map.samples) {
    increase_period += sample.period;
  }
  info.period += increase_period;
  total_period_ += increase_period;
}
...
bool ReportCommandImpl::ParseOptions(const std::vector<std::string>& args) {
  for (size_t i = 1; i < args.size(); ++i) {
    ...
    } else if (args[i] == "--exclude_fileinfo") {
      exclude_fileinfo_ = true;
    } else {
      LOG(ERROR) << "Unknown option for report command: '" << args[i] << "'\n";
      LOG(ERROR) << "Try `simpleperf help report`";
      return false;
    }
  }
  return true;
 ...
 public:
  ReportCommand()
      : Command("report", "report sampling information in perf.data",
                ...
                "    --exclude_fileinfo  don't show file specific sample info\n") {
  }
