...
"--max-stack <len>     Set max call chain length when printing call graph.\n"
"-n         Print the sample count for each item.\n"
"--no-demangle         Don't demangle symbol names.\n"
"--no-show-ip          Don't show vaddr in file for unknown symbols.\n"
"-o report_file_name   Set report file name, default is stdout.\n"
"--percent-limit <percent>  Set min percent limit when printing call graph.\n"
...
bool ReportCommand::ParseOptions(const std::vector<std::string>& args) {
  ...
  for (size_t i = 0; i < args.size(); ++i) {
    if (args[i] == "-b") {
      ...
      if (!ParseDouble(args[i].c_str(), &callgraph_percent_limit_)) {
