...
class ReportCommandImpl {
 public:
  ReportCommandImpl() : record_filename_("perf.data") {
  }
...
void ReportCommandImpl::ReadSampleTreeFromRecordFile() {
  compare_sample_func_t compare_sample_callback = std::bind(&ReportCommandImpl::CompareSampleEntry, this,
...
int ReportCommandImpl::CompareSampleEntry(const SampleEntry& sample1, const SampleEntry& sample2) {
...
void ReportCommandImpl::PrintReport() {
  PrintReportContext();
  PrintReportHeader();
  sample_tree_->VisitAllSamples(std::bind(&ReportCommandImpl::PrintReportEntry, this, std::placeholders::_1));
  fflush(stdout);
}

void ReportCommandImpl::PrintReportContext() {
  ...
  printf("Samples: %" PRIu64 " of event '%s'\n", sample_tree_->TotalSamples(), event_type_name.c_str());
  printf("Event count: %" PRIu64 "\n\n", sample_tree_->TotalPeriod());
}

void ReportCommandImpl::PrintReportHeader() {
  printf("%8s", "Overhead");
...
void ReportCommandImpl::PrintReportEntry(const SampleEntry& sample) {
 ...
 public:
  ReportCommand()
      : Command("report", "report sampling information in perf.data",
                ...
                "                         Possible keys include pid, comm, dso. default \n"
