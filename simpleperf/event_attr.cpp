...
EventAttr EventAttr::CreateDefaultAttrToMonitorAnEvent(const Event& event) {
  perf_event_attr attr;
  memset(&attr, 0, sizeof(attr));
  attr.size = sizeof(perf_event_attr);
  attr.type = event.Type();
  attr.config = event.TypeSpecificConfig();
  attr.mmap = 1;
  attr.comm = 1;
...
void EventAttr::Dump(size_t indent) const {
  ...
  const Event* event = Event::FindEventByTypeAndConfig(attr_.type, attr_.config);
  if (event != nullptr) {
