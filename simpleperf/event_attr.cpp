...
static std::string ReadFormatToString(uint64_t read_format) {
  ...
  std::string result;
  for (auto p : read_format_names) {
    if (read_format & p.first) {
      read_format &= ~p.first;
      if (!result.empty()) {
        result += ", ";
      }
  ...
  if (read_format != 0) {
    LOG(DEBUG) << "unknown read_format bits: " << std::hex << read_format;
  }
  return result;
}
...
void EventAttr::Dump(size_t indent) const {
  ...
  PrintIndented(indent + 1, "read_format (0x%llx) %s\n", attr_.read_format,
                ReadFormatToString(attr_.read_format).c_str());
