...
static void SetCpuOneOnline(bool online, bool* has_error, bool* interrupted) {
  *interrupted = false;
  ...
  if (new_state == online) {
    ...
    *has_error = true;
    FAIL() << "Failed to SetCpuOneOnline, online = " << online << ", error = " << strerror(saved_errno) << ", new_state = " << new_state;
  }
}

// On some device like flo, the kernel can't work correctly if a cpu
// is offlined when perf is monitoring a hardware event. This test
// verifies if it can work correctly.
TEST(cpu_offline, smoke) {
  if (!HaveCpuOne()) {
    GTEST_LOG_(INFO) << "This test does nothing on uniprocessor device.";
    return;
  }
  ...
  // A loop is used in case the test is interrupted by other processes controling cpu hotplug, like mpdecision.
