...
class SampleManager {
 ...
 private:
  ...
  bool SearchThreads();
  bool CreateTimerForThread(pid_t tid, timer_t* timerid);
  void DestroyTimer(timer_t& timerid);
  void SendMapData();
...
bool SampleManager::SearchThreads() {
  LOG(ERROR) << "Enter SearchThreads";
  bool send_map_data = send_map_data_failed_;
  ...
  LOG(ERROR) << "End SearchThread";
  return true;
}

bool SampleManager::CreateTimerForThread(pid_t tid, timer_t* timer_id) {
  sigevent se;
  se.sigev_signo = signo_;
  se.sigev_notify = SIGEV_THREAD_ID;
  se._sigev_un._tid = tid;
  if (timer_create(CLOCK_MONOTONIC, &se, timer_id) != 0) {
    PLOG(ERROR) << "timer_create() failed";
    return false;
  }
  uint32_t sample_period_in_ns = 1000000000ULL / sample_freq_;
  if (sample_period_in_ns == 0) {
    sample_period_in_ns = 1;
  }
  itimerspec ts;
  ts.it_value.tv_sec = 0;
  ts.it_value.tv_nsec = sample_period_in_ns;
  ts.it_interval.tv_sec = 0;
  ts.it_interval.tv_nsec = sample_period_in_ns;
  if (timer_settime(*timer_id, 0, &ts, nullptr) != 0) {
    PLOG(ERROR) << "timer_settime() failed";
...
void SampleManager::DestroyTimer(timer_t& timer_id) {
  timer_delete(timer_id);
}

void SampleManager::SendMapData() {
  ...
  send_map_data_failed_ = !conn_->SendMessage(*pmsg);
}
