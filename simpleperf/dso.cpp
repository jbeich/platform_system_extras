...
class DexFileDso : public Dso {
 public:
  ...
  std::vector<Symbol> LoadSymbolsImpl() override {
    ...
    const std::string& path = GetDebugFilePath();
    auto tuple = SplitUrlInApk(path);
    ...
    if (!IsRegularFile(std::get<0>(tuple) ? std::get<1>(tuple) : path)) {
      LOG(DEBUG) << "skip reading symbols from non-exist dex_file " << path;
      return symbols;
    }
    bool status = false;
    ...
    if (std::get<0>(tuple)) {
      ...
      if (ahelper && ahelper->FindEntry(std::get<2>(tuple), &entry) &&
        ...
        status = ReadSymbolsFromDexFileInMemory(data.data(), data.size(), path, dex_file_offsets_,
                                                symbol_callback);
      }
    } else {
      status = ReadSymbolsFromDexFile(path, dex_file_offsets_, symbol_callback);
    }
    if (!status) {
      android::base::LogSeverity level =
          symbols_.empty() ? android::base::WARNING : android::base::DEBUG;
      LOG(level) << "Failed to read symbols from dex_file " << path;
      return symbols;
    }
    LOG(VERBOSE) << "Read symbols from dex_file " << path << " successfully";
