...
bool CheckReadSymbolResult(ReadElfRet result, const std::string& filename) {
  if (result == ReadElfRet::NO_ERROR) {
    return true;
  } else if (result == ReadElfRet::NO_SYMBOL_TABLE) {
    // Laking symbol table isn't considered as an error but worth reporting.
...
bool Dso::LoadKernelModule() {
  BuildId build_id = GetExpectedBuildId();
  ReadElfRet result = ParseSymbolsFromElfFile(GetDebugFilePath(), build_id,
