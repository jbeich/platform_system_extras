...
class SimplePerfWriterFuzzer {
 public:
  SimplePerfWriterFuzzer(const uint8_t* data, size_t size) : mFdp(data, size) {
    /**
     * Use maximum of 80% of buffer to write in FD and save atleast 20% for fuzzing other APIs
     */
  ...
  while (features-- > 0) {
    auto invokeWriter = mFdp.PickValueInArray<const std::function<void()>>({
        ...
        [&]() {
          ...
          for (int32_t idx = 0; idx < iter; ++idx) {
            if (mFdp.ConsumeBool()) {
              DexFileDso* dso = new DexFileDso(mFdp.ConsumeRandomLengthString(kMaxLen));
              dsos.push_back(dso);
            } else {
              ElfDso* dso = new ElfDso(mFdp.ConsumeRandomLengthString(kMaxLen) /* path */,
