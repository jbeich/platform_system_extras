...
bool DwarfUnwindAdapter::ReadFDETableFromEhFrameHdr(uint64_t eh_frame_hdr_file_offset,
                                                    const std::vector<uint8_t>& data,
                                                    std::vector<FDEEntry>* fde_table) {
  uint64_t hdr_offset = eh_frame_hdr_file_offset;
  ...
  uint64_t eh_frame_ptr =
      ReadEncodedValue(eh_frame_ptr_encode, &p, end, hdr_offset + p - start, hdr_offset);
  CHECK_NE(0u, eh_frame_ptr);
  uint64_t fde_count =
      ReadEncodedValue(fde_count_encode, &p, end, hdr_offset + p - start, hdr_offset);
  fde_table->resize(fde_count);
  for (auto& fde : *fde_table) {
    fde.start_ip_offset =
        ReadEncodedValue(table_encode, &p, end, hdr_offset + p - start, hdr_offset);
    fde.fde_offset = ReadEncodedValue(table_encode, &p, end, hdr_offset + p - start, hdr_offset);
  }
...
const DwarfUnwindAdapter::DebugFrame* DwarfUnwindAdapter::GetDebugFrameInFile(
  ...
  // TODO: support .debug_frame.
  std::vector<ElfFileSection> sections(2);
  ...
  if (sections[0].data.empty() || sections[1].data.empty()) {
    LOG(ERROR) << "can't unwind because of laking .eh_frame_hdr/.eh_frame in " << filename;
    return nullptr;
  }
  std::vector<FDEEntry> fde_table;
  if (!ReadFDETableFromEhFrameHdr(sections[0].offset, sections[0].data, &fde_table)) {
...
bool DwarfUnwindAdapter::FindProcInfo(unw_addr_space_t addr_space, uint64_t ip,
  ...
  // Using address space starting at 0 for fde table.
  fde_table_.start = 0;
  fde_table_.end = debug_frame->fde_table.size() * sizeof(FDEEntry);
  fde_table_.data = reinterpret_cast<const uint8_t*>(debug_frame->fde_table.data());

  if (IsIntersected(map->start_addr, map->start_addr + map->len, fde_table_)) {
    LOG(ERROR) << "map intersects with fde table";
    return false;
  }
  if (IsIntersected(fde_table_.start, fde_table_.end, stack_)) {
    LOG(ERROR) << "fde table intersects with stack";
  }

  // TODO: conside load_bias.
  unw_dyn_info_t dyn_info;
  memset(&dyn_info, 0, sizeof(dyn_info));
  dyn_info.start_ip = map->start_addr;
  dyn_info.end_ip = map->start_addr + map->len;
  dyn_info.format = UNW_INFO_FORMAT_REMOTE_TABLE;
  dyn_info.u.rti.name_ptr = 0;
  dyn_info.u.rti.segbase = map->start_addr - map->pgoff;
  dyn_info.u.rti.table_data = 0;  // Point to fde_table.
  dyn_info.u.rti.table_len = (fde_table_.end - fde_table_.start) / sizeof(unw_word_t);
  int result =
      dwarf_search_unwind_table(addr_space, ip, &dyn_info, proc_info, need_unwind_info, this);
  return result == 0;
}
...
bool DwarfUnwindAdapter::ReadMem(uint64_t addr, uint64_t* value) {
  if (addr >= fde_table_.start && addr < fde_table_.end) {
    *value = AlignedRead64(fde_table_.data + addr - fde_table_.start,
                           fde_table_.data + fde_table_.end - fde_table_.start);
    return true;
  }
...
std::vector<uint64_t> DwarfUnwindAdapter::UnwindCallChain(const ThreadEntry& thread,
  ...
  while (true) {
    ...
    if (retval <= 0) {
      if (retval < 0) {
        LOG(ERROR) << "unw_step() failed: " << retval;
      }
      break;
    }
    unw_word_t value;
    ...
    if (retval < 0) {
      LOG(ERROR) << "unw_get_reg() failed: " << retval;
    }
