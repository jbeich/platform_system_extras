...
bool DwarfUnwindAdapter::FindProcInfo(unw_addr_space_t addr_space, uint64_t ip,
  ...
  if (debug_frame->is_eh_frame) {
    fde_table_.start = 0;
    fde_table_.end = debug_frame->fde_table.size() * sizeof(FdeIndex);
    ...
    di.u.rti.table_data = 0;
    di.u.rti.table_len = (fde_table_.end - fde_table_.start) / sizeof(unw_word_t);
    result = dwarf_search_unwind_table(addr_space, ip, &di, proc_info, need_unwind_info, this);
    fde_table_.end = 0;
  } else {
    unw_dyn_info_t di;
    // TODO: Using dwarf_find_debug_frame() have address conflicts if more than one process
    // are involved. Fix this.
    int found = dwarf_find_debug_frame(0, &di, ip, segbase, nullptr, map->start_addr,
