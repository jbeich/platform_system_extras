...
bool DwarfUnwindAdapter::ReadFdeTableFromDebugFrameSection(const std::vector<uint8_t>& data,
                                                           std::vector<FdeIndex>* fde_table) {
...
bool DwarfUnwindAdapter::GetFdeOffsetByIp(const std::vector<FdeIndex>& fde_table,
                                          uint32_t ip_offset, uint32_t* fde_offset) {
  size_t low, high;
...
bool DwarfUnwindAdapter::FindProcInfo(unw_addr_space_t addr_space, uint64_t ip,
  ...
  if (!GetFdeOffsetByIp(debug_frame->fde_table, ip_offset, &fde_offset)) {
    LOG(DEBUG) << "GetFdeOffsetByIp() failed for file " << filename << ", ip_offset " << std::hex
               << ip_offset;
    return false;
  }
  ...
  if (debug_frame->is_eh_frame) {
    ...
    result = dwarf_extract_proc_info_from_fde(addr_space, unw_get_accessors(addr_space), &fde_addr,
                                              proc_info, need_unwind_info, 0, this);
  } else {
    ...
    result = dwarf_extract_proc_info_from_fde(unw_local_addr_space,
                                              unw_get_accessors(unw_local_addr_space), &fde_addr,
                                              proc_info, need_unwind_info, debug_frame_base, this);
    if (result == 0) {
      // .debug_frame uses an absolute encoding that does not know about any shared library
      // relocation.
