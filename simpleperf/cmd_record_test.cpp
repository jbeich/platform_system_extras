...
TEST(record_cmd, include_filter_option) {
  ...
  ASSERT_TRUE(RunRecordCmd({"-e", "cs-etm", "--include-filter", sleep_exec_path}, record_file.path));
  TemporaryFile inject_file;
  ASSERT_TRUE(CreateCommandInstance("inject")->Run({"-i", record_file.path, "-o", inject_file.path}));
  std::string data;
