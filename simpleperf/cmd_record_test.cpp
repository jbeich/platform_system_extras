...
#if defined(__ANDROID__)
    // Gerrit runs Test Mapping presubmit check using cloud cuttlefish devices without hardware pmu
    // support. So skip hardware counter tests on them.
    std::string prop_value = android::base::GetProperty("ro.build.flavor", "");
    if (prop_value.find("cf_x86") != std::string::npos) {
      has_hw_counter = 0;
    }
#endif
#endif
