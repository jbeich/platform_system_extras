...
void CheckKernelInfo(const std::string& path, bool has_kallsyms,
                     bool* success) {
  *success = false;
  std::unique_ptr<RecordFileReader> reader =
  ...
  for (const auto& record : records) {
    if (record->type() != SIMPLE_PERF_RECORD_KERNEL_INFO) {
      continue;
    }
    auto& r = *static_cast<KernelInfoRecord*>(record.get());
    ASSERT_FALSE(r.version.empty());
    ASSERT_EQ(has_kallsyms, !r.kallsyms.empty());
    *success = true;
    break;
  }
}

TEST(record_cmd, kernel_info) {
  TemporaryFile tmpfile;
  ASSERT_TRUE(RunRecordCmd({}, tmpfile.path));
  bool success;
  CheckKernelInfo(tmpfile.path, true, &success);
  ASSERT_TRUE(success);
  ASSERT_TRUE(RunRecordCmd({"--no-dump-kernel-symbols"}, tmpfile.path));
  CheckKernelInfo(tmpfile.path, false, &success);
  ASSERT_TRUE(success);
}
