...
TEST(record_cmd, addr_filter_option) {
  ...
  ASSERT_TRUE(RunRecordCmd({"-e", "cs-etm", "--addr-filter", filter}));
  // file range
  filter = StringPrintf("filter 0x%" PRIx64 "-0x%" PRIx64 "@%s", addr, addr + 4, sleep_exec_path.c_str());
  ASSERT_TRUE(RunRecordCmd({"-e", "cs-etm", "--addr-filter", filter}));
  // TODO: enable kernel addr test after getting "perf/core: Fix crash when using HW tracing kernel filters" to
  // android kernel 4.14.
  // kernel start
