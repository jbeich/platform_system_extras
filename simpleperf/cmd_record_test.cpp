...
TEST(record_cmd, tp_filter_option) {
  ...
  // Check that samples with prev_comm == sleep is filtered out. Although we do the check all the
  // time, it only makes sense when running as root. Because tracepoint event fields are not allowed
