...
char* RecordBuffer::AllocWriteSpace(size_t record_size) {
  ...
  if (write_head < write_tail) {
    ...
    if (buffer_size_ - write_head >= sizeof(perf_event_header)) {
      ...
      memset(buffer_ + write_head, 0, sizeof(perf_event_header));
    }
    cur_write_record_size_ += buffer_size_ - write_head;
    write_head = 0;
  }
  return buffer_ + write_head;
}
...
char* RecordBuffer::GetCurrentRecord() {
  ...
  if (read_head > write_head) {
    if (buffer_size_ - read_head < sizeof(header) ||
        (memcpy(&header, buffer_ + read_head, sizeof(header)) && header.size == 0)) {
    ...
    memcpy(&header, buffer_ + read_head, sizeof(header));
  }
  cur_read_record_size_ += header.size;
  return buffer_ + read_head;
}
...
static constexpr int kNoCmd = 0;
static constexpr int kCmdAddEventFds = 1;
static constexpr int kCmdRemoveEventFds = 2;
static constexpr int kCmdSyncKernelBuffer = 3;
static constexpr int kCmdStopThread = 4;
...
RecordReadThread::RecordReadThread(size_t record_buffer_size, const perf_event_attr& attr,
  ...
  record_buffer_low_level_ = std::min<size_t>(record_buffer_size / 4, 10 * 1024 * 1024);
  record_buffer_critical_level_ = std::min<size_t>(record_buffer_size / 6, 5 * 1024 * 1024);
}

RecordReadThread::~RecordReadThread() {
  if (read_thread_) {
    SendCmdToReadThread(kCmdStopThread, nullptr);
    read_thread_->join();
  }
}
...
bool RecordReadThread::AddEventFds(const std::vector<EventFd*>& event_fds) {
  return SendCmdToReadThread(kCmdAddEventFds, const_cast<std::vector<EventFd*>*>(&event_fds));
}

bool RecordReadThread::RemoveEventFds(const std::vector<EventFd*>& event_fds) {
  return SendCmdToReadThread(kCmdRemoveEventFds, const_cast<std::vector<EventFd*>*>(&event_fds));
}

bool RecordReadThread::SyncKernelBuffer() {
  return SendCmdToReadThread(kCmdSyncKernelBuffer, nullptr);
}

bool RecordReadThread::SendCmdToReadThread(int cmd, void* cmd_arg) {
  {
