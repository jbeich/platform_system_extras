...
std::vector<uint64_t> RecordFileReader::ReadAuxTraceFeature() {
  ...
  const char* end = buf.data() + buf.size();
  if (buf.size() / 8 % 2 == 1) {
    // Recording file generated by linux perf contains an uint64 field. It isn't used by
    // simpleperf. But we need to skip it to support recording files generated by perf.
    p += sizeof(uint64_t);
  }
  while (p < end) {
    uint64_t offset;
