...
bool RecordFileReader::ReadRecord(std::unique_ptr<Record>& record,
  ...
  while (read_record_size_ < header_.data.size && record == nullptr) {
    ...
    if (record->type() == SIMPLE_PERF_RECORD_EVENT_ID) {
      ...
      // Although we have removed ip == 0 in callchains when recording dwarf based callgraph,
      // stack frame based callgraph can also generate ip == 0. Remote it here to avoid caller's
      // effort.
