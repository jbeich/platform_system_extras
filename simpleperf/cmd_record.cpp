...
class RecordCommandImpl {
 ...
 private:
  ...
  EventSelection event_selection_;
...
bool RecordCommandImpl::Run(const std::vector<std::string>& args) {
  ...
  if (system_wide_collection_) {
    if (!event_selection_.OpenEventFilesForAllCpus()) {
      return false;
    }
  } else {
    event_selection_.EnableOnExec();
    if (!event_selection_.OpenEventFilesForProcess(workload->GetPid())) {
      return false;
    }
  }
  if (!event_selection_.MmapEventFiles(perf_mmap_pages_)) {
    return false;
  }
  std::vector<pollfd> pollfds;
  event_selection_.PreparePollForEventFiles(&pollfds);

  // 4. Open record file writer.
  record_file_writer_ = RecordFileWriter::CreateInstance(record_filename_, event_selection_.Attr(),
                                                         event_selection_.EventFds());
  ...
  // recording manually.
  if (system_wide_collection_) {
    if (!event_selection_.EnableEvents()) {
      return false;
    }
  }
  if (!workload->Start()) {
  ...
  while (true) {
    if (!event_selection_.ReadMmapEventData(callback)) {
      return false;
    }
...
void RecordCommandImpl::CreateEventSelection() {
  event_selection_ = EventSelection(measured_event_type_);
  if (use_sample_freq_) {
    event_selection_.SetSampleFreq(sample_freq_);
  } else {
    event_selection_.SetSamplePeriod(sample_period_);
  }
  event_selection_.SampleIdAll();
}
