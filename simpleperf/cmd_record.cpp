...
bool RecordCommand::ParseOptions(const std::vector<std::string>& args,
  ...
  for (i = 0; i < args.size() && !args[i].empty() && args[i][0] == '-'; ++i) {
    if (args[i] == "-a") {
      ...
      uint64_t value;
      if (!GetValueOption(args, &i, &value, 1)) {
        return false;
      }
      if (args[i-1] == "-c") {
      ...
      if (!GetValueOption(args, &i, &duration_in_sec_, 1e-9)) {
        return false;
      }
    } else if (args[i] == "-e") {
      if (!NextArgumentOrError(args, &i)) {
      ...
      uint64_t pages;
      if (!GetValueOption(args, &i, &pages)) {
        return false;
      }
      ...
      uint64_t value;
      if (!GetValueOption(args, &i, &value, 1, std::numeric_limits<size_t>::max())) {
        return false;
      }
      callchain_joiner_min_matching_nodes_ = value;
    } else if (args[i] == "-o") {
      if (!NextArgumentOrError(args, &i)) {
        return false;
      }
      ...
      if (!GetValueOption(args, &i, &size_limit_in_bytes_, 1, std::numeric_limits<uint64_t>::max(),
                          true)) {
        return false;
      }
    } else if (args[i] == "--start_profiling_fd") {
      uint64_t value;
      if (!GetValueOption(args, &i, &value, 0, std::numeric_limits<int>::max())) {
        return false;
      }
      start_profiling_fd_ = static_cast<int>(value);
