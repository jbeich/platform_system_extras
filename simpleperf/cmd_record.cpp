...
"--app package_name    Profile an Android application. It uses run-as to run\n"
"                      as the same user as the application. By default, the\n"
"                      whole process will be monitored.\n"
#endif
...
bool RecordCommand::Run(const std::vector<std::string>& args) {
  ...
  if (!app_package_name_.empty() && !in_app_context_) {
    // Root can do everything, so we only switch to app's context when we are not root.
    if (!IsRoot()) {
