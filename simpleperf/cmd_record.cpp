...
bool RecordCommand::SaveRecordForPostUnwinding(Record* record) {
  ...
  if (!record_file_writer_->WriteRecord(*record)) {
    LOG(ERROR)
        << "If no enough space for storing profiling data, consider using --no-post-unwind option.";
    return false;
  }
  return true;
}
...
bool RecordCommand::PostUnwindRecords() {
  ...
  // 2. Read records from the temporary file, and write unwound records with back to
  // record_filename_.
  record_file_writer_ = CreateRecordFile(record_filename_);
