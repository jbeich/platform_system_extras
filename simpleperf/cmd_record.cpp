...
std::optional<size_t> GetDefaultRecordBufferSize(bool system_wide_recording) {
  ...
  // But on devices with memory >= 4G, we increase buffer size to 256MB. This reduces the chance
  // of cutting samples, which can cause broken callchains.
  static constexpr size_t kLowMemoryRecordBufferSize = 64 * 1024 * 1024;
  static constexpr size_t kHighMemoryRecordBufferSize = 256 * 1024 * 1024;
  static constexpr size_t kSystemWideRecordBufferSize = 256 * 1024 * 1024;
  static constexpr uint64_t kHighMemoryLimit = 3ull * 1024 * 1024 * 1024;
  ...
  return device_memory.value() > kHighMemoryLimit ? kHighMemoryRecordBufferSize
                                                  : kLowMemoryRecordBufferSize;
}
...
std::string RecordCommand::LongHelpString() const {
  ...
  if (auto size = GetDefaultRecordBufferSize(false); size) {
    process_buffer_size = size.value() / 1024 / 1024;
  }
  if (auto size = GetDefaultRecordBufferSize(true); size) {
    system_wide_buffer_size = size.value() / 1024 / 1024;
  }
  ...
  if (process_buffer_size == system_wide_buffer_size) {
    ...
    buffer_size_str = android::base::StringPrintf(
        "%" PRIu64 "M for process recording and %" PRIu64
