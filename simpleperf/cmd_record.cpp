...
bool RecordCommand::AdjustPerfEventLimit() {
  ...
  uint64_t max_freq = max_sample_freq_;
  uint64_t cur_max_freq;
  if (GetMaxSampleFrequency(&cur_max_freq) && cur_max_freq < max_freq &&
      !SetMaxSampleFrequency(max_freq)) {
    set_prop = true;
  }
  // 2. Adjust perf_cpu_time_max_percent.
  size_t cpu_percent = cpu_time_max_percent_;
  size_t cur_percent;
  if (GetCpuTimeMaxPercent(&cur_percent) && cur_percent != cpu_percent &&
      !SetCpuTimeMaxPercent(cpu_percent)) {
    set_prop = true;
  }
  ...
  if (GetAndroidVersion() >= kAndroidVersionP + 1 && set_prop) {
    return SetPerfEventLimits(std::max(max_freq, cur_max_freq), cpu_percent,
