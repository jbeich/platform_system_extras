...
bool RecordCommand::PrepareRecording(Workload* workload) {
  ...
  if (app_pid != 0 && GetAndroidVersion() >= kAndroidVersionP) {
    ...
    // It takes about 25us to check update when no update happens. We want to know the JIT debug
    // info change as soon as possible, while not wasting too much time checking updates.
    // Probably any period between 1ms to 1s works. Here we just use a period of 100 ms.
