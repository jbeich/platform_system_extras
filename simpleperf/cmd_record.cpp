 ...
 public:
  RecordCommand()
      : Command(
            ...
            "                 method to parse call graph in stack. Default in dwarf,8192.\n"
            "    -e event[:modifier]\n"
  ...
  for (i = 0; i < args.size() && args[i].size() > 0 && args[i][0] == '-'; ++i) {
    if (args[i] == "-a") {
      ...
      if (args[i] == "fp") {
        ...
        dwarf_callchain_sampling_ = false;
      } else if (android::base::StartsWith(args[i], "dwarf")) {
        fp_callchain_sampling_ = false;
        dwarf_callchain_sampling_ = true;
        const char* p = args[i].c_str() + strlen("dwarf");
        if (*p == ',') {
          ++p;
          char* endptr;
          uint64_t size = strtoull(p, &endptr, 0);
          if (*endptr != '\0' || size > UINT_MAX) {
            LOG(ERROR) << "invalid dump stack size in --call-graph option: " << p;
            return false;
          }
          if ((size & 7) != 0) {
