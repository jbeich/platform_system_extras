 ...
 public:
  RecordCommand()
      : Command(
            ...
            "    --no-unwind  If `--call-graph dwarf` option is used, simpleperf calls unwinder\n"
            "                 to unwind user's stack by default. It will not call unwinder if\n"
            "                 this option is used.\n"
  ...
  for (auto& record : records) {
    ...
    if (record->header.type == PERF_RECORD_SAMPLE) {
      ...
      if ((r.sample_type & PERF_SAMPLE_CALLCHAIN) &&
          (r.sample_type & PERF_SAMPLE_REGS_USER) && (r.regs_user_data.reg_mask != 0) &&
