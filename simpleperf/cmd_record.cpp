...
class RecordCommand : public Command {
 public:
  RecordCommand()
    ...
    measured_event_name = "cpu-cycles";
 ...
 private:
  ...
  bool SetMeasuredEvent();
  size_t OpenEventFilesForProcess(pid_t pid);
  size_t OpenEventFilesForCpus(const std::vector<int>& cpu_list);
  bool MmapEventFiles();
 ...
 private:
  std::string measured_event_name;
...
bool RecordCommand::SetMeasuredEvent() {
  const Event* event = Event::FindEventByName(measured_event_name);
  if (event == nullptr) {
    fprintf(stderr, "Unrecognized event_name: %s\n", measured_event_name.c_str());
    return false;
  }
  if (!event->Supported()) {
...
size_t RecordCommand::OpenEventFilesForProcess(pid_t pid) {
  event_fds.clear();
...
size_t RecordCommand::OpenEventFilesForCpus(const std::vector<int>& cpu_list) {
