...
void LoadSymbolMapFile(int pid, ThreadTree* thread_tree) {
  ...
  // For now, use /data/local/tmp, which works for standalone programs.
  // TODO: support local data directory for apps.
  auto path = android::base::StringPrintf("/data/local/tmp/perf-%d.map", pid);
  ...
  auto callback = [&](const Record* r) {
    ...
    if (r->type() == PERF_RECORD_SAMPLE) {
      ...
      if (loaded_symbol_maps.insert(sample->tid_data.pid).second) {
        LoadSymbolMapFile(sample->tid_data.pid, &thread_tree_);
      }
