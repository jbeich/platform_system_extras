...
"--stdio-control-profiling    Use stdin/stdout to control pause/resume profiling.\n"
...
"--stop-signal-fd <fd>  Stop recording when fd is readable.\n"
 ...
 private:
  ...
  bool stdio_control_profiling_ = false;
...
bool RecordCommand::ParseOptions(const std::vector<std::string>& args,
  ...
  for (i = 0; i < args.size() && !args[i].empty() && args[i][0] == '-'; ++i) {
    ...
    } else if (args[i] == "--stdio-control-profiling") {
      stdio_control_profiling_ = true;
...
bool RecordCommand::ProcessControlCmd(IOEventLoop* loop) {
  ...
  std::string cmd = android::base::Trim(buf);
