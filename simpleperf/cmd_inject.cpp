...
static size_t AddrPairHash(const AddrPair& ap) {
  return (ap.first << 32) | ap.second;
}

struct BinaryInfo {
  std::unordered_map<AddrPair, uint64_t, decltype(&AddrPairHash)> range_count_map;
  std::unordered_map<AddrPair, uint64_t, decltype(&AddrPairHash)> branch_count_map;

  BinaryInfo() : range_count_map(0, AddrPairHash), branch_count_map(0, AddrPairHash) {}
};
 ...
 private:
  ...
  void PostProcess() {
    for (auto& p : binary_map_) {
      ...
      auto& range_count_map = p.second.range_count_map;
      fprintf(output_fp_.get(), "%zu\n", range_count_map.size());
      for (auto& p2 : range_count_map) {
      ...
      auto& branch_count_map = p.second.branch_count_map;
      fprintf(output_fp_.get(), "%zu\n", branch_count_map.size());
      for (auto& p2 : branch_count_map) {
        fprintf(output_fp_.get(), "%" PRIx64 "->%" PRIx64 ":%" PRIu64 "\n", p2.first.first,
                p2.first.second, p2.second);
      }
