...
static std::vector<AutoFDOBinaryInfo> ConvertLBRDataToAutoFDO(const LBRData& lbr_data) {
...
class LBRPerfDataReader : public PerfDataReader {
 ...
 private:
  ...
  bool PostProcess() override {
    if (autofdo_callback_) {
      std::vector<AutoFDOBinaryInfo> binaries = ConvertLBRDataToAutoFDO(lbr_data_);
 ...
 private:
  ...
  bool ConvertPerfDataToBranchList() {
    ...
    auto reader_callback = [&](PerfDataReader& reader) {
      reader.AddCallback(
          [&](const BinaryKey& key, ETMBinary& binary) { merger.AddETMBinary(key, binary); });
      reader.AddCallback([&](LBRData& lbr_data) { merger.AddLBRData(lbr_data); });
    };
    if (!ReadPerfDataFiles(reader_callback)) {
      return false;
    }
  ...
  bool ConvertBranchListToAutoFDO() {
    ...
    if (!merger.GetLBRData().samples.empty()) {
      ...
      std::vector<AutoFDOBinaryInfo> binaries = ConvertLBRDataToAutoFDO(lbr_data);
      for (size_t i = 0; i < binaries.size(); ++i) {
        autofdo_writer.AddAutoFDOBinary(lbr_data.binaries[i], binaries[i]);
      }
    }
