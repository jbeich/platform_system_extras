 ...
 private:
  bool ParseOptions(const std::vector<std::string>& args) {
    ...
    if (auto value = options.PullValue("--symdir"); value) {
      ...
      // processing input files. To make symbol dirs always available, create a dummy dso to
      // prevent cleaning from happening.
      dummy_dso_ = Dso::CreateDso(DSO_UNKNOWN_FILE, "dummy");
    }
