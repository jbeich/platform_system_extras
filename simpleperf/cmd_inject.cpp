...
"$ simpleperf inject -i branch_list.data -o autofdo.txt --output autofdo\n"
 ...
 private:
  ...
  void PostProcessInputFile() {
    ...
    if (output_format_ == OutputFormat::AutoFDO) {
      for (auto& p : autofdo_binary_map_) {
        autofdo_writer_.AddAutoFDOBinary(BinaryKey(p.first, 0), p.second);
      }
  ...
  void ProcessInstrRange(const ETMInstrRange& instr_range) {
    ...
    AutoFDOBinaryInfo* binary;
    auto it = autofdo_binary_map_.find(instr_range.dso);
    if (it == autofdo_binary_map_.end()) {
      binary = &autofdo_binary_map_[instr_range.dso];
      binary->first_load_segment_addr = GetFirstLoadSegmentVaddr(instr_range.dso);
    } else {
      binary = &it->second;
    }
