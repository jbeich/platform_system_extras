  ...
  for (; begin != end; ++begin) {
    ...
    if (type == llvm::ELF::STT_FUNC) {
      ...
      if (symbol.is_in_text_section) {
        ...
        // Arm has meaningless labels like $t, $d, $x, $d.1, $x.2, exclude them.
        if (is_arm) {
          size_t test_pos = (symbol.name.find(linker_prefix) == 0) ? linker_prefix.size() : 0;
          test_pos = symbol.name.rfind('$', test_pos);
          if (test_pos != std::string::npos &&
              (test_pos + 2 == symbol.name.size() ||
               (test_pos + 2 < symbol.name.size() && !isalpha(symbol.name[test_pos + 2])))) {
