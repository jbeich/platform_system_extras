...
static BinaryRet OpenObjectFile(const std::string& filename) {
  BinaryRet ret;
  auto binary_or_err = llvm::object::createBinary(llvm::StringRef(filename));
  if (!binary_or_err) {
    PLOG(DEBUG) << "can't open file " << filename << ": " << binary_or_err.getError().message();
    return ret;
  }
  ret.binary = std::move(binary_or_err.get());
  ret.obj = llvm::dyn_cast<llvm::object::ObjectFile>(ret.binary.getBinary());
  if (ret.obj == nullptr) {
    LOG(ERROR) << filename << " is not an object file";
  }
  return ret;
}

static BinaryRet OpenObjectFile(const std::string& filename, uint64_t file_offset, uint32_t file_size) {
  BinaryRet ret;
