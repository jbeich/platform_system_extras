...
template <class ELFT>
bool ReadMinVirtualAddress(const llvm::object::ELFFile<ELFT>* elf, uint64_t* p_vaddr) {
...
bool ReadMinVirtualAddressFromElfFile(const std::string& filename, const BuildId& expected_build_id,
                                      uint64_t* min_vaddr) {
  auto owning_binary = llvm::object::createBinary(llvm::StringRef(filename));
  llvm::object::ObjectFile* obj = GetObjectFile(owning_binary, filename, expected_build_id);
  if (obj == nullptr) {
  ...
  bool result = false;
  if (auto elf = llvm::dyn_cast<llvm::object::ELF32LEObjectFile>(obj)) {
    result = ReadMinVirtualAddress(elf->getELFFile(), min_vaddr);
  } else if (auto elf = llvm::dyn_cast<llvm::object::ELF64LEObjectFile>(obj)) {
    result = ReadMinVirtualAddress(elf->getELFFile(), min_vaddr);
  } else {
    LOG(ERROR) << "unknown elf format in file" << filename;
