...
const char* ReadElfRetToString(ReadElfRet result) {
  switch (result) {
    case ReadElfRet::NO_ERROR:
      return "No error";
    case ReadElfRet::FILE_NOT_EXIST:
      return "File not exist";
    case ReadElfRet::READ_FAILED:
      return "Read failed";
    case ReadElfRet::FILE_MALFORMAT:
      return "File malformat";
    case ReadElfRet::NO_SYMBOL_TABLE:
      return "No symbol table";
    case ReadElfRet::NO_BUILD_ID:
      return "No build id";
    case ReadElfRet::BUILD_ID_MISMATCH:
      return "Build id mismatch";
    case ReadElfRet::SECTION_NOT_FOUND:
...
ReadElfRet IsValidElfFile(int fd) {
  ...
  if (!android::base::ReadFully(fd, buf, 4)) {
    return ReadElfRet::READ_FAILED;
  }
  if (memcmp(buf, elf_magic, 4) != 0) {
    return ReadElfRet::FILE_MALFORMAT;
  }
  return ReadElfRet::NO_ERROR;
}

ReadElfRet IsValidElfPath(const std::string& filename) {
  if (!IsRegularFile(filename)) {
    return ReadElfRet::FILE_NOT_EXIST;
  }
  ...
  ReadElfRet result = IsValidElfFile(fileno(fp));
  fclose(fp);
  return result;
}
...
ReadElfRet GetBuildIdFromNoteFile(const std::string& filename, BuildId* build_id) {
  ...
  if (!GetBuildIdFromNoteSection(content.c_str(), content.size(), build_id)) {
    return ReadElfRet::NO_BUILD_ID;
  }
  return ReadElfRet::NO_ERROR;
}

template <class ELFT>
ReadElfRet GetBuildIdFromELFFile(const llvm::object::ELFObjectFile<ELFT>* elf, BuildId* build_id) {
  ReadElfRet result = ReadElfRet::NO_BUILD_ID;
  for (auto it = elf->section_begin(); it != elf->section_end(); ++it) {
    const llvm::object::ELFSectionRef& section_ref = *it;
    if (section_ref.getType() == llvm::ELF::SHT_NOTE) {
      llvm::StringRef data;
      if (it->getContents(data)) {
        result = ReadElfRet::READ_FAILED;
        continue;
      }
      ...
      result = ReadElfRet::NO_BUILD_ID;
    }
  }
  return result;
}

static ReadElfRet GetBuildIdFromObjectFile(llvm::object::ObjectFile* obj, BuildId* build_id) {
...
static ReadElfRet OpenObjectFile(const std::string& filename, uint64_t file_offset,
...
static ReadElfRet OpenObjectFileFromString(const std::string& s, BinaryWrapper* wrapper) {
...
ReadElfRet GetBuildIdFromElfFile(const std::string& filename, BuildId* build_id) {
  ReadElfRet result = IsValidElfPath(filename);
...
ReadElfRet GetBuildIdFromEmbeddedElfFile(const std::string& filename, uint64_t file_offset,
  ...
  ReadElfRet result = OpenObjectFile(filename, file_offset, file_size, &wrapper);
  if (result != ReadElfRet::NO_ERROR) {
    return result;
  }
...
template <class ELFT>
ReadElfRet ReadSectionFromELFFile(const llvm::object::ELFObjectFile<ELFT>* elf, const std::string& section_name,
                                  std::string* content) {
  ...
  return ReadElfRet::SECTION_NOT_FOUND;
}
...
template <class ELFT>
ReadElfRet ParseSymbolsFromELFFile(const llvm::object::ELFObjectFile<ELFT>* elf,
                                   const std::function<void(const ElfFileSymbol&)>& callback) {
  ...
  ReadElfRet result = ReadSectionFromELFFile(elf, ".gnu_debugdata", &debugdata);
  if (result == ReadElfRet::SECTION_NOT_FOUND) {
    return ReadElfRet::NO_SYMBOL_TABLE;
  } else if (result == ReadElfRet::NO_ERROR) {
...
ReadElfRet MatchBuildId(llvm::object::ObjectFile* obj, const BuildId& expected_build_id) {
  if (expected_build_id.IsEmpty()) {
    return ReadElfRet::NO_ERROR;
  }
  BuildId real_build_id;
  ReadElfRet result = GetBuildIdFromObjectFile(obj, &real_build_id);
  if (result != ReadElfRet::NO_ERROR) {
    return result;
  }
  if (expected_build_id != real_build_id) {
    return ReadElfRet::BUILD_ID_MISMATCH;
  }
  return ReadElfRet::NO_ERROR;
}
...
template <class ELFT>
ReadElfRet ReadMinExecutableVirtualAddress(const llvm::object::ELFFile<ELFT>* elf, uint64_t* p_vaddr) {
