...
void ThreadTree::FixOverlappedMap(std::set<MapEntry*, MapComparator>* map_set, const MapEntry* map) {
  for (auto it = map_set->begin(); it != map_set->end();) {
    ...
    if ((*it)->start_addr + (*it)->len <= map->start_addr) {
      ...
      uint64_t old_end = old->start_addr + old->len;
      uint64_t map_end = map->start_addr + map->len;
      if (old_end > map_end) {
        MapEntry* after =
            AllocateMap(MapEntry(map_end, old_end - map_end, map_end - old->start_addr + old->pgoff,
                                 old->time, old->dso));
