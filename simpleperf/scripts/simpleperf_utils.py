...
class BaseArgumentParser(argparse.ArgumentParser):
    ...
    def add_trace_offcpu_option(self, subparser: Optional[Any] = None):
        ...
        parser.add_argument(
            ...
            help="""Set trace-offcpu mode for report. The mode is only available for profiles
                    recorded with --trace-offcpu option. All possible modes are:
                    on-cpu (only on-cpu samples), off-cpu (only off-cpu samples), on-off-cpu (
                    both on-cpu and off-cpu samples, can be split by event name), mixed-on-off-cpu
                    (on-cpu and off-cpu samples using the same event name).
                """)
