...
"""stackcollapse.py: convert perf.data to Brendan Gregg's "Folded Stacks" format,
  ...
  Example:
    ./app_profiler.py
    ./stackcollapse.py | ~/FlameGraph/flamegraph.pl --color=java --countname=ns > flamegraph.html
"""
  ...
  while True:
    ...
    if not event_filter:
      ...
      if event_defaulted and not event_warning_shown:
        logging.warning('Input has multiple event types. Filtering for the first event type seen: %s' % event_filter)
...
def main():
  ...
  parser.add_argument('--kernel', action='store_true', help='Annotate kernel functions with a _[k]')
