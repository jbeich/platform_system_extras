...
class BinaryCacheBuilder(object):
    ...
    def _need_to_copy(self, source_file, target_file, expected_build_id):
        ...
        if self._read_build_id(target_file) != expected_build_id:
            return True
        return self._get_file_unstripped_level(source_file) > self._get_file_unstripped_level(
            target_file)


    def _get_file_unstripped_level(self, file_path):
        """Return unstripped level of an ELF file."""
