...
class RecordData(object):
    ...
    def add_disassembly(self, filter_lib: Callable[[str], bool], jobs: int):
        ...
        function_results: List[Tuple[Function, Future]] = []
        for lib_id, functions in lib_functions.items():
            ...
            for function in functions:
                future = executor.submit(objdump.disassemble_code, dso_info,
                                         function.start_addr, function.addr_len)
                function_results.append((function, future))
        for function, future in function_results:
            function.disassembly = future.result()
        executor.shutdown()
