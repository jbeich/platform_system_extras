...
class ThreadScope(object):
    ...
    def sort_call_graph_by_function_name(self, get_func_name: Callable[[int], str]):
...
class CallNode(object):
    ...
    def sort_by_function_name(self, get_func_name: Callable[[int], str]):
        if self.children:
...
class RecordData(object):
    ...
    def sort_call_graph_by_function_name(self):
        for event in self.events.values():
            for process in event.processes.values():
                for thread in process.threads.values():
