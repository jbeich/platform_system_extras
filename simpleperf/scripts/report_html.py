...
class ThreadScope(object):
    ...
    def add_callstack(self, event_count, callstack, build_addr_hit_map):
        """ callstack is a list of (lib_id, func_id, addr) pairs.
...
class SourceFile(object):
    ...
    def __init__(self, file_id, abstrace_path):
        self.file_id = file_id
        self.abstrace_path = abstrace_path  # path reported by addr2line
...
class SourceFileSet(object):
    ...
    def load_source_code(self, source_dirs):
        ...
        for source_file in self.path_to_source_files.values():
            real_path = file_searcher.get_real_path(source_file.abstrace_path)
            if real_path:
...
class SourceFileSearcher(object):
        ...
        for source_dir in source_dirs:
            for parent, _, file_names in os.walk(source_dir):
                ...
                for file_name in file_names:
                    if self._is_source_filename(file_name):
    ...
    def _is_source_filename(self, file_name):
        dot_index = file_name.rfind('.')
        if dot_index == -1:
            return False
        suffix = file_name[dot_index + 1:]
        if suffix.find('h') != -1 or suffix.find('c') != -1 or suffix == 'java' or suffix == 'kt':
            return True
        return False

    def get_real_path(self, abstrace_path):
        abstrace_path = abstrace_path.replace('/', os.sep)
        abstract_parent, file_name = os.path.split(abstrace_path)
        abstrace_rparent = abstract_parent[::-1]
        ...
        for real_rparent in real_rparents:
            length = len(os.path.commonprefix((real_rparent, abstrace_rparent)))
...
class RecordData(object):
    ...
    def _gen_function_map(self):
        func_map = {}
        for func_id in sorted(self.functions.id_to_func.keys()):
