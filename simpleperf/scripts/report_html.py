...
class HtmlWriter(object):
    ...
    def open_tag(self, tag, attr_dict={}):
        attr_str = ''
        for key in attr_dict.keys():
            attr_str += ' %s="%s"' % (key, attr_dict[key])
        if attr_str:
            attr_str = ' ' + attr_str
...
class ThreadScope(object):
    ...
    def add_callstack(self, event_count, callstack):
        ...
        for i in range(len(callstack)):
            ...
            function.add_callchain(callstack, i - 1, -1, event_count)
...
class RecordData(object):
    ...
    """RecordData reads perf.data, and generates data used by report.js in json format.
        All generated items are listed as below:
            ...
            9. functionMap: map from functionId to functionName.
...
class ReportGenerator(object):
    ...
    def __init__(self, html_path):
        ...
        self.hw.open_tag('link',
                         {'rel': 'stylesheet',
                          'type': 'text/css',
                          'href': 'https://code.jquery.com/ui/1.12.0/themes/smoothness/jquery-ui.css'}).close_tag()

        self.hw.open_tag(
            'link',
            {'rel': 'stylesheet', 'type': 'text/css',
             'href': 'https://cdn.datatables.net/1.10.16/css/jquery.dataTables.min.css'}).close_tag()
        ...
        self.hw.open_tag(
            'script', {
                'src': 'https://code.jquery.com/ui/1.12.1/jquery-ui.js'}).close_tag()
        self.hw.open_tag(
            'script', {
