...
class SourceLine(object):
  ...
  def get_key(self, for_file=False, for_function=False, for_line=False):
    if for_file:
      return self.file
    if for_function:
...
class SourceFileAnnotator(object):
    ...
    def _add_file_period(self, source, period, used_file_dict):
        key = source.get_key(for_file=True)
        if not used_file_dict.has_key(key):
            used_file_dict[key] = True
    ...
    def _add_line_period(self, source, period, used_line_dict):
        key = source.get_key(for_line=True)
        if not used_line_dict.has_key(key):
    ...
    def _add_function_period(self, source, period, used_function_dict):
        key = source.get_key(for_function=True)
        if not used_function_dict.has_key(key):
