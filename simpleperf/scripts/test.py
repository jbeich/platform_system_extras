...
class TestApiProfiler(TestBase):
    def run_api_test(self, package_name, apk_name, expected_reports, need_android_q_device):
        adb = AdbHelper()
        if need_android_q_device:
            if adb.get_android_version() <= 9:
                log_info('skip this test on Android < Q.')
                return
        ...
        adb.check_run(['install', '-r', '-t', apk_path])
    ...
    def run_cpp_api_test(self, apk_name, need_android_q_device):
        self.run_api_test('simpleperf.demo.cpp_api', apk_name, ['BusyThreadFunc'],
                          need_android_q_device)
    ...
    def test_cpp_api_on_a_debuggable_app_targeting_prev_q(self):
        ...
        # after recording).
        self.run_cpp_api_test('cpp_api-debug_prev_Q.apk', False)

    def test_cpp_api_on_a_debuggable_app_targeting_q(self):
        self.run_cpp_api_test('cpp_api-debug_Q.apk', True)

    def test_cpp_api_on_a_profileable_app_targeting_prev_q(self):
        # a release apk with <profileable android:shell="true" />
        self.run_cpp_api_test('cpp_api-profile_prev_Q.apk', True)

    def test_cpp_api_on_a_profileable_app_targeting_q(self):
        self.run_cpp_api_test('cpp_api-profile_Q.apk', True)

    def run_java_api_test(self, apk_name, need_android_q_device):
    ...
    def test_java_api_on_a_debuggable_app_targeting_prev_q(self):
        ...
        # after recording).
        self.run_java_api_test('java_api-debug_prev_Q.apk', False)

    def test_java_api_on_a_debuggable_app_targeting_q(self):
        self.run_java_api_test('java_api-debug_Q.apk', True)

    def test_java_api_on_a_profileable_app_targeting_prev_q(self):
        # a release apk with <profileable android:shell="true" />
        self.run_java_api_test('java_api-profile_prev_Q.apk', True)

    def test_java_api_on_a_profileable_app_targeting_q(self):
        self.run_java_api_test('java_api-profile_Q.apk', True)
