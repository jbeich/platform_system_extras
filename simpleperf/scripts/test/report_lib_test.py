...
class TestReportLib(TestBase):
    ...
    def test_use_vmlinux(self):
        ...
        kernel_build_id = kernel_build_id = ReadElf(
            TestHelper.ndk_path).get_build_id(
            symfs_dir / 'vmlinux')
    ...
    def _build_id_string_to_bytes(cls, build_id: str) -> bytes:
        res = b''
        for i in range(0, len(build_id), 2):
            res += int(build_id[i:i+2])
        return res
