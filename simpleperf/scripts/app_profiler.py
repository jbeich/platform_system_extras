...
class AppProfiler(object):
    ...
    def start_and_wait_profiling(self):
        ...
        try:
            ...
            if subproc:
                self.kill_simpleperf_with_sigint()
                returncode = 0
        log_debug('run adb cmd: %s [result %s]' % (adb_args, returncode == 0))


    def kill_simpleperf_with_sigint(self):
        has_killed = False
        while True:
            (result, stdout_data) = self.run_in_app_dir(['pidof', 'simpleperf'],
                                                        check_result=False)
            if not result:
                break
            if not has_killed:
                has_killed = True
                pid = int(stdout_data)
                self.run_in_app_dir(['kill', '-2', str(pid)], check_result=False)
            time.sleep(1)
