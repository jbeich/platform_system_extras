    ...
    def _find_app_process(self):
        ...
        if self.android_version >= 7:
            ...
            if self.android_version >= 8 and self.config['app_package_name']:
                ...
                # sometimes we run into the process started in step 1. To solve it, we can
                # double check the process's maps file.
                result, output = self.run_in_app_dir(['cat', '/proc/%d/maps' % pid],
                                                     check_result=False, log_output=False)
                if not result or output.find(self.config['app_package_name']) == -1:
                    return None
            return pid
