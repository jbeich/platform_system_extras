...
from utils import get_script_dir, log_debug, log_info, log_exit, get_target_binary_path
from utils import AdbHelper, ReadElf
...
class ProfilerBase(object):
    ...
    def __init__(self, args):
        ...
        if self.android_version < 7:
            log_exit("""app_profiler.py isn't tested prior Android N, please switch to use
...
def main():
    ...
    target_group.add_argument('-p', '--app',
                              help="""Profile an Android app, given the package name. Like
    ...
    target_group.add_argument('-a', '--activity',
                              help="""Profile the launch time of an activity in an Android app.
                                      The app will be started or restarted to run the activity.
                                      Like `-a .MainActivity`.""")

    target_group.add_argument('-t', '--test',
                              help="""Profile the launch time of an instrumentation test in an
                                      Android app. The app will be started or restarted to run
                                      the instrumentation test. Like
                                      `-t test_class_name`.""")

    target_group.add_argument('-np', '--native_program',
    ...
    record_group.add_argument('-lib', '--native_lib_dir',
    ...
    other_group.add_argument('--ndk_path',
    ...
    def check_args(args):
        ...
        if args.app:
            target_count += 1
        if args.native_program:
            target_count += 1
        if args.cmd:
            target_count += 1
        if target_count == 0:
            log_exit('Please select one profiling target.')
        if target_count > 1:
            log_exit("Cann't profile more than one target at the same time.")
        if args.app:
            if args.activity and args.test:
                log_exit("Can't profile an activity and a test at the same time.")
        elif args.compile_java_code or args.activity or args.test:
            log_exit('--compile_java_code, -a, -t can only be used when profiling an Android app.')
        if args.native_lib_dir:
            if not os.path.isdir(args.native_lib_dir):
                log_exit("native_lib_dir %s isn't a directory." % args.native_lib_dir)
        if args.ndk_path:
            if not os.path.isdir(args.ndk_path):
                log_exit("ndk_path %s isn't a directory." % args.ndk_path)
