...
class AppProfiler(object):
    ...
    def check_config(self, config):
        config_names = ['app_package_name', 'native_program', 'native_lib_dir', 'apk_file_path',
        ...
        if config['recompile_app']:
            if not config['launch_activity'] and not config['launch_inst_test']:
                # If recompile app, we must restart the app to take effect.
    ...
    def _find_app_process(self):
        ...
        for line in output.split('\n'):
            strs = line.split()
            if len(strs) > 2 and strs[-1].find(self.app_program) != -1:
                return int(strs[1])
        return None
    ...
    def start_and_wait_profiling(self):
        ...
        try:
            ...
            if is_windows():
                # create subprocess in new process group to ignore ctrl-c.
                subproc = subprocess.Popen(adb_args, creationflags=0x00000200)
            else:
                subproc = subprocess.Popen(adb_args, preexec_fn=
                                           lambda: os.setpgrp())
