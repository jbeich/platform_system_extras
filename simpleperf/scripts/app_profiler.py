...
class AppProfiler(object):
    ...
    def check_config(self, config):
        config_names = ['app_package_name', 'native_program', 'cmd', 'native_lib_dir',
                        ...
                        'record_options', 'perf_data_path', 'startup_profiling', 'app_arch']
        ...
        if config['startup_profiling']:
            if not config['app_package_name']:
                log_exit('-p needs to be set for startup profiling.')
            if not config['launch_activity']:
                log_exit('-a needs to be set for startup profiling.')
            if not config['app_arch']:
                log_exit('--arch needs to be set for startup profiling.')
    ...
    def prepare_profiling(self):
        ...
        if not self.config['startup_profiling']:
    ...
    def _restart_app(self):
        ...
        if self.config['startup_profiling']:
            self._download_simpleperf()
...
binary_cache directory. It can be used to annotate source code. This option skips it.""")
    parser.add_argument('--startup_profiling', action='store_true', help=
"""Profile an activity's startup time. It should be used with -p, -a, and --arch options.
...
But with --startup_profiling option, we change the order as below: kill the app if it is
...
"""Force adb to run in non root mode.""")
    ...
    config['startup_profiling'] = args.startup_profiling
