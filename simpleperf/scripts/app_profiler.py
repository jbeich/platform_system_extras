...
class AppProfiler(object):
    """Used to manage the process of profiling an android app. There are three steps:
       1. Prepare profiling.
       2. Profile the app.
    ...
    def __init__(self, config):
        ...
        if len(native_lib_dir) > 0 and not os.path.isdir(native_lib_dir):
            log_fatal('[native_lib_dir] "%s" is not a dir' % native_lib_dir)
        apk_file_path = config.get('apk_file_path')
        if len(apk_file_path) > 0 and not os.path.isfile(apk_file_path):
    ...
    def _get_device_environment(self):
        ...
        if build_version is not None and len(build_version) > 0:
    ...
    def _recompile_app(self):
        ...
        if self.android_version == 0:
            ...
            elif len(self.config['apk_file_path']) == 0:
                log_warning("Can't fully compile the app without reinstalling it on android version < N.")
            else:
    ...
    def _download_native_libs(self):
        if len(self.config['native_lib_dir']) == 0:
            return
        filename_dict = dict()
        for root, _, files in os.walk(self.config['native_lib_dir']):
            for file in files:
                ...
                log_info('app_arch = %s' % self.app_arch)
                if self._compare_native_lib(path, old_path) < 0:
    ...
    def _compare_native_lib(self, new_path, old_path):
        """ Return -1 if new_path is more likely to be used on device. """
        if old_path is None:
            return -1
