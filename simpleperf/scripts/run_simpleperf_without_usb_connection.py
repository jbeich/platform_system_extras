...
def start_recording(args):
    ...
    if args.size_limit:
        shell_cmd += ' --size-limit ' + str(args.size_limit)
...
def main():
    ...
    start_parser.add_argument('--size_limit', type=float,
                              help="""Stop profiling when recording data reaches [size_limit]
                                      M bytes. Like `--size_limit 100`.""")
