...
"""unwinding_result_reporter.py: report dwarf unwinding results generated by debug-unwind cmd.
    ...
    3. For samples don't have complete callchains, report their regs, stack data,
       unwinding failure info, callchains. Samples having the same failure reason
       in the same function are only reported once.

    It can be used to:
    1. detect unwinding failures.
    2. help debug unwinding failures.
    3. show performance and memory consumption change caused by simpleperf/unwinder changes.
"""
