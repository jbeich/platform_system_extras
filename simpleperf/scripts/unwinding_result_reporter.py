...
class ProcessMaps(object):
    ...
    def fork_pid(self, pid, ppid):
        ...
        for entry in entry_list:
            new_list.append(copy.copy(entry))
        self.process_maps[pid] = new_list
    ...
    def show(self):
        for pid in sorted(self.process_maps.keys()):
            print('  pid %d' % pid)
...
class SampleResult(object):
    ...
    def show(self):
        ...
        elif self.stop_reason == 'EXECUTE_DWARF_INSTRUCTION_FAILED':
            print('  execute_result = %d' % self.stop_info)
...
class FunctionResult(object):
    ...
    def show(self):
        for stop_reason in sorted(self.sample_results.keys()):
...
class FileResult(object):
    ...
    def show(self):
        for function_name in sorted(self.function_results.keys()):
            print('  function %s' % function_name)
            self.function_results[function_name].show()
            print('\n')


class UnwindingResultReport(object):
    ...
    def should_omit(self, sample_result):
        ...
        if sample_result.callchain[-1].filename in ['/dev/ashmem/dalvik-jit-code-cache',
                                                    '//anon']:
            return True
        # 2. No need to report if we can reach __libc_init or __start_thread in
        # libc.so.
...
def build_unwinding_result_report(record_file):
    ...
    if 'record callchain' not in stdoutdata or 'record unwinding_result' not in stdoutdata:
        log_exit("Can't parse unwinding result. Because %s is not recorded using '--log debug'."
                 % record_file)
    unwinding_report = UnwindingResultReport()
        ...
        if lines[i].startswith('record mmap:'):
            ...
            while i < len(lines) and not lines[i].startswith('record'):
                if lines[i].startswith('  pid'):
                    ...
                    if m:
                        pid = int(m.group(1))
                        map_entry.start = int(m.group(2), 16)
                        map_entry.end = map_entry.start + int(m.group(3), 16)
                elif lines[i].startswith('  pgoff'):
                    pos = lines[i].find('filename') + len('filename')
                    map_entry.filename = lines[i][pos:].strip()
                i += 1
            process_maps.add(pid, map_entry)
            ...
            n = len(ips)
            if (n == 0 or len(sps) != n or len(function_names) != n or len(filenames) != n or
                    len(vaddr_in_files) != n or len(map_start_addrs) != n or
                    len(map_end_addrs) != n):
                log_fatal('unexpected dump output %d' % i)
            callchain = []
            for j in range(n):
