...
class Thread:
  ...
  def intern_frame(self, frame_str: str) -> int:
    ...
    column = None
    category = 0
    # Heuristic: all kernel code contains "kallsyms" as the library name.
  ...
  def add_sample(self, comm: str, stack: List[str], time_ms: float):
    ...
    responsiveness = 0
    self.samples.append((stack_id, time_ms, responsiveness))
...
def gecko_profile(
  ...
  while True:
    ...
    thread.add_sample(comm=sample.thread_comm, stack=stack, time_ms=sample_time_ms)
