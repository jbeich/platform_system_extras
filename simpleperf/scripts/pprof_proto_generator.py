...
class PprofProfileGenerator(object):
    ...
    def __init__(self, config):
        ...
        if config.get('pid_filters'):
            self.pid_filter = set([int(x) for x in config['pid_filters']])
        else:
            self.pid_filter = None
        if config.get('tid_filters'):
            self.tid_filter = set([int(x) for x in config['tid_filters']])
        else:
    ...
    def add_sample(self, sample):
        key = sample.get_key()
        exist_sample = self.sample_map.get(key)
        if exist_sample:
            exist_sample.add_values(sample.values)
        else:
            self.sample_list.append(sample)
            self.sample_map[key] = sample
    ...
    def gen_profile_sample(self, sample):
        ...
        values = [0 for i in range(sample_type_count)]
