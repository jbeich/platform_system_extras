...
class MonitorCommand : public Command {
 public:
  MonitorCommand()
      : Command(
            "monitor", "monitor events and return a textual representation on stdout",
            // clang-format off
"Usage: simpleperf monitor [options] [--] [command [command-args]]\n"
...
"\n"
 ...
 private:
  ...
  bool DoMonitoring(Workload* workload);
  bool SetEventSelectionFlags();
  bool DumpProcessMaps(pid_t pid, const std::unordered_set<pid_t>& tids);
  bool ProcessRecord(Record* record);
  void ProcessSampleRecord(const SampleRecord& sr);
  SymbolInfo GetSymbolInfo(uint32_t pid, uint32_t tid, uint64_t ip, bool
                           in_kernel);
...
bool MonitorCommand::Run(const std::vector<std::string>& args) {
  ...
  return DoMonitoring(workload.get());
}

bool MonitorCommand::PrepareMonitoring() {
  // 1. Prepare in other modules.
  // This requires a temporary file. Required?
  //PrepareVdsoFile();
...
bool MonitorCommand::DoMonitoring(Workload* workload) {
...
bool MonitorCommand::DumpProcessMaps(pid_t pid, const std::unordered_set<pid_t>& tids) {
  // Dump mmap records.
...
void MonitorCommand::ProcessSampleRecord(const SampleRecord& sr) {
  ...
  //TODO( Dump tracepoint fields.)
}

SymbolInfo MonitorCommand::GetSymbolInfo(uint32_t pid, uint32_t tid, uint64_t ip,
                                             bool in_kernel) {
...
void RegisterMonitorCommand() {
  RegisterCommand("monitor",
                  [] { return std::unique_ptr<Command>(new MonitorCommand()); });
}
