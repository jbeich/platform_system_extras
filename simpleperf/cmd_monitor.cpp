...
class MonitorCommand : public Command {
 public:
  MonitorCommand()
      : Command(
            "monitor",
            "monitor events and print their textual representations on stdout",
            // clang-format off
...
"\n"
 ...
 private:
  ...
  uint64_t
      last_record_timestamp_;  // used to insert Mmap2Records for JIT debug info
...
bool MonitorCommand::PrepareMonitoring() {
  ...
  if (system_wide_collection_) {
    ...
    LOG(ERROR)
        << "No threads to monitor. Try `simpleperf help monitor` for help";
    return false;
  }
  ...
  if (!event_selection_set_.MmapEventFiles(
          mmap_page_range_.first, mmap_page_range_.second, aux_buffer_size_,
          record_buffer_size, false /* allow_cutting_samples */,
          exclude_perf_)) {
    return false;
  }
  auto callback =
      std::bind(&MonitorCommand::ProcessRecord, this, std::placeholders::_1);
  ...
  // 4. Load kallsyms
  if ((dwarf_callchain_sampling_ || fp_callchain_sampling_)
...
bool MonitorCommand::DoMonitoring() {
  ...
  LOG(INFO) << "Processed samples: " << sample_record_count_;
  return true;
}
...
bool MonitorCommand::ParseOptions(const std::vector<std::string>& args) {
  ...
  if (!PreprocessOptions(args, GetRecordCmdOptionFormats(), &options,
  ...
  if (auto value = options.PullValue("--tracepoint-events"); value) {
    if (!EventTypeManager::Instance().ReadTracepointsFromFile(
            *value->str_value)) {
      return false;
    }
  }
  ...
  for (const auto& pair : ordered_options) {
    ...
    if (name == "-c" || name == "-f") {
      ...
      std::vector<std::string> strs =
          android::base::Split(*value.str_value, ",");
      if (strs[0] == "fp") {
  ...
  if (fp_callchain_sampling_) {
    if (GetBuildArch() == ARCH_ARM) {
      LOG(WARNING)
          << "`--callgraph fp` option doesn't work well on arm architecture, "
...
bool MonitorCommand::AdjustPerfEventLimit() {
  ...
  size_t cur_percent;
  if (GetCpuTimeMaxPercent(&cur_percent) &&
      cur_percent != cpu_time_max_percent_ &&
  ...
  if (GetAndroidVersion() >= kAndroidVersionP + 1 && set_prop) {
    return SetPerfEventLimits(std::max(max_sample_freq_, cur_max_freq),
                              cpu_time_max_percent_,
...
bool MonitorCommand::DumpKernelMaps() {
  KernelMmap kernel_mmap;
  std::vector<KernelMmap> module_mmaps;
  GetKernelAndModuleMmaps(&kernel_mmap, &module_mmaps);

  MmapRecord mmap_record(*dumping_attr_id_.attr, true, UINT_MAX, 0,
                         kernel_mmap.start_addr, kernel_mmap.len, 0,
                         kernel_mmap.filepath, dumping_attr_id_.ids[0]);
  if (!ProcessRecord(&mmap_record)) {
    return false;
  }
  for (auto& module_mmap : module_mmaps) {
    MmapRecord mmap_record(*dumping_attr_id_.attr, true, UINT_MAX, 0,
                           module_mmap.start_addr, module_mmap.len, 0,
                           module_mmap.filepath, dumping_attr_id_.ids[0]);
    if (!ProcessRecord(&mmap_record)) {
      return false;
    }
  }
...
bool MonitorCommand::DumpProcessMaps(pid_t pid,
                                     const std::unordered_set<pid_t>& tids) {
  // Dump mmap records.
  ...
  for (const auto& map : thread_mmaps) {
    ...
    Mmap2Record record(attr, false, pid, pid, map.start_addr, map.len,
                       map.pgoff, map.prot, map.name, event_id,
  ...
  if (!process_name.empty()) {
    CommRecord record(attr, pid, pid, process_name, event_id,
                      last_record_timestamp_);
    if (!ProcessRecord(&record)) {
      return false;
    }
  ...
  for (const auto& tid : tids) {
    ...
    if (tid != pid && GetThreadName(tid, &name)) {
      // If a thread name matches the suffix of its process name, probably the
      // thread name is stripped by TASK_COMM_LEN.
      if (android::base::EndsWith(process_name, name)) {
        name = process_name;
      }
      CommRecord comm_record(attr, pid, tid, name, event_id,
                             last_record_timestamp_);
...
bool MonitorCommand::ProcessRecord(Record* record) {
  UpdateRecord(record);
  last_record_timestamp_ =
      std::max(last_record_timestamp_, record->Timestamp());
...
void MonitorCommand::DumpSampleRecord(const SampleRecord& sr) {
  ...
  if (sr.sample_type & (PERF_SAMPLE_ID | PERF_SAMPLE_IDENTIFIER)) {
    StringAppendF(&output, " id=%" PRId64, sr.id_data.id);
  }
  if (sr.sample_type & PERF_SAMPLE_IP) {
    StringAppendF(&output, " ip=%p", reinterpret_cast<void*>(sr.ip_data.ip));
  }
  if (sr.sample_type & PERF_SAMPLE_TID) {
    StringAppendF(&output, " pid=%u tid=%u", sr.tid_data.pid, sr.tid_data.tid);
  }
  if (sr.sample_type & PERF_SAMPLE_CPU) {
    StringAppendF(&output, " cpu=%u res=%u", sr.cpu_data.cpu, sr.cpu_data.res);
  }
  printf("%s\n", output.c_str());
}
  ...
  if (sr.sample_type & PERF_SAMPLE_CALLCHAIN) {
    for (size_t i = 0; i < sr.callchain_data.ip_nr; ++i) {
      ...
      SymbolInfo s = GetSymbolInfo(sr.tid_data.pid, sr.tid_data.tid,
                                   sr.callchain_data.ips[i], in_kernel);
...
SymbolInfo MonitorCommand::GetSymbolInfo(uint32_t pid, uint32_t tid,
                                         uint64_t ip, bool in_kernel) {
  ThreadEntry* thread = thread_tree_.FindThreadOrNew(pid, tid);
  const MapEntry* map = thread_tree_.FindMap(thread, ip, in_kernel);
  SymbolInfo info;
  info.symbol =
      thread_tree_.FindSymbol(map, ip, &info.vaddr_in_file, &info.dso);
  return info;
}
...
bool MonitorCommand::DumpMapsForRecord(Record* record) {
  if (record->type() == PERF_RECORD_SAMPLE) {
    ...
    if (dumped_processes_.find(pid) == dumped_processes_.end()) {
      ...
      if (!tids.empty() &&
          !DumpProcessMaps(
              pid, std::unordered_set<pid_t>(tids.begin(), tids.end()))) {
        return false;
      }
...
bool MonitorCommand::UnwindRecord(SampleRecord& r) {
  if ((r.sample_type & PERF_SAMPLE_CALLCHAIN) &&
      ...
      (r.sample_type & PERF_SAMPLE_STACK_USER) && (r.GetValidStackSize() > 0)) {
    ThreadEntry* thread =
        thread_tree_.FindThreadOrNew(r.tid_data.pid, r.tid_data.tid);
    RegSet regs(r.regs_user_data.abi, r.regs_user_data.reg_mask,
                r.regs_user_data.regs);
    std::vector<uint64_t> ips;
    std::vector<uint64_t> sps;
    if (!offline_unwinder_->UnwindCallChain(
            *thread, regs, r.stack_user_data.data, r.GetValidStackSize(), &ips,
            &sps)) {
      return false;
    }
...
void RegisterMonitorCommand() {
  RegisterCommand(
      "monitor", [] { return std::unique_ptr<Command>(new MonitorCommand()); });
}
