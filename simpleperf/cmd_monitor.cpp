...
class MonitorCommand : public Command {
 public:
  MonitorCommand()
      : Command(
            "monitor", "monitor events and print their textual representations on stdout",
            // clang-format off
...
bool MonitorCommand::PrepareMonitoring() {
  ...
  // 4. Load kallsyms
  if ((dwarf_callchain_sampling_ || fp_callchain_sampling_) && CheckKernelSymbolAddresses()) {
