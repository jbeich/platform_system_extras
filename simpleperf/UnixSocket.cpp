...
bool UnixSocketConnection::WriteData() {
  if (write_data_size_ == 0u) {
    if (!GetDataFromSendBuffer()) {
      return false;
    }
    if (write_data_size_ == 0u) {
      return true;
  ...
  ssize_t result = TEMP_FAILURE_RETRY(
      send(fd_, write_data_, write_data_size_, MSG_NOSIGNAL));
  if (result >= 0) {
    write_data_ += result;
    write_data_size_ -= result;
    std::lock_guard<std::mutex> lock(send_buffer_and_write_event_mtx_);
...
bool UnixSocketConnection::GetDataFromSendBuffer() {
  {
    std::lock_guard<std::mutex> lock(send_buffer_and_write_event_mtx_);
    write_data_size_ = send_buffer_.PeekData(&write_data_);
    if (write_data_size_ != 0u) {
      return true;
    }
    if (!no_more_message_) {
