...
std::unique_ptr<UnixSocketConnection> UnixSocketConnection::Connect(
  ...
  if (server_path.size() + 1 > sun_path_len) {
    LOG(DEBUG) << "can't create unix domain socket as server path is too long: "
               << server_path;
    return nullptr;
  }
  char* p = serv_addr.sun_path;
...
bool UnixSocketConnection::PrepareForIO(
  ...
  if (nullptr == loop.AddReadEvent(fd_, [&]() { return ReadMessage(); })) {
    return false;
  }
  std::lock_guard<std::mutex> lock(send_buffer_and_write_event_mtx_);
  write_event_ = loop.AddWriteEvent(fd_, [&]() { return WriteMessage(); });
...
bool UnixSocketConnection::WriteMessage() {
  ...
  ssize_t result =
      TEMP_FAILURE_RETRY(write(fd_, write_message_.data() + write_message_head_,
...
bool UnixSocketConnection::ReadMessage() {
  ...
  if (read_message_head_ < sizeof(uint32_t)) {
    ...
    ssize_t result =
        TEMP_FAILURE_RETRY(read(fd_, read_message_.data() + read_message_head_,
  ...
  if (read_message_head_ >= sizeof(uint32_t)) {
    ...
    if (read_message_head_ == len) {
      if (!read_callback_(
              *reinterpret_cast<UnixSocketMessage*>(read_message_.data()))) {
        return false;
      }
