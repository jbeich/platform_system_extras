...
std::unique_ptr<UnixSocketServer> UnixSocketServer::Create(const std::string& server_path, bool is_abstract) {
...
bool UnixSocketConnection::PrepareForIO(IOEventLoop& loop,
  ...
  if (nullptr == loop.AddReadEvent(fd_, [&]() {
    return ReadMessage();
  })) {
    return false;
  }
