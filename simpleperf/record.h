...
// ensure the recorded in order. Each time we pop a record A, it is the earliest record
...
// record, which is based on the assumption that if we have received an record for time t,
// we are not likely to receive a record for time (t - min_time_diff).
class RecordCache {
 public:
