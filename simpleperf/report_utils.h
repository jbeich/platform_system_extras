...
class CallChainReportBuilder {
 public:
  ...
  // If ture, remove interpreter frames both before and after a Java frame.
  // Default is true.
  void SetRemoveArtFrame(bool enable) { remove_art_frame_ = enable; }
  // If ture, convert a JIT method into its corresponding interpreted Java method. So they can be
  // merged in reports like flamegraph. It is effective only when remove_art_frame = true.
  // Default is true.
