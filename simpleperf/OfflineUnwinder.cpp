...
bool OfflineUnwinder::UnwindCallChain(const ThreadEntry& thread, const RegSet& regs,
  ...
  if (cached_map.version < thread.maps->version) {
    ...
    cached_map.map.reset(BacktraceMap::Create(thread.pid, bt_maps));
    if (!cached_map.map) {
      return false;
    }
  ...
  cached_map.map->SetStack(stack_info);
  std::vector<backtrace_frame_data_t> frames;
  BacktraceUnwindError error;
  if (Backtrace::Unwind(unwind_regs.get(), cached_map.map.get(), &frames, 0u, nullptr, &error)) {
