...
static_assert(simpleperf::PROT_JIT_SYMFILE_MAP == PROT_JIT_SYMFILE_MAP, "");
static_assert(simpleperf::PROT_JIT_SYMFILE_MAP == unwindstack::MAPS_FLAGS_JIT_SYMFILE_MAP, "");
...
bool OfflineUnwinder::UnwindCallChain(const ThreadEntry& thread, const RegSet& regs,
  ...
  for (auto& map : *thread.maps) {
    ...
    bt_map.flags = PROT_READ | PROT_EXEC | map->prot;
  }
