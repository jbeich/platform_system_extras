...
// to UnixSocketMessage* directly. To access UnixSocketMessage.len
// without causing alignment problems, we have to make sure all messages
// are stored at 4-bytes aligned addresses. Namely, each message will
// be padded to 4-bytes aligned size.
static constexpr uint32_t UnixSocketMessageAlignment = 4u;
...
class UnixSocketConnection {
 ...
 private:
  ...
  // write_data_ is the data to be written, write_data_size_ is its size.
  const char* write_data_;
