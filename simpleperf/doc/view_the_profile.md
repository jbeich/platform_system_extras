...
After using `simpleperf record` or `app_profiler.py`, we get a profile data. The profile data
contains a list of samples. Each samples has a timestamp, a thread id, a callstack, events (like
cpu-cycles or cpu-clock) used in this sample, etc. We have many choices for viewing the profile. We
can show samples in chronological order, or show aggregated flamegraph. We can show report in text
formats, or in some interactive UIs.
...
Android Studio Profiler support recording and reporting profiles of app processes. It supports
...
This UI is fairly rough; we recommend using the Continuous PProf UI or Firefox Profiler instead.
...
PProf has an interactive command line, allowing drilldown, pivoting and filtering.

PProf's command line allows pivoting on specific threads, which the Continuous PProf UI does not.

The below session demonstrates filtering to stack frames containing processBitmap.

```
$ ~/go/bin/pprof pprof.profile
...
See also [report command's README](executable_commands_reference.md#The-report-command) and `report.py -h`.
...
See [simpleperf_report_lib.py's README](scripts_reference.md#simpleperf_report_libpy) for more details.
