...
- [Executable commands reference](#executable-commands-reference)
    ...
    - [The record command](#the-record-command)
        ...
        - [Record call graph](#record-call-graph-in-record-cmd)
    ...
    - [The report command](#the-report-command)
        ...
        - [Report call graph](#report-call-graph-in-report-cmd)
...
events because they have different hardwares and kernels.
...
information like the time generating the sample, events happened since last sample, the ip address
of a thread, the call chain of a thread.
...
By default, simpleperf stores profiling data in perf.data in the current directory. But it can be
changed using -o option.
...
<a name="record-call-graph-in-record-cmd"></a>
#### Record call graph
...
based call graph. Recording dwarf based call graph needs support of debug information in native
binaries. While recording stack frame based call graphs needs support of stack frame register.
...
using -O0 -g -fno-omit-frame-pointer options. Because the kernel can't unwind user stack containing
both arm/thumb code. So please consider using dwarf based call graph on arm architecture, or
profiling in arm64 environment.

[Here](#how-to-improve-call-graph-result) are some suggestions to improve call graph result.
...
on a CPU, or staying in CPUs' ready queue, or waiting for something like I/O events.

To support this, the record command uses --trace-offcpu option to trace both on CPU time and off
CPU time. When --trace-offcpu is used, simpleperf generates a sample when a running thread is
scheduled out, so we know the callstack of a thread when it is scheduled out. And when reporting
a perf.data generated with --trace-offcpu, we use timestamp to the next sample (instead of event
counts from the previous sample) as the weight of current sample. As a result, we can get a
callgraph based on timestamp, including both on CPU time and off CPU time.
...
But if we add --trace-offcpu option, the result changes.
...
into sample entries, where to find profiling data and binaries.
...
But it can be changed using -i option.
...
<a name="report-call-graph-in-report-cmd"></a>
#### Report call graph

To report a call graph, please make sure the profiling data is recorded with call graph
as [here](#record-call-graph-in-record-cmd).
...
2. Dwarf based call graph needs to have unstripped native binaries on device to generate good call
   graph. It can be supported in two ways:
...
3. Try on arm64 if possible. Because arm64 has better support for stack frame register, so the
   stack frame based call graph is better on arm64 than on arm.
```
