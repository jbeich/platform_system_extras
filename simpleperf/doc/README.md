...
We suggest profiling on an Android >= N device for reason [here](#why-we-suggest-profiling-on-android-n-devices).
...
We can use [report_html.py](#report_html-py) to show profiling result in html interface.
...
To show flame graph, we need to first record call graphs. Flame graph is shown by report_html.py
in the "Flamegraph" tab.
We can also use [inferno](#inferno) to show flame graph directly.
...
We can also build flame graph based on scripts in https://github.com/brendangregg/FlameGraph.
...
you use on device are stripped, you can still result in a lot of unknown symbols in the report. A
...
in binary_cache/. And you can use it when reporting.
