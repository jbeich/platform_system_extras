...
64K stack data is dumped by the kernel. By unwinding the stack based on dwarf information, we get a
callchain. But the thread stack can be much longer than 64K. In that case, we can't unwind to the
thread start point.

To alleviate the problem, simpleperf joins callchains in post recording time. If two callchains of
a thread have an entry containing the same ip and sp address, then simpleperf tries to join them to
make the callchains longer. In that case, the longer we run, the more samples we get, we are more
likely to get complete callchains. But it's still not guaranteed to get complete call graphs.
