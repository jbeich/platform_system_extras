...
to the Java virtual machine, so the app runs as a release app. wrap.sh can be done by adding below
scripts in app/build.gradle.
```
android {
    buildTypes {
        release {
...
On Android >= P, simpleperf supports profiling Java code, no matter it is executed by
