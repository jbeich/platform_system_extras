...
Here are some tips for Android platform developers, who build and flush system images on rooted
devices:
1. After running `adb root`, simpleperf can be used to profile any process or system wide.
2. It is recommended to use the latest simpleperf available in AOSP master, instead of the one
shipped on device. Scripts are in $AOSP_MASTER/system/extras/simpleperf/scripts, binaries are
in $AOSP_MASTER/system/extras/simpleperf/scripts/bin/android.
3. It is recommended to use app_profiler.py for recording, and report_html.py for reporting. Below
is an example.
...
# Report source code and disassembly. Disassembly all binaries is slow, so it's better to add
# --binary_filter option to only disassembly selected binaries. $AOSP_MASTER should be replaced
# by the path of the Android platform code.
$ python report_html.py --add_source_code --source_dirs $AOSP_MASTER --add_disassembly \
...
# Stat the process of an Android application, which should be debuggable on non-rooted devices.
...
# Read symbols for system libraries built locally. Note that it is not needed since Android O,
...
# Record a native process given pid.
...
# Adding disassembly for all binaries can cost a lot of time. So we can only add disassembly
