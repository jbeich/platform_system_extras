...
misses a few patches. Feel free to ask if you need ETM function on 5.4 kernel.

Besides ETM driver, we also need to add Coresight devices in device tree. An example is in
`<linux_kernel>/arch/arm64/boot/dts/arm/juno-base.dtsi`. There should be a path flowing ETM data
from ETM device through funnels, ETF and replicators, all the way to ETR, which writes ETM data
to system memory.
...
enable non-Secure non-invasive debug on ARM CPU.
