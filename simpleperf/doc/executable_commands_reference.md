...
threads, blocked in IO or waiting for some events). To support this, simpleperf added
--trace-offcpu option in record command. When --trace-offcpu is used, simpleperf does below things:

1) Only cpu-clock/task-clock event is allowed to be used with --trace-offcpu. And simpleperf
   generates on-cpu samples for cpu-clock event.
...
2) off-cpu (sched_switch) samples generated for sched:sched_switch event. The period value is
   calculated as (Timestamp of the next switch on record - Timestamp of the current sample) by
   simpleperf. So the period value in each sample means how many nanoseconds are spent off cpu
...
* calculate the period of an off-cpu sample as (Timestamp of the next switch on record or
* sample - Timestamp of the current sample).
