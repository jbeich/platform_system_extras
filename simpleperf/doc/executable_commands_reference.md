...
event is only monitored for part of the total time. As a result, the number of events shown are
smaller than actually happened. Below is an example.
...
In the example above, we try to monitor 7 events. Each event is only monitored part of time. The
number of cache-references is smaller than the number of cache-references:u (cache-references only
for userspace) and cache-references:k. And there is no time to monitor instructions. At the end,
simpleperf gives a warning. Because it feels counter multiplexing happens.

To avoid hardware counter multiplexing, we can use `simpleperf stat --print-hw-counter` to show
available counters on each cpu. Then don't to monitor more hardware events than counters available.
