...
class RegExMatchImpl : public RegExMatch {
 public:
  RegExMatchImpl(const std::string& s, const std::regex& re) : match_it(s.begin(), s.end(), re) {}

  bool IsValid() const override { return match_it != std::sregex_iterator(); }

  std::string GetField(size_t index) const override {
    if (!IsValid() || index >= match_it->size()) {
      return "";
    }
    return match_it->str(index);
  }

  void MoveToNextMatch() override { ++match_it; }

 private:
  std::sregex_iterator match_it;
};
...
class RegExImpl : public RegEx {
 public:
  ...
  std::unique_ptr<RegExMatch> SearchAll(const std::string& s) const override {
