...
class SampleUnwinder : public RecordFileProcesser {
 public:
...
class TestFileGenerator : public RecordFileProcesser {
 public:
 ...
 protected:
  ...
  bool WriteFeatureSections() {
    ...
    for (const auto& p : reader_->FeatureSectionDescriptors()) {
      ...
      } else if (feat_type == PerfFileFormat::FEAT_DEBUG_UNWIND_FILE ||
                 feat_type == PerfFileFormat::FEAT_BUILD_ID) {
        continue;
...
"--generate-test-file <file_path>   Generate a test recording file with one one sample.\n"
"-i <file>                 Input recording file\n"
"-o <file>                 Output file. Default is stdout.\n"
"--keep-binaries-in-test-file  binary1,binary2...   Binaries to keep in test file.\n"
...
"$ simpleperf debug-unwind -i perf.data --generate-test-file test.data --sample-time 626970493946976\n"
"     --keep-binaries-in-test-file perf.data_jit_app_cache:255984-259968\n"
