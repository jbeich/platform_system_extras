...
We suggest profiling on an Android >= N device. For detail reasons, see [here](#why-we-suggest-profiling-on-android-n-devices).
...
2. To profile Java code, we need the jvm running in oat mode, which is only
