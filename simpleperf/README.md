...
We highly suggest profiling on a real device, because on a device we can measure
the real running situation and have the hardware support for performance
monitoring. But when an Android device is not available, or you need to be root,
you can also profile on the Android emulator. On the Android emulator, the
hardware events are not available, but we can use software events instead.
