...
class StatCommand : public Command {
 ...
 private:
  ...
  size_t OpenEventFilesForProcess(pid_t pid);
  size_t OpenEventFilesForCpus(const std::vector<int>& cpus);
  bool StartCounting();
  bool ReadEventFiles();
...
bool StatCommand::RunCommand(std::vector<std::string>& args) {
  ...
  if (measured_events.size() == 0) {
    ...
    if (measured_events.size() == 0) {
      fprintf(stderr, "No measured events are supported.\n");
      return false;
    }
  }
  ...
  if (all_cpus_option == true) {
    if (OpenEventFilesForCpus(GetOnlineCpus()) == 0) {
      fprintf(stderr, "Open event files for cpus failed\n");
      return false;
    }
  } else {
    if (OpenEventFilesForProcess(work_load->GetWorkProcess()) == 0) {
      fprintf(stderr, "Open event files for process failed\n");
      return false;
    }
  }
...
size_t StatCommand::OpenEventFilesForProcess(pid_t pid) {
  size_t open_event_file_count = 0;
  ...
  for (auto& elem : measured_events) {
    ...
    if (event_fd != nullptr) {
      ...
      ++open_event_file_count;
    }
  }
  return open_event_file_count;
}

size_t StatCommand::OpenEventFilesForCpus(const std::vector<int>& cpu_list) {
  size_t open_event_file_count = 0;
...
void StatCommand::ShowCounters(int64_t elapsed_ns) {
  ...
  for (auto& elem : measured_events) {
    ...
    bool scaled = false;
    int64_t scaled_count = counter.count;
    ...
    if (counter.time_running < counter.time_enabled) {
      ...
      if (counter.time_running == 0) {
        ...
        scaled_count = static_cast<int64_t>(static_cast<double>(counter.count) *
                                counter.time_enabled / counter.time_running);
      }
    }

    if (verbose_option == false) {
      printf("%30s%s  %s\n", FormatCount(scaled_count).c_str(),
                             scaled ? "(scaled)" : "        ",
                             event->Name());
    } else {
