...
static bool UseDevfreqCounters() {
  if (!IsRoot()) {
  ...
  for (auto& name : GetSubDirs(devfreq_dir)) {
    ...
    if (IsRegularFile(governor_path)) {
      ...
      if (android::base::Trim(governor) == "mem_latency") {
        if (!android::base::WriteStringToFile("performance", governor_path)) {
          PLOG(ERROR) << "failed to write " << governor_path;
          return false;
        }
        LOG(DEBUG) << "governor of " << governor_path << " is changed from "
                   << android::base::Trim(governor) << " to performance";
      }
    }
  }
  return true;
...
"--use-devfreq-counters    Devfreq may use performance counters to monitor memory\n"
"                          latency, making less hardware counters available to users.\n"
"                          This option sets devfreq to use performance governor instead\n"
"                          of memory-latency governor.\n"
