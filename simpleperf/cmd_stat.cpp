...
struct CounterSummary {
  ...
  bool IsMonitoredAtTheSameTime(const CounterSummary& other) const {
    ...
    // group or both have scale ~= 1.0.
    if (group_id == other.group_id) {
      return true;
    }
    // When system wide profiling for 1 second, the scale of a summary running
    // all the time is < 1.0 + 3e-6. The scale difference is smaller if
    // monitored for a longer time or doing non system wide profiling. So it
    // seems proper to set SCALE_ERROR_LIMIT to 1e-5.
    constexpr double SCALE_ERROR_LIMIT = 1e-5;
    return (fabs(scale - 1.0) < SCALE_ERROR_LIMIT &&
            fabs(other.scale - 1.0) < SCALE_ERROR_LIMIT);
  }
