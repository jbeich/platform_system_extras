...
bool StatCommandImpl::Run(const std::vector<std::string>& args) {
  ...
  if (!workload->Start() || !workload->WaitFinish()) {
    return false;
  }
  if (!StopCounting()) {
    return false;
...
bool StatCommandImpl::ShowCounters(std::chrono::duration<double> counting_duration) {
...
class StatCommand : public Command {
 public:
  ...
  bool Run(const std::vector<std::string>& args) override {
    ...
    std::unique_ptr<StatCommandImpl> impl(new StatCommandImpl);
