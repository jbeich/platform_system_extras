...
struct CounterSummary {
  ...
  bool IsMonitoredAtTheSameTime(const CounterSummary& other) const {
    ...
    // all the time is < 1.0 + 3e-6. The scale difference is smaller if monitor
    // for longer time or doing non system wide profiling. So it seems proper
    // to set SCALE_ERROR_LIMIT to 1e-5.
