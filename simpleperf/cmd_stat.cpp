...
"--no-cmd         Profiling a command line isn't allowed.\n"
...
bool StatCommand::Run(const std::vector<std::string>& args) {
  ...
  if (!output_filename_.empty()) {
    ...
    fp_holder.reset(fdopen(out_fd_.release(), "w"));
    if (fp_holder == nullptr) {
...
bool StatCommand::ParseOptions(const std::vector<std::string>& args,
  ...
  if (no_cmd && !non_option_args->empty()) {
    LOG(ERROR) << "Profiling a command line isn't allowed for security reason.";
    return false;
  }
  return true;
}
