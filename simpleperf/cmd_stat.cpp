...
class StatCommand : public Command {
 ...
 private:
  ...
  bool AddMeasuredEventType(const std::string& event_type_name, bool report_unsupport_error = true);
  ...
  struct EventElem {
    ...
    std::vector<read_format> event_counters;
    read_format sum_counter;
...
bool StatCommand::ParseOptions(const std::vector<std::string>& args,
  ...
  for (i = 0; i < args.size() && args[i].size() > 0 && args[i][0] == '-'; ++i) {
    if (args[i] == "-a") {
      ...
      size_t pos = 0, npos;
      while ((npos = args[i].find(",", pos)) != args[i].npos) {
        event_type_list.push_back(args[i].substr(pos, npos - pos));
        pos = npos + 1;
      }
      if (pos != args[i].size()) {
        event_type_list.push_back(args[i].substr(pos));
      }
      for (auto& event_type : event_type_list) {
...
bool StatCommand::AddMeasuredEventType(const std::string& event_type_name,
  ...
  if (!event_type->IsSupportedByKernel()) {
    if (report_unsupport_error) {
      LOG(ERROR) << "Event type " << event_type->name << " is not supported by the kernel";
    } else {
      LOG(DEBUG) << "Event type " << event_type->name << " is not supported by the kernel";
    }
    return false;
  }
...
bool StatCommand::AddDefaultMeasuredEventTypes() {
  for (auto& name : default_measured_event_types) {
    // It is not an error when some event types in default list are not supported by the kernel.
...
bool StatCommand::ReadCounters() {
  for (auto& elem : measured_events_) {
    ...
    read_format sum_counter = event_counters.front();
    for (size_t i = 1; i < event_counters.size(); ++i) {
      sum_counter.nr += event_counters[i].nr;
  ...
  for (auto& elem : measured_events_) {
    ...
    if (verbose_mode_) {
      ...
      for (size_t i = 0; i < elem.event_fds.size(); ++i) {
        printf("%s: nr %s, time_enabled %" PRId64 ", time_disabled %" PRId64 ", id %" PRId64 "\n",
               event_fds[i]->Name().c_str(), FormatCount(counters[i].nr).c_str(),
               counters[i].time_enabled, counters[i].time_running, counters[i].id);
      }
    }

    auto& counter = elem.sum_counter;
    bool scaled = false;
    int64_t scaled_count = counter.nr;
    if (counter.time_running < counter.time_enabled) {
      if (counter.time_running == 0) {
        scaled_count = 0;
      } else {
        scaled = true;
        scaled_count = static_cast<int64_t>(static_cast<double>(counter.nr) * counter.time_enabled /
                                            counter.time_running);
      }
    }
    printf("%30s%s  %s\n", FormatCount(scaled_count).c_str(), scaled ? "(scaled)" : "       ",
...
static std::string FormatCount(int64_t count) {
  ...
  for (size_t level = 0; count != 0; ++level) {
