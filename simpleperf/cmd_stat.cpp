...
struct CounterSummary {
  ...
  bool IsMonitoredAtTheSameTime(const CounterSummary& other) const {
    ...
    // group or both having scale == 1.0.
    return (group_id == other.group_id || (scale == 1.0 && other.scale == 1.0));
  }
...
class CounterSummaries {
 public:
  ...
  // If we have two summaries monitoring one event type in user space and
  // kernel space at the same time period, we can automatically generate a
  // summary combining the results generated in user space and kernel space.
