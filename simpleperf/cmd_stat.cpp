...
class StatCommandImpl {
 ...
 private:
  ...
  EventSelectionList event_selection_list_;
  bool verbose_mode_;
...
bool StatCommandImpl::Run(const std::vector<std::string>& args) {
  ...
  if (event_selection_list_.Empty()) {
  ...
  if (system_wide_collection_) {
    if (!event_selection_list_.OpenEventFilesForAllCpus()) {
      return false;
    }
  } else {
    event_selection_list_.EnableOnExec();
    if (!event_selection_list_.OpenEventFilesForProcess(workload->GetPid())) {
      return false;
    }
  }
  ...
  // counting manually.
  if (system_wide_collection_) {
    if (!event_selection_list_.EnableEvents()) {
      return false;
    }
  }
  if (!workload->Start()) {
  ...
  if (!event_selection_list_.ReadCounters(&counters_map)) {
    return false;
  }
...
bool StatCommandImpl::AddMeasuredEventType(const std::string& event_type_name,
  ...
  event_selection_list_.AddEventType(event_type);
  return true;
}
    ...
    if (verbose_mode_) {
      for (auto& counter : counters) {
        printf("%s: value %'" PRId64 ", time_enabled %" PRId64 ", time_running %" PRId64
               ", id %" PRId64 "\n",
               event_selection_list_.FindEventFileNameById(counter.id).c_str(), counter.value,
