...
using MapTree = std::set<MapEntry*, MapComparator>;
...
class ThreadTree {
 ...
 private:
  ...
  void FixOverlappedMap(MapTree* map_tree, const MapEntry* map);

  std::unordered_map<int, std::unique_ptr<ThreadEntry>> thread_tree_;
  std::vector<std::unique_ptr<std::string>> thread_comm_storage_;

  std::vector<std::unique_ptr<MapTree>> map_tree_storage_;
  MapTree kernel_map_tree_;
