...
void UnwindingResultRecord::DumpData(size_t indent) const {
  ...
  static std::unordered_map<int, std::string> map = {
      {UnwindingStat::UNKNOWN_REASON, "UNKNOWN_REASON"},
      {UnwindingStat::UNW_STEP_STOPPED, "UNW_STEP_STOPPED"},
      {UnwindingStat::MAX_FRAMES_LIMIT, "MAX_FRAME_LIMIT"},
      {UnwindingStat::ACCESS_REG_FAILED, "ACCESS_REG_FAILED"},
      {UnwindingStat::ACCESS_STACK_FAILED, "ACCESS_STACK_FAILED"},
      {UnwindingStat::ACCESS_MEM_FAILED, "ACCESS_MEM_FAILED"},
      {UnwindingStat::FIND_PROC_INFO_FAILED, "FIND_PROC_INFO_FAILED"},
      {UnwindingStat::EXECUTE_DWARF_INSTRUCTION_FAILED, "EXECUTE_DWARF_INSTRUCTION_FAILED"},
      {UnwindingStat::DIFFERENT_ARCH, "DIFFERENT_ARCH"},
  };
  PrintIndented(indent, "stop_reason %s\n", map[stop_reason].c_str());
  if (stop_reason == UnwindingStat::ACCESS_REG_FAILED) {
    PrintIndented(indent, "regno %" PRIu64 "\n", stop_info);
  } else if (stop_reason == UnwindingStat::ACCESS_STACK_FAILED ||
             stop_reason == UnwindingStat::ACCESS_MEM_FAILED) {
    PrintIndented(indent, "addr 0x%" PRIx64 "\n", stop_info);
  } else if (stop_reason == UnwindingStat::EXECUTE_DWARF_INSTRUCTION_FAILED) {
    PrintIndented(indent, "execute_result %" PRIu64 "\n", stop_info);
  }
}
