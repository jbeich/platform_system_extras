...
static bool IsRecordHappensBefore(const Record* r1, const Record* r2) {
  ...
  uint64_t time1 = (is_r1_sample ? static_cast<const SampleRecord*>(r1)->time_data.time
                                 : r1->sample_id.time_data.time);
  uint64_t time2 = (is_r2_sample ? static_cast<const SampleRecord*>(r2)->time_data.time
...
static bool IsRecordHappensBeforeA(const std::unique_ptr<Record>& r1,
...
std::vector<std::unique_ptr<Record>> ReadRecordsFromBuffer(const perf_event_attr& attr,
  ...
  if ((attr.sample_type & PERF_SAMPLE_TIME) && attr.sample_id_all) {
    std::sort(result.begin(), result.end(), IsRecordHappensBeforeA);
  }
  return result;
}
...
RecordCache::RecordCache(const perf_event_attr& attr, size_t cache_size_before_pop,
                         uint64_t time_diff_before_pop)
    : attr_(attr),
      sample_time_(attr.sample_id_all && (attr.sample_type & PERF_SAMPLE_TIME)),
      cache_size_before_pop_(cache_size_before_pop),
      time_diff_before_pop_(time_diff_before_pop),
      last_time_(0),
...
void RecordCache::Push(const char* data, size_t size) {
  ...
  if (sample_time_) {
    for (const auto& r : records) {
      uint64_t record_time = (r->header.type == PERF_RECORD_SAMPLE
                                  ? static_cast<const SampleRecord*>(r.get())->time_data.time
                                  : r->sample_id.time_data.time);
      last_time_ = std::max(last_time_, record_time);
    }
  }
  for (auto& r : records) {
...
std::unique_ptr<Record> RecordCache::Pop() {
  if (queue_.size() < cache_size_before_pop_) {
    return nullptr;
  }
  Record* r = queue_.top();
  if (sample_time_) {
    uint64_t record_time =
        (r->header.type == PERF_RECORD_SAMPLE ? static_cast<const SampleRecord*>(r)->time_data.time
                                              : r->sample_id.time_data.time);
    if (record_time + time_diff_before_pop_ > last_time_) {
      return nullptr;
    }
  }
  queue_.pop();
