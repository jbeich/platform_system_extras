...
std::vector<char> SampleRecord::BinaryFormat() const {
  return BinaryFormatWithRealSize().first;
}

void SampleRecord::GenerateSize() {
  size_t size = BinaryFormatWithRealSize().second;
  LOG(DEBUG) << "SampleRecord size changed from " << header.size << " to " << size;
  header.size = size;
}

std::pair<std::vector<char>, size_t> SampleRecord::BinaryFormatWithRealSize() const {
  std::vector<char> buf(header.size);
  char* p = buf.data();
  MoveToBinaryFormat(header, p);
  if (sample_type & PERF_SAMPLE_IP) {
  ...
  return std::make_pair(buf, p - buf.data());
}
