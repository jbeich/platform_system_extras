...
class EventTypeFinder {
 public:
  ...
  EventTypeFinder(FinderType type) : finder_type_(type) {}
  virtual ~EventTypeFinder() {}

  FinderType GetFinderType() const { return finder_type_; }
...
class BuiltinTypeFinder : public EventTypeFinder {
 public:
  BuiltinTypeFinder() : EventTypeFinder(FinderType::BUILTIN) {}
...
class TracepointStringFinder : public EventTypeFinder {
 public:
  TracepointStringFinder(std::string&& s)
      : EventTypeFinder(FinderType::TRACEPOINT_STRING), s_(std::move(s)) {}
...
class TracepointSystemFinder : public EventTypeFinder {
 public:
  TracepointSystemFinder() : EventTypeFinder(FinderType::TRACEPOINT_SYSTEM) {}
...
class PMUTypeFinder : public EventTypeFinder {
 public:
  PMUTypeFinder() : EventTypeFinder(FinderType::PMU) {}
...
class ETMTypeFinder : public EventTypeFinder {
 public:
  ETMTypeFinder() : EventTypeFinder(FinderType::ETM) {}
...
class RawTypeFinder : public EventTypeFinder {
 public:
  RawTypeFinder() : EventTypeFinder(FinderType::RAW) {}
...
class ScopedTypeFinder : public EventTypeFinder {
 public:
  ScopedTypeFinder(std::set<EventType>&& types) : EventTypeFinder(FinderType::SCOPED) {
...
bool EventTypeManager::ReadTracepointsFromFile(const std::string& filepath) {
  ...
  for (auto& finder : type_finders_) {
    if (finder->GetFinderType() == EventTypeFinder::FinderType::TRACEPOINT_SYSTEM) {
      finder.reset(new TracepointStringFinder(std::move(data)));
      return true;
    }
  }
  LOG(FATAL) << "Failed to find TracepointSystemFinder";
  return false;
}
...
bool EventTypeManager::WriteTracepointsToFile(const std::string& filepath) {
  for (auto& finder : type_finders_) {
    if (finder->GetFinderType() == EventTypeFinder::FinderType::TRACEPOINT_SYSTEM) {
      auto tp_finder = static_cast<TracepointSystemFinder*>(finder.get());
...
const EventType* EventTypeManager::AddRawType(const std::string& name) {
  ...
  for (auto& finder : type_finders_) {
    if (finder->GetFinderType() == EventTypeFinder::FinderType::RAW) {
      auto raw_finder = static_cast<RawTypeFinder*>(finder.get());
      return raw_finder->AddType(EventType(name, PERF_TYPE_RAW, config, "", ""));
    }
  }
  LOG(FATAL) << "Failed to find RawTypeFinder";
  return nullptr;
}
...
bool IsEtmEventType(uint32_t type) {
  const EventType* event_type = EventTypeManager::Instance().FindType("cs-etm");
