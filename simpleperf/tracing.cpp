...
bool SimpleperfTracer::StartTracing(const std::vector<std::string>& events,
  ...
  std::string event_arg = android::base::Join(events, ',');
  tracing_workload_ = CreateTracerCmdWorkload(
      {"--trace-events", event_arg, "--clock", clock, "-o", output_filename});
  if (tracing_workload_ != nullptr) {
    sleep(1);  // wait 1 second to start tracing.
  }
...
bool SimpleperfTracer::StopTracing() {
  ...
  int status;
  if (!tracing_workload_->Join(&status)) {
    return false;
  }
