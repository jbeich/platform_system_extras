...
TEST(read_apk, GetBuildIdFromApkFile) {
  BuildId build_id;
  ASSERT_EQ(ReadElfRet::NO_ERROR, GetBuildIdFromApkFile(GetTestData(APK_FILE), NATIVELIB_IN_APK, &build_id));
  ASSERT_EQ(build_id, native_lib_build_id);
}

TEST(read_apk, ParseSymbolsFromApkFile) {
  std::map<std::string, ElfFileSymbol> symbols;
  ASSERT_EQ(ReadElfRet::NO_SYMBOL_TABLE, ParseSymbolsFromApkFile(GetTestData(APK_FILE), NATIVELIB_IN_APK, native_lib_build_id,
