...
class ListCommand : public Command {
 ...
 private:
  void PrintEventTypesInType(uint32_t type, const char* type_name,
...
bool ListCommand::Run(const std::vector<std::string>& args) {
  ...
  PrintEventTypesInType(PERF_TYPE_HARDWARE, "hardware events", event_types);
  PrintEventTypesInType(PERF_TYPE_SOFTWARE, "software events", event_types);
  PrintEventTypesInType(PERF_TYPE_HW_CACHE, "hw-cache events", event_types);
  return true;
}

void ListCommand::PrintEventTypesInType(uint32_t type, const char* type_name,
                                        const std::vector<const EventType>& event_types) {
  printf("List of %s:\n", type_name);
