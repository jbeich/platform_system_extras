...
// and call corresponding callbacks. Possible events are: file ready to read,
// file ready to write, signal happens, timer timeout.
class IOEventLoop {
 public:
  IOEventLoop();
  ~IOEventLoop();
  ...
  // happens.
  bool AddSignalEvent(int sig, std::function<bool()> callback);

  // Register a vector of signal Events.
  bool AddSignalEvents(std::vector<int> sigs, std::function<bool()> callback);

  // Register a time Event, so [callback] is called periodically every
  // [duration].
  bool AddTimeEvent(timeval duration, std::function<bool()> callback);
