...
static bool RunCmd(const std::vector<std::string>& args, std::string* stdout) {
  int stdout_fd[2];
  if (pipe(stdout_fd) != 0) {
    return false;
  }
  ...
  for (size_t i = 0; i < args.size(); ++i) {
    argv[i] = const_cast<char*>(&args[i][0]);
  }
