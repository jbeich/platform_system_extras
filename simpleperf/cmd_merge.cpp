...
struct FileFeature {
  FileFeature(uint32_t file_type, uint64_t min_vaddr, uint64_t file_offset_of_min_vaddr,
              std::vector<Symbol>&& symbols, std::vector<uint64_t>&& dex_file_offsets)
      : file_type(file_type),
        min_vaddr(min_vaddr),
        file_offset_of_min_vaddr(file_offset_of_min_vaddr),
        dex_file_offsets(std::move(dex_file_offsets)) {
    for (auto& symbol : symbols) {
      symbol_map.emplace(symbol.addr, std::move(symbol));
    }
  }

  bool MergeSymbols(std::vector<Symbol>&& symbols) {
    for (auto& symbol : symbols) {
      auto it = symbol_map.lower_bound(symbol.addr);
  ...
  std::map<uint64_t, Symbol> symbol_map;
...
"$ simpleperf merge -i perf1.data,perf2.data -o perf.data\n"
 ...
 private:
  ...
  bool WriteGapInDataSection(size_t prev_reader_id, size_t next_reader_id) {
    ...
    // map event_ids in readers_[next_read_id] to event attrs. The map info is put in an
    // EventIdRecord.
  ...
  bool WriteFileFeature() {
    std::map<std::string, FileFeature> file_map;
    std::unordered_set<std::string> files_to_drop;
    ...
    for (auto& reader : readers_) {
      ...
      while (reader->ReadFileFeature(read_pos, &file_path, &file_type, &min_vaddr,
        ...
        if (auto it = file_map.find(file_path); it == file_map.end()) {
          file_map.emplace(std::piecewise_construct, std::forward_as_tuple(file_path),
                           std::forward_as_tuple(file_type, min_vaddr, file_offset_of_min_vaddr,
                                                 std::move(symbols), std::move(dex_file_offsets)));
        } else {
          auto& file = it->second;
          if (file.file_type == file_type && file.min_vaddr == min_vaddr &&
              file.file_offset_of_min_vaddr == file_offset_of_min_vaddr &&
              file.dex_file_offsets == dex_file_offsets && file.MergeSymbols(std::move(symbols))) {
            continue;
          }
          LOG(WARNING)
              << file_path
              << " has address-conflict symbols in different record files. So drop its symbols.";
          files_to_drop.emplace(file_path);
        }
      }
    }
    // Write file features.
    for (auto& [file_path, file] : file_map) {
      if (files_to_drop.count(file_path) != 0) {
        continue;
      }
      std::vector<const Symbol*> symbols;
      for (const auto& [_, symbol] : file.symbol_map) {
        symbols.emplace_back(&symbol);
      }
      std::vector<uint64_t>* dex_file_offsets =
          (file.file_type == static_cast<uint32_t>(DSO_DEX_FILE)) ? &file.dex_file_offsets
                                                                  : nullptr;
      if (!writer_->WriteFileFeature(file_path, file.file_type, file.min_vaddr,
                                     file.file_offset_of_min_vaddr, symbols, dex_file_offsets)) {
        return false;
      }
    }
    return true;
