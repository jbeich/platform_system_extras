...
TEST(command, GetValueForOption) {
  ...
  for (bool allow_suffixes : {true, false}) {
    i = 0;
    ASSERT_TRUE(command.GetValueOption({"-s", "156"}, &i, &value, 0,
                                       std::numeric_limits<uint64_t>::max(), allow_suffixes));
    ASSERT_EQ(i, 1u);
    ASSERT_EQ(value, 156u);
  }
  i = 0;
  ASSERT_TRUE(command.GetValueOption({"-s", "156k"}, &i, &value, 0,
                                     std::numeric_limits<uint64_t>::max(), true));
  ASSERT_EQ(value, 156 * (1ULL << 10));
  i = 0;
  ASSERT_FALSE(command.GetValueOption({"-s"}, &i, &value));
  i = 0;
  ASSERT_FALSE(command.GetValueOption({"-s", "0"}, &i, &value, 1));
  i = 0;
  ASSERT_FALSE(command.GetValueOption({"-s", "156"}, &i, &value, 0, 155));
  i = 0;
  double double_value;
  ASSERT_TRUE(command.GetValueOption({"-s", "3.2"}, &i, &double_value, 0, 4));
