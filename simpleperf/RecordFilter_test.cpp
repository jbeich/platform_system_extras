...
TEST_F(RecordFilterTest, parse_options) {
  ...
  OptionValueMap option_map;
  auto build_options = [&](bool exclude, const std::string& name,
                           const char* value) -> OptionValueMap& {
    std::string option_name = (exclude ? "--exclude-" : "--include-") + name;
    str_value = value;
    OptionValue option_value;
    option_value.str_value = &str_value;
    option_map.values.clear();
    option_map.values.emplace(option_name, option_value);
    return option_map;
  };

  for (bool exclude : {true, false}) {
    filter.ParseOptions(build_options(exclude, "pid", "1,2"));
    ASSERT_EQ(filter.GetCondition(exclude).pids, std::set<pid_t>({1, 2}));
    filter.ParseOptions(build_options(exclude, "tid", "3,4"));
    ASSERT_EQ(filter.GetCondition(exclude).tids, std::set<pid_t>({3, 4}));
    filter.ParseOptions(build_options(exclude, "process-name", "processA"));
    ASSERT_EQ(filter.GetCondition(exclude).process_name_regs.size(), 1);
    ASSERT_TRUE(std::regex_match("processA", filter.GetCondition(exclude).process_name_regs[0]));
    filter.ParseOptions(build_options(exclude, "thread-name", "threadA"));
    ASSERT_EQ(filter.GetCondition(exclude).thread_name_regs.size(), 1);
    ASSERT_TRUE(std::regex_match("threadA", filter.GetCondition(exclude).thread_name_regs[0]));
    filter.ParseOptions(build_options(exclude, "uid", "5,6"));
    ASSERT_EQ(filter.GetCondition(exclude).uids, std::set<uid_t>({5, 6}));
  }
}
