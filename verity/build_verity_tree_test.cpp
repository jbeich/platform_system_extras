...
class BuildVerityTreeTest : public ::testing::Test {
 protected:
  ...
  std::unique_ptr<HashTreeBuilder> hasher{nullptr};
};

TEST_F(BuildVerityTreeTest, CalculateSize) {
  // The hash of source data will occupy at least one block
  ASSERT_EQ(4096u, hasher->CalculateSize(1));
  // Sha256 hash of 128 blocks fits into one block.
  ASSERT_EQ(4096u, hasher->CalculateSize(128 * 4096));
  // Requires 3 blocks to hold more data.
  ASSERT_EQ(12288u, hasher->CalculateSize(128 * 4096 + 1));

  ASSERT_EQ(20811776u, hasher->CalculateSize(2641915904));
}
