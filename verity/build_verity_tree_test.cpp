...
class BuildVerityTreeTest : public ::testing::Test {
 protected:
  ...
  void generate_hash_tree(const std::vector<unsigned char>& data,
    ...
    ASSERT_TRUE(hasher.hash_input_data(data.data(), data.size()));
    ASSERT_TRUE(hasher.build_hash_tree());
  }
...
TEST_F(BuildVerityTreeTest, StreamingDataMultipleBlocks) {
  ...
  while (offset < data.size()) {
    ...
    ASSERT_TRUE(hasher.hash_input_data(data.data() + offset, data_length));
    offset += data_length;
  }
