...
static void encode_rs(struct image_proc_ctx *ctx)
{
    ...
    void *rs = init_rs_char(FEC_PARAMS(fcx->roots));

    if (!rs) {
        FATAL("failed to initialize encoder in thread %d\n", ctx->id);
    }
    ...
    for (i = ctx->start; i < ctx->end; i += fcx->rs_n) {
        ...
        encode_rs_char(rs, data, &fcx->fec[ctx->fec_pos]);
...
static void decode_rs(struct image_proc_ctx *ctx)
{
    ...
    if (!rs) {
        FATAL("failed to initialize decoder in thread %d\n", ctx->id);
    }
    ...
    for (i = ctx->start; i < ctx->end; i += fcx->rs_n) {
        ...
        rv = decode_rs_char(rs, data, NULL, 0);
...
static void usage(void)
{
    printf("usage: fec <mode> [ <options> ] <data> <fec> [ <output> ]\n"
           ...
           "encoding options:\n"
           "  -n, --data-size=<data size>       specify input size\n"
           "decoding options:\n"
...
int main(int argc, char **argv)
{
    ...
    while (1) {
        ...
        int c = getopt_long(argc, argv, "hedSr:imj:n:s:v", long_options, NULL);
        if (c < 0) {
            break;
        }
        ...
        case 'j':
            ...
            break;
        case 'n':
            ctx.inp_size = parse_arg(optarg, "data-size", UINT64_MAX);
