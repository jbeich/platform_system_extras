...
{
    size_t key_size = ANDROID_PUBKEY_ENCODED_SIZE_FOR_KEY(private_key);
    ...
    if (!android_pubkey_encode(private_key, key_data, key_size))
        goto out;
    ...
    BIO_write(bfile, key_data, key_size);
    BIO_flush(bfile);
