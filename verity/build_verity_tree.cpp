...
int main(int argc, char **argv)
{
  ...
  sparseHashCtx hasher(kBlockSize);

  if (salt.empty()) {
    salt.resize(hasher.hash_size());
  ...
  // TODO(xunchang) move the size calculation to sparseHashCtx.
  if (calculate_size) {
    if (argc != 0) {
      usage();
      return 1;
  ...
  // Initialize the hasher to compute the hash tree.
  if (!hasher.initialize(len, salt)) {
    LOG(ERROR) << "Failed to initialize sparseHashCtx";
    return 1;
  }

  auto hash_callback = [](void* priv, const void* data, size_t len) {
    auto sparse_hasher = static_cast<sparseHashCtx*>(priv);
    return sparse_hasher->hash_input_data(
               static_cast<const unsigned char*>(data), len)
               ? 0
               : 1;
  };
  sparse_file_callback(file, false, false, hash_callback, &hasher);
  sparse_file_destroy(file);

  hasher.build_hash_tree();

  android::base::unique_fd verity_fd(
      open(verity_filename, O_WRONLY | O_CREAT, 0666));
  if (verity_fd == -1) {
    FATAL("failed to open output file '%s'\n", verity_filename);
  }

  if (!hasher.write_hash_tree_to_fd(verity_fd)) {
    return 1;
  }

  // Output the root hash and the salt.
  for (const auto& c : hasher.root_hash()) {
    printf("%02x", c);
  }
  printf(" ");
