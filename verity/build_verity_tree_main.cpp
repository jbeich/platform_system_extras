...
int main(int argc, char **argv)
{
  ...
  if (calculate_size) {
    ...
    uint64_t tree_size = hasher.CalculateSize(calculate_size);
    printf("%" PRIu64 "\n", tree_size);
    return 0;
  }
  ...
  if (salt.empty()) {
    ...
    if (random_fd < 0) {
      LOG(ERROR) << "failed to open /dev/urandom";
      return 1;
    }
  ...
  if (!generate_verity_tree(argv[0], argv[1], &hasher, salt, kBlockSize, sparse,
                            verbose)) {
    return 1;
  }

  // Output the root hash and the salt.
  std::string root_hash_string = HexToString(hasher.root_hash());
  std::string salt_string = HexToString(salt);
