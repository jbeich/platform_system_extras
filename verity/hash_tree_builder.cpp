...
bool HashTreeBuilder::Initialize(int64_t expected_data_size,
  ...
  HashBlock(zero_block, block_size_, zero_block_hash_.data());

  return true;
}

bool HashTreeBuilder::HashBlock(const unsigned char* block, size_t len,
                                unsigned char* out) {
...
bool HashTreeBuilder::HashBlocks(const unsigned char* data, size_t len,
  ...
  for (size_t i = 0; i < len; i += block_size_) {
    ...
    if (!HashBlock(data + i, block_size_, hash_buffer)) {
      return false;
    }
...
bool HashTreeBuilder::WriteHashTreeToFd(int fd, size_t offset) const {
