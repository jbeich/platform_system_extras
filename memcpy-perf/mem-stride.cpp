...
int main(int argc, char *argv[])
{
    BenchType type;
    if (argc <= 1) {
        cerr << "memcpy_perf [--memcpy|--memset]" << endl;
        return 0;
    }
    ...
    unique_ptr<uint8_t[]> src(new uint8_t[size_end + 128]);
    unique_ptr<uint8_t[]> dst(new uint8_t[size_end + 128]);
    ...
    for (double cur_pow = start_pow; cur_pow <= end_pow; cur_pow += pow_inc) {
        ...
        switch (type) {
            ...
            case MemcpyBench: {
                ...
                copy_start = chrono::high_resolution_clock::now();
                for (int i = 0; i < iter_per_size; i++) {
                    memcpy_noinline(dst.get() + 64, src.get(), cur_size);
                }
                copy_end = chrono::high_resolution_clock::now();
                break;
            }
