...
static void usage(FILE* where, int /* argc */, char* argv[])
{
    fprintf(where,
            ...
            "                                   or CANCELLED.\n"
            "  is-snapshot-merging            - Returns 0 if a snapshot-merge of any dynamic\n"
            "                                   partition is in progress.\n"
            "\n"
...
static int do_is_snapshot_merging(sp<IBootControl> module) {
    MergeStatus ret = module->getSnapshotMergeStatus();
    std::stringstream ss;
    ss << ret;
    fprintf(stdout, "%s\n", ss.str().c_str());
    if (ret == MergeStatus::MERGING) {
        return EX_OK;
    }
    return EX_SOFTWARE;
}
...
int main(int argc, char *argv[])
{
    ...
    } else if (strcmp(argv[1], "is-snapshot-merging") == 0) {
        return do_is_snapshot_merging(module);
    } else {
        usage(stderr, argc, argv);
        return EX_USAGE;
    }
