...
int main(int argc, char** argv) {
  if (argc != 5 && argc != 6) {
    fprintf(stderr, "Usage: native_alloc NUM_THREADS ALLOC_SIZE NUM_ITERATIONS NUM_ALLOCATIONS [DO_MEMSET]\n");
    fprintf(stderr, "  NUM_THREADS: The number of simultaneous threads doing allocations/frees\n");
    fprintf(stderr, "  ALLOC_SIZE: The size of each allocation done.\n");
    fprintf(stderr, "  NUM_ITERATIONS: The number of times to do an alloc/free.\n");
    fprintf(stderr, "  NUM_ALLOCATIONS: The number of allocations to keep alive.\n");
    fprintf(stderr, "  DO_MEMSET: If non-zero, then memset the allocated memory.\n");
    return 1;
  }
