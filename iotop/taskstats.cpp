...
bool TaskstatsSocket::GetStats(int pid, int type, TaskStatistics& stats) {
  ...
  std::unique_ptr<struct nl_msg, decltype(&nlmsg_free)> message(nlmsg_alloc(),
                                                                nlmsg_free);
...
TaskStatistics TaskStatistics::Delta(
    const TaskStatistics& old_statistics) const {
  ...
  ret.total_delay_ns        -= old_statistics.total_delay_ns;
  ...
  ret.read_write_bytes      -= old_statistics.read_write_bytes;
