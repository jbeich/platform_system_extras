...
struct ufdt_node *ufdt_get_node_by_path_len(struct ufdt *tree, const char *path,
  ...
  if (*path != '/') {
    ...
    aliases_node =
        ufdt_node_get_property_by_name_len(aliases_node, path, next_slash - path);

    int path_len = 0;
    const char *alias_path = ufdt_node_get_fdt_prop_data(aliases_node, &path_len);
...
struct ufdt *fdt_to_ufdt(void *fdtp, size_t fdt_size) {
  ...
  res_tree->root =
      fdt_to_ufdt_tree(fdtp, start_offset, &end_offset, start_tag);
