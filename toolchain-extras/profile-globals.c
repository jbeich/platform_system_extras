...
// This file provides a wrapper for getenv that appends the pid of the current
// process to GCOV_PREFIX.  This avoids conflicts and permissions issues when
// different processes try to create/access the same directories and files under
// $GCOV_PREFIX.
//
...
__attribute__((weak)) char *__wrap_getenv(const char *name) {
  ...
  sprintf(modified_gcov_prefix, "%s/%u", __real_getenv(name), getuid());
