...
#define MAX_KEY_SPECIFIER_SIZE_HEX (2 * FSCRYPT_KEY_IDENTIFIER_SIZE + 1)
...
static void key_specifier_to_hex(const char* key_specifier, size_t key_specifier_length, char* hex) {
    size_t j = 0;
    for (size_t i = 0; i < key_specifier_length; i++) {
        hex[j++] = HEX_LOOKUP[(key_specifier[i] & 0xF0) >> 4];
        hex[j++] = HEX_LOOKUP[key_specifier[i] & 0x0F];
    }
    hex[j] = '\0';
}
...
static uint8_t fscrypt_get_policy_flags(int filenames_encryption_mode, int policy_version) {
    ...
    // To maintain compatibility, use 4-byte filenames padding with the original
    // setting of v1 policies using AES-256-CTS.
    //
    // For all other settings, use 16-byte padding.  This is more secure (helps
    // hide the length of filenames) and makes the inputs evenly divisible into
    // blocks which is more efficient for encryption and decryption.
...
static bool fscrypt_is_encrypted(int fd) {
    fscrypt_policy_v1 fp;

    // success => encrypted with v1 policy
    // EINVAL => encrypted with v2 policy
    // ENODATA => not encrypted
    return ioctl(fd, FS_IOC_GET_ENCRYPTION_POLICY, &fp) == 0 || errno == EINVAL;
}

int fscrypt_policy_ensure(const char* directory, const char* key_specifier,
                          size_t key_specifier_length, const char* contents_encryption_mode,
    ...
    switch (policy_version) {
        case 1:
            if (key_specifier_length != FSCRYPT_KEY_DESCRIPTOR_SIZE) {
                LOG(ERROR) << "Invalid key specifier length for v1 policy: " << key_specifier_length;
                return -1;
            }
            ...
            memcpy(policy.v1.master_key_descriptor, key_specifier, FSCRYPT_KEY_DESCRIPTOR_SIZE);
            break;
        case 2:
            if (key_specifier_length != FSCRYPT_KEY_IDENTIFIER_SIZE) {
                LOG(ERROR) << "Invalid key specifier length for v2 policy: " << key_specifier_length;
                return -1;
            }
            ...
            memcpy(policy.v2.master_key_identifier, key_specifier, FSCRYPT_KEY_IDENTIFIER_SIZE);
            break;
        default:
    ...
    char keyspec_hex[MAX_KEY_SPECIFIER_SIZE_HEX];
    key_specifier_to_hex(key_specifier, key_specifier_length, keyspec_hex);
    ...
    if (ioctl(fd, FS_IOC_SET_ENCRYPTION_POLICY, &policy) != 0) {
        ...
        LOG(ERROR) << "Failed to set encryption policy of " << directory << " to " << keyspec_hex
    ...
    if (already_encrypted) {
        LOG(INFO) << "Verified that " << directory << " has the encryption policy " << keyspec_hex
                  << " modes " << contents_mode << "/" << filenames_mode;
    } else {
        LOG(INFO) << "Encryption policy of " << directory << " set to " << keyspec_hex << " modes "
