...
bool ParseOptionsForApiLevel(unsigned int first_api_level, const std::string& options_string,
    ...
    int iv_policies = 0;
    iv_policies += !!(options->flags & FSCRYPT_POLICY_FLAG_IV_INO_LBLK_64);
    iv_policies += !!(options->flags & FSCRYPT_POLICY_FLAG_IV_INO_LBLK_32);
    iv_policies += !!(options->flags & FSCRYPT_POLICY_FLAG_DIRECT_KEY);
    if (iv_policies > 1) {
        LOG(ERROR) << "At most one IV policy can be set, invalid options: " << options_string;
        return false;
    }
