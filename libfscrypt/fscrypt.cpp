...
bool EnsurePolicy(const EncryptionPolicy& policy, const std::string& directory) {
    ...
    policy_descr += " v" + std::to_string(policy.options.version) + " modes " +
                    std::to_string(policy.options.contents_mode) + "/" +
                    std::to_string(policy.options.filenames_mode) + " flags " +
                    std::to_string(policy.options.flags);
