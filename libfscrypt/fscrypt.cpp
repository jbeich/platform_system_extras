...
static const auto contents_modes = std::vector<ModeLookupEntry>{
        {"aes-256-xts"s, FSCRYPT_MODE_AES_256_XTS},  // default
...
static const auto filenames_modes = std::vector<ModeLookupEntry>{
        {"aes-256-cts"s, FSCRYPT_MODE_AES_256_CTS},  // default
...
static unsigned int GetFirstApiLevel() {
...
bool ParseOptionsForApiLevel(unsigned int first_api_level, const std::string& options_string,
    ...
    if (!parts[0].empty() && !LookupModeByName(contents_modes, parts[0], &options->contents_mode)) {
    ...
    if (parts.size() >= 2 && !parts[1].empty() &&
        !LookupModeByName(filenames_modes, parts[1], &options->filenames_mode)) {
        LOG(ERROR) << "Invalid file names encryption mode: " << parts[1];
        return false;
    }
    // Default to v2 after Q
    constexpr unsigned int pre_gki_level = 29;
    options->version = first_api_level > pre_gki_level ? 2 : 1;
    options->flags = 0;
    ...
    if (first_api_level > pre_gki_level) {
        ...
        // A bug in an earlier version of this code used filenames_mode
        // not contents_mode as the key; preserve this bug for old API levels.
