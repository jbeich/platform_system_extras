    ...
    switch (policy.options.version) {
        case 1:
            if (policy.key_raw_ref.size() != FSCRYPT_KEY_DESCRIPTOR_SIZE) {
                LOG(ERROR) << "Invalid key ref length for v1 options: "
        ...
        case 2:
            if (policy.key_raw_ref.size() != FSCRYPT_KEY_IDENTIFIER_SIZE) {
                LOG(ERROR) << "Invalid key ref length for v2 options: "
                           << policy.key_raw_ref.size();
                return false;
            }
            ...
            break;
        default:
            LOG(ERROR) << "Invalid encryption options version: " << policy.options.version;
            return false;
    }

    std::string ref;
    BytesToHex(policy.key_raw_ref, &ref);
    ref += " modes "s + std::to_string(policy.options.contents_mode) + "/" +
