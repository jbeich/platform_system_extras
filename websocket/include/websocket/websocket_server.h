...
class WebSocketServer {
  ...
  private:
    ...
    static int ServerCallback(struct lws* wsi, enum lws_callback_reasons reason, void* user,
                              void* in, size_t len);
    static std::shared_ptr<WebSocketHandler> InstantiateHandler(const std::string& uri_path,
                                                                struct lws* wsi);
