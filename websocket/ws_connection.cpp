...
int LwsCallback(struct lws* wsi, enum lws_callback_reasons reason, void* user, void* in,
                size_t len);
...
void WsConnectionContextImpl::RememberConnection(void* raw, std::weak_ptr<WsConnectionImpl> conn) {
    std::lock_guard<std::mutex> lock(map_mutex_);
    weak_by_ptr_.emplace(std::pair<void*, std::weak_ptr<WsConnectionImpl>>(raw, conn));
}
...
void WsConnectionImpl::Connect() {
    ...
    lws_sul_schedule(context_->lws_context(), 0, &extended_sul_.sul, CreateConnectionCallback, 1);
}

void WsConnectionImpl::AddHttpHeaders(unsigned char** p, unsigned char* end) const {
    for (const auto& header_entry : headers_) {
        ...
        auto res = lws_add_http_header_by_name(
                ...
                reinterpret_cast<const unsigned char*>(value.c_str()), value.size(), p, end);
        if (res != 0) {
...
void WsConnectionImpl::OnReceive(const uint8_t* data, size_t len, bool is_binary) {
    auto observer = observer_.lock();
    if (observer) {
...
void WsConnectionImpl::OnWriteable() {
    ...
    auto flags =
            lws_write_ws_flags(buffer.is_binary() ? LWS_WRITE_BINARY : LWS_WRITE_TEXT, true, true);
    auto res = lws_write(wsi_, buffer.data(), buffer.size(), (enum lws_write_protocol)flags);
    if (res != buffer.size()) {
...
int LwsCallback(struct lws* wsi, enum lws_callback_reasons reason, void* user, void* in,
    ...
    // care.
    auto with_connection = [wsi, user](std::function<void(std::shared_ptr<WsConnectionImpl>)> cb) {
    ...
    switch (reason) {
        case LWS_CALLBACK_CLIENT_CONNECTION_ERROR:
            return with_connection([in](std::shared_ptr<WsConnectionImpl> connection) {
                connection->OnError(in ? (char*)in : "(null)");
            });

        case LWS_CALLBACK_CLIENT_RECEIVE:
            return with_connection([in, len, wsi](std::shared_ptr<WsConnectionImpl> connection) {
                connection->OnReceive((const uint8_t*)in, len, lws_frame_is_binary(wsi));
            });

        case LWS_CALLBACK_CLIENT_ESTABLISHED:
            return with_connection(
                    [](std::shared_ptr<WsConnectionImpl> connection) { connection->OnOpen(); });

        case LWS_CALLBACK_CLIENT_CLOSED:
            return with_connection(
                    [](std::shared_ptr<WsConnectionImpl> connection) { connection->OnClose(); });
        ...
        case LWS_CALLBACK_CLIENT_APPEND_HANDSHAKE_HEADER:
            return with_connection([in, len](std::shared_ptr<WsConnectionImpl> connection) {
...
void CreateConnectionCallback(lws_sorted_usec_list_t* sul) {
    std::shared_ptr<WsConnectionImpl> connection =
            reinterpret_cast<WsConnectionImpl::CreateConnectionSul*>(sul)->weak_this.lock();
    if (!connection) {
    ...
    switch (security_) {
        case Security::kAllowSelfSigned:
            connect_info.ssl_connection = LCCSCF_ALLOW_SELFSIGNED |
                                          LCCSCF_SKIP_SERVER_CERT_HOSTNAME_CHECK | LCCSCF_USE_SSL;
            break;
