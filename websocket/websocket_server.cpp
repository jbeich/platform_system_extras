...
WebSocketServer::WebSocketServer(const char* protocol_name, const std::string& certs_dir,
    ...
    struct lws_protocols protocols[] = {{protocol_name, ServerCallback, 4096, 0, 0, nullptr, 0},
...
void WebSocketServer::RegisterHandlerFactory(
        const std::string& path, std::unique_ptr<WebSocketHandlerFactory> handler_factory_p) {
...
int WebSocketServer::ServerCallback(struct lws* wsi, enum lws_callback_reasons reason, void* user,
                                    void* in, size_t len) {
    switch (reason) {
...
std::shared_ptr<WebSocketHandler> WebSocketServer::InstantiateHandler(const std::string& uri_path,
                                                                      struct lws* wsi) {
