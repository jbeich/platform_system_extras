...
int avb_chain_partition_descriptor_validate_and_byteswap(
  ...
  if (dest->parent_descriptor.tag != AVB_DESCRIPTOR_TAG_CHAIN_PARTITION) {
    avb_warning("Invalid tag %" PRIu64 " for chain partition descriptor.\n",
  ...
  if (dest->rollback_index_slot < 1) {
    avb_warning("Invalid rollback index slot value %d.\n",
                dest->rollback_index_slot);
    return 0;
  }
