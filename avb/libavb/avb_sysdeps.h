 ...
 * equivalent types available in a normal C runtime. At least uint64_t
 * at bool (with |false|, |true| keywords) must be present.
 */
#include <inttypes.h>
#include <stdbool.h>
#include <stddef.h>
...
#define avb_assert(expr)                                                  \
  do {                                                                    \
    if (!(expr)) {                                                        \
      avb_fatal("assert fail: %s at %s:%d\n", #expr, __FILE__, __LINE__); \
    }                                                                     \
  } while (0)
#else
#define avb_assert(expr)
