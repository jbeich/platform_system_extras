...
int avb_property_descriptor_validate_and_byteswap(
  ...
  if (dest->parent_descriptor.tag != AVB_DESCRIPTOR_TAG_PROPERTY) {
    avb_warning("Invalid tag %" PRIu64 " for property descriptor.\n",
...
static int property_lookup_desc_foreach(const AvbDescriptor* header,
  ...
  const uint8_t* p;
  const AvbPropertyDescriptor* ph;
  uint64_t key_nb;
  uint64_t value_nb;
  int ret = 1;
  ...
  ph = (const AvbPropertyDescriptor*)header;
  key_nb = avb_be64toh(ph->key_num_bytes);
  value_nb = avb_be64toh(ph->value_num_bytes);

  // TODO: validate key_nb and value_nb.

  if (p[sizeof(AvbPropertyDescriptor) + key_nb] != 0) {
    avb_warning("No terminating NUL byte in key.\n");
    goto out;
  }

  if (data->key_size == key_nb) {
    if (avb_memcmp(p + sizeof(AvbPropertyDescriptor), data->key,
                   data->key_size) == 0) {
      data->ret_value =
          (const char*)(p + sizeof(AvbPropertyDescriptor) + key_nb + 1);
      data->ret_value_size = value_nb;
      // Stop iterating.
      ret = 0;
      goto out;
    }
