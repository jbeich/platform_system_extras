...
static AvbSlotVerifyResult load_and_verify_hash_partition(
  ...
  io_ret = ops->read_from_partition(ops, part_name, image_buf, 0 /* offset */,
                                    hash_desc.image_size, &part_num_read);
  if (io_ret != AVB_IO_RESULT_OK) {
    avb_error("Error loading data from '%s': %d\n", part_name, io_ret);
    ret = AVB_SLOT_VERIFY_RESULT_ERROR_IO;
    goto out;
  }
  if (part_num_read != hash_desc.image_size) {
    avb_error("Requested %zd bytes but only read %zd bytes\n",
              hash_desc.image_size, part_num_read);
    ret = AVB_SLOT_VERIFY_RESULT_ERROR_IO;
    goto out;
  }
  ...
  if (avb_strcmp((const char*)hash_desc.hash_algorithm, "sha256") == 0) {
    ...
    avb_error("Unsupported hash algorithm '%s'.\n", hash_desc.hash_algorithm);
    ret = AVB_SLOT_VERIFY_RESULT_ERROR_INVALID_METADATA;
    goto out;
  }

  if (digest_len != hash_desc.digest_len) {
    avb_error("Digest in descriptor is %d bytes but expected %zd bytes.\n",
  ...
  if (avb_safe_memcmp(digest, desc_digest, digest_len) != 0) {
    avb_error("Hash of data does not match digest in descriptor.\n");
...
static AvbSlotVerifyResult load_and_verify_vbmeta(
  ...
  if (is_main_vbmeta) {
    ...
    io_ret = ops->read_from_partition(ops, full_partition_name, footer_buf,
                                      ...
                                      &footer_num_read);
    if (io_ret != AVB_IO_RESULT_OK) {
      avb_error("%s: Error loading footer: %d\n", full_partition_name, io_ret);
      ret = AVB_SLOT_VERIFY_RESULT_ERROR_IO;
      goto out;
    }
    ...
    if (!avb_footer_validate_and_byteswap((const AvbFooter*)footer_buf,
                                          &footer)) {
      avb_error("%s: Error validating footer.\n", full_partition_name);
      ret = AVB_SLOT_VERIFY_RESULT_ERROR_INVALID_METADATA;
      goto out;
    }
    ...
    if (footer.vbmeta_size > VBMETA_MAX_SIZE) {
      avb_error("%s: Footer size of %" PRIu64 " is invalid.\n",
  ...
  if (io_ret != AVB_IO_RESULT_OK) {
    avb_error("%s: Error loading %zd bytes from offset %zd: %d\n",
  ...
  if (expected_public_key != NULL) {
    ...
    if (!ops->validate_vbmeta_public_key(ops, pk_data, pk_len)) {
      avb_error("%s: Public key used to sign data rejected.\n",
                full_partition_name);
      ret = AVB_SLOT_VERIFY_RESULT_ERROR_PUBLIC_KEY_REJECTED;
      goto out;
    }
  ...
  for (n = 0; n < num_descriptors; n++) {
    ...
    if (!avb_descriptor_validate_and_byteswap(descriptors[n], &desc)) {
      avb_error("%s: Descriptor %zd is invalid.\n", full_partition_name, n);
      ret = AVB_SLOT_VERIFY_RESULT_ERROR_INVALID_METADATA;
      goto out;
    }
    ...
    switch (desc.tag) {
      ...
      case AVB_DESCRIPTOR_TAG_CHAIN_PARTITION: {
        ...
        if (!avb_chain_partition_descriptor_validate_and_byteswap(
          ...
          avb_error("%s: Chain partition descriptor %zd is invalid.\n",
                    full_partition_name, n);
          ret = AVB_SLOT_VERIFY_RESULT_ERROR_INVALID_METADATA;
          goto out;
        }
      ...
      case AVB_DESCRIPTOR_TAG_KERNEL_CMDLINE: {
        ...
        if (!avb_kernel_cmdline_descriptor_validate_and_byteswap(
                ...
                &kernel_cmdline_desc)) {
          avb_error("%s: Kernel cmdline descriptor %zd is invalid.\n",
                    full_partition_name, n);
          ret = AVB_SLOT_VERIFY_RESULT_ERROR_INVALID_METADATA;
          goto out;
        }
        ...
        if (!avb_validate_utf8(kernel_cmdline,
          ...
          avb_error("Kernel cmdline is not valid UTF-8.\n");
          ret = AVB_SLOT_VERIFY_RESULT_ERROR_INVALID_METADATA;
          goto out;
        }
  ...
  if (rollback_index_slot >= AVB_MAX_NUMBER_OF_ROLLBACK_INDEX_SLOTS) {
    avb_error("%s: Invalid rollback_index_slot value %d.\n",
              full_partition_name, rollback_index_slot);
    ret = AVB_SLOT_VERIFY_RESULT_ERROR_INVALID_METADATA;
    goto out;
  }
...
static char* sub_cmdline(AvbOps* ops, const char* cmdline,
  ...
  for (size_t n = 0; n < NUM_GUIDS; n++) {
    ...
    if (!ops->get_unique_guid_for_partition(ops, part_name, guid_buf,
                                            sizeof guid_buf)) {
      avb_error("Error getting unique GUID for partition '%s'.\n", part_name);
      goto fail;
    }
