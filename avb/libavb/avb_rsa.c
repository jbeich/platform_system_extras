 ...
 */
int avb_rsa_verify(const uint8_t* key, size_t key_num_bytes, const uint8_t* sig,
  ...
  if (key == NULL || sig == NULL || hash == NULL || padding == NULL) {
    avb_warning("Invalid input.\n");
    goto out;
  }

  parsed_key = parse_key_data(key, key_num_bytes);
  if (parsed_key == NULL) {
    avb_warning("Error parsing key.\n");
    goto out;
  }

  if (sig_num_bytes != (parsed_key->len * sizeof(uint32_t))) {
    avb_warning("Signature length does not match key length.\n");
    goto out;
  }

  if (padding_num_bytes != sig_num_bytes - hash_num_bytes) {
    avb_warning("Padding length does not match hash and signature lengths.\n");
    goto out;
  }

  buf = (uint8_t*)avb_malloc(sig_num_bytes);
  if (buf == NULL) {
    avb_warning("Error allocating %d bytes.\n", (int)sig_num_bytes);
    goto out;
  }
  ...
  if (avb_safe_memcmp(buf, padding, padding_num_bytes)) {
    avb_warning("Padding check failed.\n");
    goto out;
  }

  /* Check hash. */
  if (avb_safe_memcmp(buf + padding_num_bytes, hash, hash_num_bytes)) {
    avb_warning("Hash check failed.\n");
    goto out;
  }
