...
AvbVBMetaVerifyResult avb_vbmeta_image_verify(
  ...
  if (h.header_version_major > AVB_MAJOR_VERSION) {
    avb_warning("No support for version %d.\n", h.header_version_major);
    goto out;
  }
  ...
  if ((h.authentication_data_block_size & 0x3f) != 0 ||
    ...
    avb_warning("Block size is not a multiple of 64.\n");
    goto out;
  }
  ...
  if (!avb_safe_add_to(&block_total, h.authentication_data_block_size) ||
      !avb_safe_add_to(&block_total, h.auxilary_data_block_size)) {
    avb_warning("Overflow while computing size of boot image.\n");
    goto out;
  }
  if (block_total > length) {
    avb_warning("Block sizes add up to more than given length.\n");
    goto out;
  }
  ...
  if (!avb_safe_add(NULL, (uint64_t)data_ptr, length)) {
    avb_warning("Boot image location and length mismatch.\n");
    goto out;
  }
  ...
  if (!avb_safe_add(&hash_end, h.hash_offset, h.hash_size) ||
    ...
    avb_warning("Hash is not entirely in its block.\n");
    goto out;
  }
  ...
  if (!avb_safe_add(&signature_end, h.signature_offset, h.signature_size) ||
    ...
    avb_warning("Signature is not entirely in its block.\n");
    goto out;
  }
  ...
  if (!avb_safe_add(&pubkey_end, h.public_key_offset, h.public_key_size) ||
    ...
    avb_warning("Public key is not entirely in its block.\n");
    goto out;
  }

  /* Ensure algorithm field is supported. */
  if (h.algorithm_type >= _AVB_ALGORITHM_NUM_TYPES) {
    avb_warning("Invalid or unknown algorithm.\n");
    goto out;
  }
  ...
  if (h.hash_size != algorithm->hash_len) {
    avb_warning("Embedded hash has wrong size.\n");
    goto out;
  }
  ...
  switch (h.algorithm_type) {
    ...
    case AVB_ALGORITHM_TYPE_SHA256_RSA8192:
      ...
      avb_sha256_update(&sha256_ctx, auxilary_block,
    ...
    case AVB_ALGORITHM_TYPE_SHA512_RSA8192:
      ...
      avb_sha512_update(&sha512_ctx, header_block,
                        sizeof(AvbVBMetaImageHeader));
      avb_sha512_update(&sha512_ctx, auxilary_block,
                        h.auxilary_data_block_size);
      computed_hash = avb_sha512_final(&sha512_ctx);
      break;
    default:
      avb_warning("Unknown algorithm %d.\n", h.algorithm_type);
      goto out;
  }

  if (avb_safe_memcmp(authentication_block + h.hash_offset, computed_hash,
                      h.hash_size) != 0) {
    avb_warning("Hash does not match!\n");
    ret = AVB_VBMETA_VERIFY_RESULT_HASH_MISMATCH;
    goto out;
  }

  verification_result =
      avb_rsa_verify(auxilary_block + h.public_key_offset, h.public_key_size,
...
void avb_vbmeta_image_header_to_host_byte_order(const AvbVBMetaImageHeader* src,
  ...
  dest->auxilary_data_block_size = avb_be64toh(dest->auxilary_data_block_size);
