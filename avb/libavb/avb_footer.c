...
int avb_footer_validate_and_byteswap(const AvbFooter* src, AvbFooter* dest) {
  ...
  if (avb_safe_memcmp(dest->magic, AVB_FOOTER_MAGIC, AVB_FOOTER_MAGIC_LEN) !=
      0) {
    avb_warning("Footer magic is incorrect.\n");
    return 0;
  }
  ...
  if (dest->version_major > AVB_FOOTER_MAJOR_VERSION) {
    avb_warning("No support for footer version %d.\n", dest->version_major);
    return 0;
  }
