...
class MyOps {
 public:
  ...
  void set_stored_is_unlocked(bool stored_is_unlocked) {
    stored_is_unlocked_ = stored_is_unlocked;
  }

  AvbIOResult read_from_partition(const char* partition, void* buf,
    ...
    if (offset < 0) {
      ...
      if (!base::GetFileSize(path, &file_size)) {
        avb_warning("Error getting size of file '%s'\n", path.value().c_str());
    ...
    if (fd < 0) {
      avb_warning("Error opening file '%s': %s\n", path.value().c_str(),
                  strerror(errno));
      return AVB_IO_RESULT_ERROR_IO;
    }
    if (lseek(fd, offset, SEEK_SET) != offset) {
      avb_warning("Error seeking to pos %zd in file %s: %s\n", offset,
    ...
    if (num_read < 0) {
      avb_warning("Error reading %zd bytes from pos %" PRId64
                  " in file %s: %s\n",
  ...
  AvbIOResult write_to_partition(const char* partition, const void* buf,
                                 int64_t offset, size_t num_bytes) {
    base::FilePath path =
        partition_dir_.Append(std::string(partition)).AddExtension("img");
    ...
    if (num_written < 0) {
      avb_warning("Error writing %zd bytes at pos %" PRId64 " in file %s: %s\n",
                  num_bytes, offset, path.value().c_str(), strerror(errno));
      close(fd);
      return AVB_IO_RESULT_ERROR_IO;
    }
  ...
  int validate_public_key(AvbOps* ops, const uint8_t* public_key_data,
  ...
  int read_rollback_index(AvbOps* ops, size_t rollback_index_slot,
                          uint64_t* out_rollback_index) {
    if (rollback_index_slot >= stored_rollback_indexes_.size()) {
      avb_warning("No rollback index for slot %zd (has %zd slots).\n",
  ...
  int write_rollback_index(AvbOps* ops, size_t rollback_index_slot,
                           uint64_t rollback_index) {
    avb_warning("write_rollback_index not yet implemented.\n");
    return 0;
  }

  int read_is_unlocked(AvbOps* ops, int* out_is_unlocked) {
    *out_is_unlocked = (stored_is_unlocked_ ? 1 : 0);
    return 1;
  }

  int get_unique_guid_for_partition(AvbOps* ops, const char* partition,
...
static int my_ops_validate_public_key(AvbOps* ops,
                                      const uint8_t* public_key_data,
                                      size_t public_key_length) {
  return ((MyAvbOps*)ops)
      ->my_ops->validate_public_key(ops, public_key_data, public_key_length);
}

static int my_ops_read_rollback_index(AvbOps* ops, size_t rollback_index_slot,
                                      uint64_t* out_rollback_index) {
  return ((MyAvbOps*)ops)
...
static int my_ops_write_rollback_index(AvbOps* ops, size_t rollback_index_slot,
                                       uint64_t rollback_index) {
  return ((MyAvbOps*)ops)
      ->my_ops->write_rollback_index(ops, rollback_index_slot, rollback_index);
}

static int my_ops_read_is_unlocked(AvbOps* ops, int* out_is_unlocked) {
  return ((MyAvbOps*)ops)->my_ops->read_is_unlocked(ops, out_is_unlocked);
}

static int my_ops_get_unique_guid_for_partition(AvbOps* ops,
...
MyOps::MyOps() {
  ...
  avb_ops_->parent.validate_public_key = my_ops_validate_public_key;
  ...
  avb_ops_->parent.read_is_unlocked = my_ops_read_is_unlocked;
...
class AvbSlotVerifyTest : public BaseAvbToolTest {
 public:
  ...
  virtual void SetUp() override {
    ...
    ops_.set_stored_is_unlocked(false);
  }
...
TEST_F(AvbSlotVerifyTest, BasicUnlocked) {
  ...
  ops_.set_stored_is_unlocked(true);
...
TEST_F(AvbSlotVerifyTest, CorruptedImage) {
  ...
  EXPECT_EQ(AVB_IO_RESULT_OK, ops_.avb_ops()->write_to_partition(
                                  ops_.avb_ops(), "vbmeta_a", corrupt_data,
...
TEST_F(AvbSlotVerifyTest, HashDescriptorInVBMetaCorruptBoot) {
  ...
  uint8_t corrupt_data[4] = {0xff, 0xff, 0xff, 0xff};
  EXPECT_EQ(AVB_IO_RESULT_OK, ops_.avb_ops()->write_to_partition(
                                  ops_.avb_ops(), "boot_a", corrupt_data,
