...
class MyOps {
 public:
  ...
  AvbIOResult read_from_partition(const char* partition, void* buf,
    ...
    if (offset < 0) {
      ...
      if (!base::GetFileSize(path, &file_size)) {
        avb_error("Error getting size of file '%s'\n", path.value().c_str());
    ...
    if (fd < 0) {
      avb_error("Error opening file '%s': %s\n", path.value().c_str(),
                strerror(errno));
      return AVB_IO_RESULT_ERROR_IO;
    }
    if (lseek(fd, offset, SEEK_SET) != offset) {
      avb_error("Error seeking to pos %zd in file %s: %s\n", offset,
    ...
    if (num_read < 0) {
      avb_error("Error reading %zd bytes from pos %" PRId64 " in file %s: %s\n",
  ...
  AvbIOResult write_to_partition(const char* partition, const void* buf,
    ...
    if (num_written < 0) {
      avb_error("Error writing %zd bytes at pos %" PRId64 " in file %s: %s\n",
                num_bytes, offset, path.value().c_str(), strerror(errno));
      close(fd);
      return AVB_IO_RESULT_ERROR_IO;
    }
  ...
  int read_rollback_index(AvbOps* ops, size_t rollback_index_slot,
    ...
    if (rollback_index_slot >= stored_rollback_indexes_.size()) {
      avb_error("No rollback index for slot %zd (has %zd slots).\n",
  ...
  int write_rollback_index(AvbOps* ops, size_t rollback_index_slot,
    ...
    avb_error("write_rollback_index not yet implemented.\n");
    return 0;
  }
