...
{
  public:
    ImageExtractor(unique_fd&& image_fd,
...
int main(int argc, char* argv[]) {
    ...
    ImageExtractor extractor(std::move(fd), std::move(metadata), std::move(partitions),
                             output_dir);
...
ImageExtractor::ImageExtractor(unique_fd&& image_fd,
                               std::unique_ptr<LpMetadata>&& metadata,
                               std::unordered_set<std::string>&& partitions,
                               const std::string& output_dir)
...
bool ImageExtractor::ExtractExtent(const LpMetadataExtent& extent, int output_fd) {
    ...
    while (remaining_bytes) {
        ...
        size_t bytes_to_read = (remaining_bytes < kChunkSize)
                               ? static_cast<size_t>(remaining_bytes) : kChunkSize;
