...
static int usage(int /* argc */, char* argv[]) {
    fprintf(stderr,
            ...
            "                                a flashable image.\n"
            "   -T,--footer                  Add Footer that records the physical address of every partition's vbmeta\n"
            "\n"
...
int main(int argc, char* argv[]) {
    ...
    if (!images.empty() || force_full_image) {
        if (block_devices.size() == 1) {
            if(footer == true){
                if (!WriteToImageFileWithFooter(output_path.c_str(), *metadata.get(), block_size, images,
                                    output_sparse)) {
                    return EX_CANTCREAT;
                }
            }else{
