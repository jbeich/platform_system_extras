...
static bool MergeFsUsage([[maybe_unused]] DynamicPartitionsDeviceInfoProto* proto,
                         [[maybe_unused]] std::ostream& cerr) {
#ifdef __ANDROID__
    using namespace std::string_literals;
    // A list of candidate dynamic partitions. This is a list of interesting partitions that
    // we want to include in the report; they may be static or dynamic.
    // clang-format off
    static constexpr const char* partitions[] = {
            "system", "vendor", "product", "product_services", "odm"};
    // clang-format on

    // scan /proc/mounts to determine if a partition is on a device or not.
    Fstab mounts;
    if (!ReadFstabFromFile("/proc/mounts", &mounts)) {
        cerr << "Cannot read /proc/mounts\n";
        return false;
    }
    for (auto partition : partitions) {
        // system is mounted as "/" in /proc/mounts.
        std::string mount_point;
        if (partition == "system"s) {
            mount_point = "/";
        } else {
            mount_point = "/"s + partition;
        }
        ...
        if (entry->fs_type == "overlay" || entry->fs_type == "overlayfs") {
            cerr << "Warning: " << partition
                 << " is mounted with overlayfs, fs_size and fs_used may be inaccurate.\n";
        }

        struct statvfs vst;
        if (statvfs(mount_point.c_str(), &vst) == -1) {
            continue;
        }
...
public:
    android::base::unique_fd Open(const std::string& path, int flags) const override {
        ...
        auto real = std::unique_ptr<char, decltype(&free)>(realpath(path.c_str(), nullptr), &free);
        if (real) {
            android::base::unique_fd fd;
#ifdef __ANDROID__
            fd.reset(android_get_control_file(real.get()));
            if (fd >= 0) return fd;
#endif
            fd.reset(open(real.get(), flags));
            if (fd >= 0) return fd;
        }
