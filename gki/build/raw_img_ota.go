...
type dependencyTag struct {
	...
	// Name of the image as specified in Image_goals.
	imageName string
}
...
func init() {
	pctx.HostBinToolVariable("ota_from_raw_image", "ota_from_raw_image")
	pctx.HostBinToolVariable("brillo_update_payload", "brillo_update_payload")
	pctx.HostBinToolVariable("delta_generator", "delta_generator")

	// Intentionally not register this module so that it can only be constructed
	// on the fly.
}
...
// Declare a rule that generates OTA payload from a raw image.
...
func (r *rawImageOta) DepsMutator(ctx android.BottomUpMutatorContext) {
	...
	for module, _ := range r.goalToImage(ctx) {
		ctx.AddDependency(ctx.Module(), rawImageTag, module)
	}
