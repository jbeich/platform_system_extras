...
func gkiApexMutator(mctx android.LoadHookContext, g *gkiApex) {
	...
	// Check if modules should be enabled according to board variables.
	enabled := g.bootImgHasRules(mctx) && g.kmiVersionMatches(mctx)
	...
	mctx.CreateModule(phony.PhonyFactory, &moduleCommonProperties{
