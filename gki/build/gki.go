...
func gkiApexMutator(mctx android.LoadHookContext, g *gkiApexRule) {
	...
	// Even though ota_from_raw_image can technically generate payload_properties.txt as well,
	// the generation of payload_properties.txt is in another genrule because a prebuilt_etc
	// module only accept a genrule with one output file.
	payloadGenDepTools := []string{
		"brillo_update_payload",
		"delta_generator",
	}
	toolArg := ""
	for _, tool := range payloadGenDepTools {
		toolArg += fmt.Sprintf("$(location %s) ", tool)
	}
	...
	mctx.CreateModule(genrule.GenRuleFactory, &genRuleProperties{
		...
		Tools: append(payloadGenDepTools, "ota_from_raw_image"),
		Srcs:  []string{":" + bootImage},
		Out:   []string{"payload.bin"},
		Cmd:   proptools.StringPtr(`$(location ota_from_raw_image) --tools ` + toolArg + `--out $(out) boot:$(in)`),
	})
