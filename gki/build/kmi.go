...
func parseKmiVersion(f string) (*kmiVersion, error) {
	matches := reKmi.FindAllStringSubmatch(f, 4)

	if matches == nil {
		return nil, errors.New("Poorly formed KMI version: '" + f + "' must match '" + reKmi.String() + "'")
	}
