...
BOOT_PERFETTO_TRACE_FILE = "/data/misc/perfetto-traces/boottrace.perfetto-trace"
...
class BootCommandExecutor(ProfilerCommandExecutor):
  ...
  def prepare_device_for_run(self, command, device, run):
    device.root_device()
    device.remove_file(BOOT_PERFETTO_TRACE_FILE)
  ...
  def retrieve_perf_data(self, command, device, host_file):
    device.pull_file(BOOT_PERFETTO_TRACE_FILE, host_file)
