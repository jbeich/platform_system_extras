...
class TorqUnitTest(unittest.TestCase):
  ...
  @patch("torq.os.path.isdir")
  def test_valid_out_dir_path(self, mock_is_dir):
  ...
  @patch("torq.os.path.isfile")
  @patch("torq.os.path.isdir")
  def test_invalid_out_dir_paths(self, mock_is_dir, mock_is_file):
  ...
  def test_valid_between_dur_ms_values(self):
    parser = self.set_up_parser(("torq.py -r 2 --between-dur-ms %d"
                                 % DEFAULT_DUR_MS))

    self.validate_command(parser, "profiler")

  def test_invalid_between_dur_ms_values(self):
  ...
  @patch("torq.os.path.isfile")
  def test_valid_perfetto_config_path(self, mock_is_file):
    mock_is_file.side_effect = (lambda x: x in ["mock-file"])
  ...
  @patch("torq.os.path.isfile")
  @patch("torq.os.path.isdir")
  def test_invalid_perfetto_config_path(self, mock_is_dir, mock_is_file):
    mock_is_dir.side_effect = (lambda x: x in ["mock-directory", "."])
    mock_is_file.side_effect = (lambda x: x in ["mock-file"])
  ...
  def test_valid_hw_config_show_values(self):
  ...
  def test_invalid_hw_config_show_values(self):
    parser = self.set_up_parser("torq.py config show fake-config")

    with self.assertRaises(SystemExit):
      parser.parse_args()

  def test_valid_hw_config_pull_values(self):
  ...
  def test_invalid_hw_config_pull_values(self):
  ...
  def test_invalid_set_subcommands(self):
  ...
  def test_invalid_config_subcommands(self):
  ...
  def test_invalid_mixing_of_profiler_and_hw_subcommand(self):
  ...
  def test_invalid_mixing_of_profiler_and_config_subcommand(self):
