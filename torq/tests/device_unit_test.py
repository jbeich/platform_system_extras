...
class DeviceUnitTest(unittest.TestCase):
  ...
  @mock.patch.object(subprocess, "run", autospec=True)
  def test_kill_perfetto_trace_and_get_perfetto_trace_pid_fails_error(self,
      mock_subprocess_run):
    mock_subprocess_run.side_effect = TEST_EXCEPTION
    adbDevice = AdbDevice(TEST_DEVICE_SERIAL)

    with self.assertRaises(Exception) as e:
      adbDevice.kill_perfetto_trace()

    self.assertEqual(str(e.exception), TEST_FAILURE_MSG)

  @mock.patch.object(subprocess, "run", autospec=True)
  def test_kill_perfetto_trace_and_kill_perfetto_trace_pid_fails_error(self,
      mock_subprocess_run):
    mock_subprocess_run.side_effect = [
        self.generate_mock_completed_process(TEST_PROCESS_ID_OUTPUT),
        TEST_EXCEPTION]
  ...
  @mock.patch.object(subprocess, "run", autospec=True)
  def test_kill_perfetto_trace_success(self, mock_subprocess_run):
    mock_subprocess_run.side_effect = [
