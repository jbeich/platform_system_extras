...
class DeviceUnitTest(unittest.TestCase):
  ...
  def create_mock_list_of_adb_devices_string(self, devices, stdout_string):
    if len(devices) > 0:
  ...
  def generate_adb_devices_result(self, devices):
    devices = [device.encode('utf-8') for device in devices]
    stdout_string = b'List of devices attached\n'
    return self.create_mock_list_of_adb_devices_string(devices, stdout_string)

  def generate_adb_devices_and_adb_not_started_result(self, devices):
    devices = [device.encode('utf-8') for device in devices]
  ...
  def test_get_adb_devices_returns_devices_and_adb_not_started(self,
      mock_subprocess_run):
    mock_subprocess_run.return_value = (
        self.generate_adb_devices_and_adb_not_started_result(
            [MOCK_DEVICE_SERIAL, MOCK_DEVICE_SERIAL2]))
    adbDevice = AdbDevice(None)
