...
class DeviceUnitTest(unittest.TestCase):
  ...
  @mock.patch.object(subprocess, "run", autospec=True)
  def test_package_exists_and_not_on_device_error(self, mock_subprocess_run):
    mock_subprocess_run.return_value = self.mock_packages()
    adbDevice = AdbDevice(TEST_DEVICE_SERIAL)
  ...
  @mock.patch.object(subprocess, "run", autospec=True)
  def test_is_package_running_success(self, mock_subprocess_run):
    mock_subprocess_run.return_value = self.generate_mock_completed_process()
    adbDevice = AdbDevice(TEST_DEVICE_SERIAL)

    running_error = adbDevice.is_package_running(TEST_PACKAGE_1)

    self.assertEqual(running_error, False)

  @mock.patch.object(subprocess, "run", autospec=True)
  def test_is_package_running_and_running_error(self, mock_subprocess_run):
    mock_subprocess_run.return_value = self.generate_mock_completed_process(
        TEST_PROCESS_ID_OUTPUT)
    adbDevice = AdbDevice(TEST_DEVICE_SERIAL)
  ...
  @mock.patch.object(subprocess, "run", autospec=True)
  def test_package_running_command_failure(self, mock_subprocess_run):
    mock_subprocess_run.side_effect = TEST_EXCEPTION
    adbDevice = AdbDevice(TEST_DEVICE_SERIAL)
  ...
  @mock.patch.object(subprocess, "run", autospec=True)
  def test_start_package_and_is_service_app_error(self, mock_subprocess_run):
    mock_subprocess_run.return_value = self.generate_mock_completed_process(
  ...
  @mock.patch.object(subprocess, "run", autospec=True)
  def test_start_package_command_failure(self, mock_subprocess_run):
    mock_subprocess_run.side_effect = TEST_EXCEPTION
    adbDevice = AdbDevice(TEST_DEVICE_SERIAL)
  ...
  @mock.patch.object(subprocess, "run", autospec=True)
  def test_perform_force_stop_package_failure(self, mock_subprocess_run):
    mock_subprocess_run.side_effect = TEST_EXCEPTION
    adbDevice = AdbDevice(TEST_DEVICE_SERIAL)
