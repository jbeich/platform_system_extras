...
class DeviceUnitTest(unittest.TestCase):
  ...
  def test_root_device_failure(self, mock_subprocess_run):
    ...
    self.assertEqual(str(e.exception), ("Command 'adb -s %s root' failed. %s"
  ...
  def test_remove_file_success(self, mock_subprocess_run):
    ...
    error = adbDevice.remove_file(MOCK_TRACE_PATH)
  ...
  def test_remove_file_failure(self, mock_subprocess_run):
    ...
    with self.assertRaises(Exception) as e:
      adbDevice.remove_file(MOCK_TRACE_PATH)

    self.assertEqual(str(e.exception), ("Command 'adb -s %s shell rm "
  ...
  def test_start_perfetto_trace_failure(self, mock_subprocess_popen):
    ...
    self.assertEqual(str(e.exception), ("Command 'adb -s %s shell perfetto -c -"
  ...
  def test_get_perfetto_trace_data_failure(self, mock_subprocess_run):
    ...
    with self.assertRaises(Exception) as e:
      adbDevice.get_perfetto_trace_data(MOCK_TRACE_PATH, MOCK_OUT_DIR)

    self.assertEqual(str(e.exception), ("Command 'adb -s %s pull %s"
