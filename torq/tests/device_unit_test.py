...
class DeviceUnitTest(unittest.TestCase):
  ...
  @staticmethod
  def generate_mock_completed_process(process_output=None):
    stdout_string = b'\n'
    if process_output is not None:
      stdout_string = process_output.encode('utf-8') + stdout_string
  ...
  def test_is_boot_completed_and_is_completed(self, mock_subprocess_run):
    mock_subprocess_run.return_value = (
        self.generate_mock_completed_process(TEST_COMPLETED_BOOT_OUTPUT))
    adbDevice = AdbDevice(TEST_DEVICE_SERIAL)
