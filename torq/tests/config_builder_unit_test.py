...
class ConfigBuilderUnitTest(unittest.TestCase):
  ...
  def test_default_config_builder_setting_valid_dur_ms(self):
    self.mock_command.dur_ms = TEST_DUR_MS_9000
    self.mock_command.excluded_ftrace_events = None
    self.mock_command.included_ftrace_events = None

    config, error = build_default_config(self.mock_command)

    self.assertEqual(error, None)
    self.assertEqual(config, DEFAULT_CONFIG_9000_DUR_MS)

  def test_default_config_builder_setting_invalid_dur_ms(self):
    self.mock_command.dur_ms = None
    self.mock_command.excluded_ftrace_events = None
    self.mock_command.included_ftrace_events = None

    with self.assertRaises(ValueError) as e:
      build_default_config(self.mock_command)

    self.assertEqual(str(e.exception), ("Cannot create config because a valid"
                                        " dur_ms was not set."))

  def test_default_config_builder_removing_valid_excluded_ftrace_events(self):
    self.mock_command.dur_ms = DEFAULT_DUR_MS
    self.mock_command.excluded_ftrace_events = ["power/suspend_resume",
  ...
  def test_default_config_builder_removing_invalid_excluded_ftrace_events(self):
    self.mock_command.dur_ms = DEFAULT_DUR_MS
    self.mock_command.excluded_ftrace_events = ["invalid_ftrace_event"]
    self.mock_command.included_ftrace_events = None
  ...
  def test_default_config_builder_adding_valid_included_ftrace_events(self):
    self.mock_command.dur_ms = DEFAULT_DUR_MS
    self.mock_command.excluded_ftrace_events = None
    self.mock_command.included_ftrace_events = ["mock_ftrace_event1",
                                                "mock_ftrace_event2"]
  ...
  def test_default_config_builder_adding_invalid_included_ftrace_events(self):
    self.mock_command.dur_ms = DEFAULT_DUR_MS
    self.mock_command.excluded_ftrace_events = None
    self.mock_command.included_ftrace_events = ["power/suspend_resume"]
  ...
  def test_custom_config_builder_extracting_dur_ms(self):
    self.mock_command.dur_ms = None
    self.mock_command.perfetto_config = None

    config, error = build_custom_config(self.mock_command)

    self.assertEqual(error, None)
    self.assertEqual(config, f"<<EOF\n\n{CUSTOM_CONFIG_9000_DUR_MS}\n\n\nEOF")
    self.assertEqual(self.mock_command.dur_ms, TEST_DUR_MS_9000)

  @mock.patch("builtins.open", mock.mock_open(
      read_data=CUSTOM_CONFIG_NO_DUR_MS))
  def test_custom_config_builder_injecting_dur_ms(self):
    self.mock_command.dur_ms = DEFAULT_DUR_MS
    self.mock_command.perfetto_config = None
    duration_string = "duration_ms: " + str(self.mock_command.dur_ms)
  ...
  def test_custom_config_builder_perfetto_config_parsing_error(self,
      mock_open_file):
    self.mock_command.dur_ms = None
    self.mock_command.perfetto_config = "test-path"
    ...
    self.assertEqual(error.message, ("Failed to parse custom perfetto-config on"
                                     "local file path: %s. %s"
                                     % (self.mock_command.perfetto_config,
                                        TEST_FAILURE_MSG)))
