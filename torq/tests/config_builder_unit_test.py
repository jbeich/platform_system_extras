...
EOF
  ...
  def test_config_builder_setting_valid_dur_ms(self):
    ...
    config, error = ConfigBuilder.build_default_config(mock_command)

    self.assertEqual(error, None)
    self.assertEqual(config, textwrap.dedent(
        ConfigBuilderUnitTest.default_config_9000_dur_ms))
  ...
  def test_config_builder_setting_invalid_dur_ms(self):
    ...
    with self.assertRaises(ValueError) as e:
      ConfigBuilder.build_default_config(mock_command)
  ...
  def test_config_builder_removing_valid_excluded_ftrace_events(self):
    ...
    self.assertEqual(error, None)
    self.assertEqual(config, textwrap.dedent(
        ConfigBuilderUnitTest.default_config_excluded_ftrace_events))
  ...
  def test_config_builder_adding_valid_included_ftrace_events(self):
    ...
    self.assertEqual(error, None)
    self.assertEqual(config, textwrap.dedent(
        ConfigBuilderUnitTest.default_config_included_ftrace_events))
  ...
  def test_config_builder_adding_invalid_included_ftrace_events(self):
    ...
    self.assertEqual(error.message, ("Cannot add ftrace event"
                                     ...
                                     " events."))
    self.assertEqual(error.suggestion, ("Please do not specify one of the"
