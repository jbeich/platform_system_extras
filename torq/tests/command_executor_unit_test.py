...
class UserSwitchCommandExecutorUnitTest(unittest.TestCase):
  ...
  def setUp(self):
    self.command = ProfilerCommand(
        "profiler", "user-switch", None, DEFAULT_OUT_DIR, DEFAULT_DUR_MS,
        None, None, None, DEFAULT_PERFETTO_CONFIG, None, None, [], [], None,
        None)
    self.mock_device = mock.create_autospec(AdbDevice, instance=True,
                                            serial=TEST_SERIAL)
  ...
  def mock_user_switch(self, user):
    self.current_user = user

  @mock.patch.object(subprocess, "Popen", autospec=True)
  def test_execute_command_user_switch_all_users_different_ui_success(self,
      mock_process):
    with mock.patch("command_executor.open_trace", autospec=True):
      self.command.runs = 1
      ...
      self.mock_device.perform_user_switch.side_effect = (
          lambda user: self.mock_user_switch(user))
  ...
  @mock.patch.object(subprocess, "Popen", autospec=True)
  def test_execute_command_user_switch_all_users_different_no_ui_success(self,
      mock_process):
    self.command.runs = 1
