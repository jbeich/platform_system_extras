...
class ProfilerCommandExecutorUnitTest(unittest.TestCase):
  ...
  def test_execute_command_one_run_and_use_ui_success(self, mock_process,
      mock_open_trace, mock_terminal_output):
    ...
    self.mock_device.root_device.return_value = None
    self.mock_device.remove_file.return_value = None
    self.mock_device.start_perfetto_trace.return_value = mock_process
    mock_process.wait.return_value = None
    self.mock_device.pull_file.return_value = None
    mock_open_trace.return_value = None

    error = self.executor.execute_command(self.mock_command, self.mock_device)
  ...
  def test_execute_command_prepare_device_for_run_root_failure(self):
    ...
    with self.assertRaises(Exception) as e:
      self.executor.execute_command(self.mock_command, self.mock_device)
