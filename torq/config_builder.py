...
class ConfigBuilder:
  """
  Class used to build default, lightweight, memory, and custom perfetto
  configs. Users can use the default dur-ms or overwrite it. For the default,
  ...
  def validate_dur_ms(self, dur_ms):
    if dur_ms is None:
      raise Exception("Cannot create config because a valid dur_ms was not"
                      " passed into the ConfigBuilder constructor.")

  def create_ftrace_events_string(self, predefined_ftrace_events,
      exclude_ftrace_events, include_ftrace_events):
  ...
  def build_default_config(self, command):
    self.validate_dur_ms(command.dur_ms)
    ...
    ftrace_events_string, error = self.create_ftrace_events_string(
