import argparse
from command import Command, ProfilerCommand, HWCommand, ConfigCommand
...
def create_parser():
  ...
  parser.add_argument('--perfetto-config', default='default',
                      help=('Filepath of the perfetto config to use '
                            ' predefined configs.'))
  parser.add_argument('--between-dur-ms', type=int, default=10000,
...
def verify_args_valid(args):
  ...
  if args.between_dur_ms != 10000 and args.runs == 1:
    # Test has failed, so we return an error.
    return None, ValidationError(
  ...
  if (args.subcommands == "hw" and args.hw_subcommand == "set" and
    ...
    if index == -1 or index != len(args.memory) - 1:
      return None, ValidationError(
  ...
  if args.subcommands == "config" and args.config_subcommand is None:
    return None, ValidationError(
        ("Command is invalid because torq config cannot be called "
         " a subcomand."),
        ("For instance, using of of the following subcommands:"
...
def create_profiler_command(args):
  ...
  command = ProfilerCommand(args.event, args.profiler, args.out_dir,
                            args.dur_ms,
...
def create_hw_command(args):
  ...
  if args.hw_subcommand == "set":
    command = HWCommand(args.hw_subcommand, args.config, args.num_cpus,
                        args.memory)
  else:
...
def create_config_command(args):
  ...
  if args.config_subcommand == "pull":
    command = ConfigCommand(args.config_subcommand, args.config_name,
                            args.file_path)
  if args.config_subcommand == "show":
    command = ConfigCommand(args.config_subcommand, args.config_name, None)
  if args.config_subcommand == "list":
    command = ConfigCommand(args.config_subcommand, None, None)
  return command
