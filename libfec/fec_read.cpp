        ...
        bool use_erasures, uint8_t *ecc_data, size_t *errors)
{
    ...
    for (int i = 0; i < e->rsn; ++i) {
        ...
        if (unlikely(interleaved >= f->ecc.start)) {
            memset(bbuf, 0, FEC_BLOCKSIZE);
        } else {
            check(interleaved + FEC_BLOCKSIZE <= f->ecc.start);

            if (!__raw_pread(f, bbuf, FEC_BLOCKSIZE, interleaved)) {
    ...
    for (int i = 0; i < FEC_BLOCKSIZE; ++i) {
        /* copy parity data */
        if (!__raw_pread(f, &ecc_data[i * FEC_RSM + e->rsn], e->roots,
    ...
    if (*errors) {
        warn("RS block %" PRIu64 ": corrected %zu errors", rsb, *errors);
    }
        ...
        uint64_t offset, size_t *errors)
{
    ...
    void *rs = init_rs_char(FEC_PARAMS(f->ecc.roots));
    ...
    uint8_t *ecc_data = new uint8_t[FEC_RSM * FEC_BLOCKSIZE];
        ...
        uint64_t offset, size_t *errors)
{
    ...
    while (left > 0) {
        ...
        if (!__raw_pread(f, data, FEC_BLOCKSIZE, curr * FEC_BLOCKSIZE)) {
            error("failed to read: %s", strerror(errno));
            goto err;
        }
...
corrected:
        ...
        if (f->mode & O_RDWR) {
            if (!__raw_pwrite(f, data, FEC_BLOCKSIZE,
                    curr * FEC_BLOCKSIZE)) {
...
bool __raw_pread(fec_handle *f, void *buf, size_t count, uint64_t offset)
{
    check(f);
    check(buf);
...
bool __raw_pwrite(fec_handle *f, const void *buf, size_t count, uint64_t offset)
{
    check(f);
    check(buf);
        ...
        uint64_t offset)
{
    ...
    if (__raw_pread(f, buf, count, offset)) {
        return count;
    }
