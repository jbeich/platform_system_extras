...
static uint64_t get_verity_size(uint64_t size, int)
{
    return VERITY_METADATA_SIZE +
           hashtree_get_size(size, NULL, NULL, SHA256_DIGEST_LENGTH);
}
...
static int load_verity(fec_handle *f)
{
    ...
    if (rc == 0) {
        ...
        offset += hashtree_get_size(offset, NULL, NULL, SHA256_DIGEST_LENGTH);
...
static int parse_avb_image(fec_handle *f, const AvbFooter &footer) {
    ...
    // vbmeta.
    // TODO(xunchang) Add parser for hash footer.
    ...
    // Check both the salt & root are within the range of vbmeta.
    ...
    check(salt_offset < vbmeta_size);
    check(salt_offset < vbmeta_size - hashtree_descriptor.salt_len);
    check(root_offset < vbmeta_size);
    check(root_offset < vbmeta_size - hashtree_descriptor.root_digest_len);
    ...
    check(hashtree_descriptor.fec_offset <=
          vbmeta_offset - hashtree_descriptor.fec_size);
    ...
    // The hashtree descriptor doesn't carry the hash for eec data.
        ...
        int roots)
{
    ...
    if (f->size > AVB_FOOTER_SIZE) {
        ...
        if (avb_footer_validate_and_byteswap(&footer_read, &verified_footer)) {
            if (parse_avb_image(f.get(), verified_footer) != 0) {
