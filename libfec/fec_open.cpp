...
/* computes the ecc offset for a file with size `f->size' and `f->ecc.roots'
   parity bytes. */
static int find_ecc_offset(fec_handle *f, uint64_t *offset)
{
    ...
    return find_offset(f->size, f->ecc.roots, offset,
                [](uint64_t total_size, int roots) {
                    /* very close, but nearly always slightly too small
                       estimate */
                    return fec_round_up((total_size / (FEC_RSM - roots)) *
                                roots, FEC_BLOCKSIZE);
                },
                fec_ecc_get_size);
}
...
static int find_verity_offset(fec_handle *f, uint64_t *offset)
{
    ...
    if (f->ecc.start) {
        ...
        check(f->ecc.start >= FEC_BLOCKSIZE);
...
static int parse_ecc_header(fec_handle *f, uint64_t offset)
{
    ...
    if (f->size < header.inp_size ||
            f->size < header.inp_size + header.fec_size + FEC_BLOCKSIZE) {
