...
static inline uint64_t get_min_ecc_size(uint64_t total_size, int roots)
{
    return total_size * roots / FEC_RSM;
}

static int get_ecc_offset(uint64_t file_size, int roots, uint64_t *offset)
{
    uint64_t lo, mi, hi, total;
    ...
    mi = get_min_ecc_size(file_size, roots);
    lo = file_size - mi * 2;
    hi = file_size - mi / 2;

    while (lo < hi) {
        mi = ((hi + lo) / (2 * FEC_BLOCKSIZE)) * FEC_BLOCKSIZE;
        total = mi + fec_ecc_get_size(mi, roots);
...
err:
    error("could not determine offset");
    errno = ERANGE;
    return -1;
}
...
static int parse_ecc_header(fec_handle *f, uint64_t offset)
{
    ...
    if (!__raw_pread(f, &header, sizeof(fec_header), offset)) {
    ...
    f->ecc.start = FEC_BLOCKSIZE + offset;
    ...
    while (n < f->ecc.size) {
        ...
        if (!__raw_pread(f, buf, len, f->ecc.start + n)) {
...
static int load_ecc(fec_handle *f)
{
    ...
    /* best case: we can locate ecc without relying on file content */
    if (get_ecc_offset(f->size, f->ecc.roots, &offset) == 0 &&
            parse_ecc_header(f, offset) == 0) {
        debug("found at %" PRIu64, offset);
        return 0;
    }
    ...
    if (rc == 0) {
        ...
        /* if verity metadata exists, ecc data starts after it, give it
           a try */
        offset = VERITY_METADATA_SIZE +
                    verity_get_size(offset, NULL, NULL);
    ...
    error("failed to locate ecc data");
    errno = ENODATA;
    return -1;
}
