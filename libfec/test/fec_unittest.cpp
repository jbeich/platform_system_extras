...
class FecUnitTest : public ::testing::Test {
   protected:
    ...
    void BuildAndAppendsVerityMetadata(const std::string &hash_name) {
        BuildHashtree(hash_name);
...
TEST_F(FecUnitTest, LoadVerityImage_ParseVerity) {
    ...
    BuildAndAppendsVerityMetadata("sha256");
    ASSERT_TRUE(android::base::WriteFully(verity_image.fd, image_.data(),
                                          image_.size()));

    struct fec_handle *handle = nullptr;
    ASSERT_EQ(0, fec_open(&handle, verity_image.path, O_RDWR, FEC_FS_EXT4, 2));
