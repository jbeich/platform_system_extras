...
class FecUnitTest : public ::testing::Test {
  protected:
    ...
    void BuildAndAppendsVerityMetadata() {
        ...
        image_.insert(image_.end(), hashtree_content_.begin(), hashtree_content_.end());
        ...
        verity_header_ = {0xb001b001, 0, {}, static_cast<unsigned int>(verity_table_.size())};

        // Construct the verity metadata with header, table, and padding.
        constexpr auto VERITY_META_SIZE = 8 * 4096;
        image_.insert(image_.end(), reinterpret_cast<uint8_t*>(&verity_header_),
                      reinterpret_cast<uint8_t*>(&verity_header_) + sizeof(verity_header_));
        image_.insert(image_.end(), verity_table_.data(),
                      verity_table_.data() + verity_table_.size());
        std::vector<uint8_t> padding(
                VERITY_META_SIZE - sizeof(verity_header_) - verity_table_.size(), 0);
    ...
    static void BuildAndAppendsEccImage(const std::string& image_name,
        ...
        std::vector<std::string> cmd = {"fec", "--encode", "--roots", "2", image_name, fec_name};
        ASSERT_EQ(0, std::system(android::base::Join(cmd, ' ').c_str()));
    }

    void AddAvbHashtreeFooter(const std::string& image_name, std::string algorithm = "sha256") {
        salt_ = std::vector<uint8_t>(64, 10);
...
TEST_F(FecUnitTest, LoadVerityImage_ParseVerity) {
    ...
    ASSERT_TRUE(android::base::WriteFully(verity_image.fd, image_.data(), image_.size()));
    ...
    ASSERT_EQ(1024 * 1024 + hashtree_content_.size(), handle->verity.metadata_start);
    ...
    ASSERT_EQ(std::vector<uint8_t>(hashtree_content_.begin() + 4096, hashtree_content_.end()),
              handle->hashtree().hash_data);

    uint64_t hash_size = verity_get_size(handle->hashtree().data_blocks * FEC_BLOCKSIZE, nullptr,
...
TEST_F(FecUnitTest, VerityImage_FecRead) {
    ...
    ASSERT_TRUE(android::base::WriteFully(verity_image.fd, corruption.data(), corruption.size()));
...
TEST_F(FecUnitTest, LoadAvbImage_HashtreeFooter) {
    ...
    ASSERT_TRUE(android::base::WriteFully(avb_image.fd, image_.data(), image_.size()));
...
TEST_F(FecUnitTest, LoadAvbImage_CorrectHashtree) {
    ...
    ASSERT_TRUE(android::base::WriteFully(avb_image.fd, corruption.data(), corruption.size()));
