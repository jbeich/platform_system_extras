...
static inline int verity_hash(const std::vector<uint8_t> &salt,
                              const uint8_t *block, uint8_t *hash) {
  SHA256_CTX ctx;
  SHA256_Init(&ctx);
...
bool verity_check_block(const uint8_t *expected, const uint8_t *block,
  ...
  if (unlikely(verity_hash(salt, block, hash) == -1)) {
...
static int parse_table(fec_handle *f, uint64_t offset, uint32_t size, bool useecc)
{
    ...
    if (v->metadata_start < v->hashtree.hash_start) {
      ...
      check(v->data_blocks == v->hashtree.hash_start / FEC_BLOCKSIZE);
    }
    ...
    if (!(f->flags & FEC_VERITY_DISABLE)) {
      ...
      if (verity_hash(v->hashtree.salt, zero_block.data(),
                      v->hashtree.zero_hash.data()) == -1) {
        error("failed to hash");
        return -1;
      }
    }
