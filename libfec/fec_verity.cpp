...
static bool read_hashes(fec_handle *f, uint64_t hash_offset,
...
static int verify_tree(fec_handle *f, const uint8_t *root)
{
    ...
    if (!read_hashes(f, 0, NULL, hash_offset, data) ||
    ...
    for (uint32_t i = 1; i < levels; ++i) {
        ...
        for (uint32_t j = 0; j < blocks; ++j) {
            ...
            if (!__raw_pread(f, hash, SHA256_DIGEST_SIZE,
                    hash_offset + j * SHA256_DIGEST_SIZE) ||
                !__raw_pread(f, data, FEC_BLOCKSIZE,
            ...
            if (!verity_check_block(f, VERITY_NO_CACHE, hash, data)) {
                ...
                /* update the corrected blocks to the file if we are in r/w
                   mode */
                if (f->mode & O_RDWR) {
                    if (!__raw_pwrite(f, hash, SHA256_DIGEST_SIZE,
                            hash_offset + j * SHA256_DIGEST_SIZE) ||
                        !__raw_pwrite(f, data, FEC_BLOCKSIZE,
                            data_offset + j * FEC_BLOCKSIZE)) {
        ...
        uint32_t size)
{
    ...
    std::vector<std::string> tokens = android::base::Split(v->table, " ");
...
int verity_parse_header(fec_handle *f, uint64_t offset)
{
    ...
    if (!__raw_pread(f, &raw_header, sizeof(raw_header), offset)) {
        error("failed to read verity header: %s", strerror(errno));
        return -1;
    }
