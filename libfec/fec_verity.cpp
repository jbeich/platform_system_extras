...
static int parse_uint64(const char* src, uint64_t maxval, uint64_t* dst) {
    ...
    if (*src == '\0' || *end != '\0' || (errno == ERANGE && value == ULLONG_MAX)) {
        errno = EINVAL;
        return -1;
    }
...
uint64_t verity_get_size(uint64_t file_size, uint32_t* verity_levels, uint32_t* level_hashes,
...
bool hashtree_info::check_block_hash(const uint8_t* expected, const uint8_t* block) {
    check(block);
...
bool hashtree_info::check_block_hash_with_index(uint64_t index, const uint8_t* block) {
...
// available.
bool hashtree_info::ecc_read_hashes(fec_handle* f, uint64_t hash_offset, uint8_t* hash,
                                    uint64_t data_offset, uint8_t* data) {
    check(f);

    if (hash && fec_pread(f, hash, digest_length_, hash_offset) != digest_length_) {
        error("failed to read hash tree: offset %" PRIu64 ": %s", hash_offset, strerror(errno));
        return false;
    }
    ...
    if (fec_pread(f, data, FEC_BLOCKSIZE, data_offset) != FEC_BLOCKSIZE) {
        error("failed to read hash tree: data_offset %" PRIu64 ": %s", data_offset,
              strerror(errno));
        return false;
    }
...
int hashtree_info::verify_tree(const fec_handle* f, const uint8_t* root) {
    ...
    uint64_t hash_size =
            verity_get_size(data_blocks * FEC_BLOCKSIZE, &levels, NULL, padded_digest_length_);
    ...
    if (!raw_pread(f->fd, data, FEC_BLOCKSIZE, hash_offset) || !check_block_hash(root, data)) {
        /* try to correct */
        if (!ecc_read_hashes(const_cast<fec_handle*>(f), 0, nullptr, hash_offset, data) ||
            !check_block_hash(root, data)) {
            error("root hash invalid");
            return -1;
        } else if (f->mode & O_RDWR && !raw_pwrite(f->fd, data, FEC_BLOCKSIZE, hash_offset)) {
    ...
    verity_get_size(data_blocks * FEC_BLOCKSIZE, NULL, hashes, padded_digest_length_);
    ...
    for (uint32_t i = 1; i < levels; ++i) {
        ...
        for (uint32_t j = 0; j < blocks; ++j) {
            ...
            if (!raw_pread(f->fd, buffer.data(), padded_digest_length_,
                ...
                !raw_pread(f->fd, data, FEC_BLOCKSIZE, data_offset + j * FEC_BLOCKSIZE)) {
            ...
            if (!check_block_hash(buffer.data(), data)) {
                ...
                if (!ecc_read_hashes(const_cast<fec_handle*>(f),
                                     hash_offset + j * padded_digest_length_, buffer.data(),
                ...
                if (f->mode & O_RDWR) {
                    if (!raw_pwrite(f->fd, buffer.data(), padded_digest_length_,
                                    hash_offset + j * padded_digest_length_) ||
                        !raw_pwrite(f->fd, data, FEC_BLOCKSIZE, data_offset + j * FEC_BLOCKSIZE)) {
            ...
            if (blocks == hash_data_blocks) {
                std::copy(data, data + FEC_BLOCKSIZE, data_hashes.begin() + j * FEC_BLOCKSIZE);
            }
        }
...
static int parse_table(fec_handle* f, uint64_t offset, uint32_t size, bool useecc) {
    ...
    } else if (fec_pread(f, const_cast<char*>(table.data()), size, offset) != (ssize_t)size) {
    ...
    for (const auto& token : tokens) {
        switch (i++) {
            ...
            case 5: /* num_data_blocks */
                if (parse_uint64(token.c_str(), f->data_size / FEC_BLOCKSIZE, &data_blocks) == -1) {
                    error("invalid number of verity data blocks: %s", token.c_str());
                    return -1;
                }
                break;
            case 6: /* hash_start_block */
                if (parse_uint64(token.c_str(), f->data_size / FEC_BLOCKSIZE, &hash_start) == -1) {
    ...
    if (i < VERITY_TABLE_ARGS) {
        error("not enough arguments in verity table: %d; expected at least " stringify(
                      VERITY_TABLE_ARGS),
              i);
        return -1;
    }
...
static int validate_header(const fec_handle* f, const verity_header* header, uint64_t offset) {
    ...
    if (header->magic != VERITY_MAGIC && header->magic != VERITY_MAGIC_DISABLE) {
        return -1;
    }
    ...
    if (header->length < VERITY_MIN_TABLE_SIZE || header->length > VERITY_MAX_TABLE_SIZE) {
        error("invalid verity table size: %u; expected [" stringify(
                      VERITY_MIN_TABLE_SIZE) ", " stringify(VERITY_MAX_TABLE_SIZE) ")",
              header->length);
        return -1;
    }
...
int verity_parse_header(fec_handle* f, uint64_t offset) {
    ...
    if (offset > f->data_size - VERITY_METADATA_SIZE) {
        debug("failed to read verity header: offset %" PRIu64 " is too far", offset);
        return -1;
    }
    ...
    if (fec_pread(f, &v->ecc_header, sizeof(v->ecc_header), offset) != sizeof(v->ecc_header)) {
    ...
    if (validate_header(f, &v->header, offset)) {
        ...
        if (memcmp(v->header.signature, v->ecc_header.signature, sizeof(v->header.signature))) {
    ...
    if (parse_table(f, offset + sizeof(v->header), v->header.length, false) == -1 &&
        parse_table(f, offset + sizeof(v->header), v->header.length, true) == -1) {
        return -1;
    }

    /* if we corrected something while parsing metadata and we are in r/w
       mode, rewrite the corrected metadata */
    if (f->mode & O_RDWR && f->errors > errors && rewrite_metadata(f, offset) < 0) {
...
int fec_verity_set_status(struct fec_handle* f, bool enabled) {
    ...
    if (!raw_pwrite(f->fd, &magic, sizeof(magic), v->metadata_start)) {
        error("failed to update verity magic to %08x: %s", magic, strerror(errno));
        return -1;
    }

    warn("updated verity magic to %08x (%s)", magic, enabled ? "enabled" : "disabled");
    v->disabled = !enabled;
