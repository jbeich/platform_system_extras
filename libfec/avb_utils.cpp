...
int parse_vbmeta_from_footer(fec_handle* f, std::vector<uint8_t>* vbmeta) {
    ...
    if (!raw_pread(f->fd, &footer_read, AVB_FOOTER_SIZE, f->size - AVB_FOOTER_SIZE)) {
    ...
    if (!raw_pread(f->fd, vbmeta_data.data(), vbmeta_data.size(), vbmeta_offset)) {
        error("failed to read avb vbmeta: %s", strerror(errno));
        return -1;
    }

    if (auto status =
                avb_vbmeta_image_verify(vbmeta_data.data(), vbmeta_data.size(), nullptr, nullptr);
        status != AVB_VBMETA_VERIFY_RESULT_OK && status != AVB_VBMETA_VERIFY_RESULT_OK_NOT_SIGNED) {
...
int parse_avb_image(fec_handle* f, const std::vector<uint8_t>& vbmeta) {
    ...
    auto parse_descriptor = [](const AvbDescriptor* descriptor, void* user_data) {
        if (descriptor && avb_be64toh(descriptor->tag) == AVB_DESCRIPTOR_TAG_HASHTREE) {
    ...
    uint32_t salt_offset = sizeof(AvbHashtreeDescriptor) + hashtree_descriptor.partition_name_len;
    ...
    std::vector<uint8_t> salt(read_ptr + salt_offset,
                              read_ptr + salt_offset + hashtree_descriptor.salt_len);
    std::vector<uint8_t> root_hash(read_ptr + root_offset,
    ...
    check(hashtree_descriptor.fec_offset <= f->size - hashtree_descriptor.fec_size);
    ...
    std::string hash_algorithm = reinterpret_cast<char*>(hashtree_descriptor.hash_algorithm);
    int nid = -1;
    ...
    hashtree.initialize(hashtree_descriptor.tree_offset,
                        hashtree_descriptor.tree_offset / FEC_BLOCKSIZE, salt, nid);
